...
public class QueenAnt extends L2AttackableAIScript
{
	...
	public String onAttack(Npc npc, Creature attacker, int damage, L2Skill skill)
	{
		if (attacker instanceof Playable)
		{
			...
			if (npc.getNpcId() == QUEEN && !npc.getCast().isCastingNow())
			{
				if (skill != null && skill.getElement() == ElementType.FIRE && Rnd.get(100) < 70)
				{
					npc.setTarget(attacker);
					npc.getCast().doCast(FrequentSkill.QUEEN_ANT_STRIKE.getSkill());
				}
				else
				{
					...
					{
						npc.setTarget(attacker);
						npc.getCast().doCast((Rnd.get(10) < 8) ? FrequentSkill.QUEEN_ANT_STRIKE.getSkill() : FrequentSkill.QUEEN_ANT_SPRINKLE.getSkill());
					}
					else if (dist < 250 && Rnd.get(100) < 5)
					{
						npc.setTarget(attacker);
						npc.getCast().doCast(FrequentSkill.QUEEN_ANT_BRANDISH.getSkill());
					}
				}
			}
		}
		...
		switch (npc.getNpcId())
		{
			...
			case NURSE:
				...
				if (caller.getNpcId() == LARVA)
				{
					...
					npc.getCast().doCast(Rnd.nextBoolean() ? FrequentSkill.NURSE_HEAL_1.getSkill() : FrequentSkill.NURSE_HEAL_2.getSkill());
				}
				...
				else if (caller.getNpcId() == QUEEN)
				{
					...
					{
						npc.setTarget(caller);
						npc.getCast().doCast(FrequentSkill.NURSE_HEAL_1.getSkill());
					}
				}
				break;
		}
	...
	public String onSkillSee(Npc npc, Player caster, L2Skill skill, WorldObject[] targets, boolean isPet)
	{
		...
		if (npc.getNpcId() == QUEEN && !npc.getCast().isCastingNow() && skill.getAggroPoints() > 0 && Rnd.get(100) < 15)
		{
			npc.setTarget(realAttacker);
			npc.getCast().doCast(FrequentSkill.QUEEN_ANT_STRIKE.getSkill());
		}
