...
public final class RequestDestroyItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (player.getCast().isCastingNow() && player.getCurrentSkill().getSkill() != null && player.getCurrentSkill().getSkill().getItemConsumeId() == itemId)
		{
		...
		if (player.getCast().isCastingSimultaneouslyNow() && player.getCast().getLastSimultaneousSkillCast() != null && player.getCast().getLastSimultaneousSkillCast().getItemConsumeId() == itemId)
		{
			player.sendPacket(SystemMessageId.CANNOT_DISCARD_THIS_ITEM);
			return;
		}
