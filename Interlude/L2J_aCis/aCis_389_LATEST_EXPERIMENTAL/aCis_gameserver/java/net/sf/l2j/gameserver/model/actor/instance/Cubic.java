...
public class Cubic
{
	...
	public void useCubicContinuous(Cubic activeCubic, L2Skill skill, WorldObject[] targets)
	{
		for (WorldObject obj : targets)
		{
			...
			if (target instanceof Player && ((Player) target).isInDuel() && skill.getSkillType() == SkillType.DEBUFF && activeCubic.getOwner().getDuelId() == ((Player) target).getDuelId())
			{
				for (AbstractEffect effect : skill.getEffects(activeCubic.getOwner(), target))
				{
					if (effect != null)
	...
	public void useCubicDisabler(SkillType type, Cubic activeCubic, L2Skill skill, WorldObject[] targets)
	{
		for (WorldObject obj : targets)
		{
			if (!(obj instanceof Creature))
			...
			switch (type)
			{
				...
				case CANCEL_DEBUFF:
					...
					{
						if (effect.getSkill().isDebuff() && count < skill.getMaxNegatedEffects())
						{
							// Do not remove raid curse skills
							if (effect.getSkill().getId() != 4215 && effect.getSkill().getId() != 4515 && effect.getSkill().getId() != 4082)
							{
								effect.exit();
								if (count > -1)
									count++;
