...
public final class ItemInstance extends WorldObject implements Runnable, Comparable<ItemInstance>
{
	...
	public boolean isAvailable(Player player, boolean allowAdena, boolean allowNonTradable, boolean allowStoreBuy)
	{
		return ((!isEquipped() || allowStoreBuy) // Not equipped
			...
			&& (player.getCurrentSkill().getSkill() == null || player.getCurrentSkill().getSkill().getItemConsumeId() != getItemId()) && (!player.getCast().isCastingSimultaneouslyNow() || player.getCast().getLastSimultaneousSkillCast() == null || player.getCast().getLastSimultaneousSkillCast().getItemConsumeId() != getItemId()) && (allowNonTradable || isTradable()));
	}
	...
	public void onAction(Player player)
	{
		...
		player.getAI().tryTo(IntentionType.PICK_UP, this, false);
	}
	...
	public class ItemDropTask implements Runnable
	{
		...
		public final void run()
		{
			...
			if (_dropper != null)
			{
				Location dropDest = GeoEngine.getInstance().getValidLocation(_dropper.getX(), _dropper.getY(), _dropper.getZ(), _x, _y, _z);
				_x = dropDest.getX();
				_y = dropDest.getY();
				_z = dropDest.getZ();
			}
