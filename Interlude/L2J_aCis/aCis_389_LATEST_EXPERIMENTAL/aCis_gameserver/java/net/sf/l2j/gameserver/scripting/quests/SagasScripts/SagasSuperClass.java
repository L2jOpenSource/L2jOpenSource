...
public class SagasSuperClass extends Quest
{
	...
	public void giveHallishaMark(QuestState st2)
	{
		if (st2.getInt("spawned") == 0)
		{
			if (st2.getQuestItemsCount(Items[3]) >= 700)
			{
				...
				AddSpawn(st2, Archon);
				st2.set("spawned", "1");
				startQuestTimer("Archon Hellisha has despawned", Archon, st2.getPlayer(), 600000);
				
				// Attack player
				((Attackable) Archon).addDamageHate(st2.getPlayer(), 0, 99999);
				Archon.getAI().tryTo(IntentionType.ATTACK, st2.getPlayer(), null);
			}
			else
			{
		...
		if (st != null)
		{
			...
			else if (event.equalsIgnoreCase("7-1"))
			{
				...
				else if (st.getInt("spawned") == 0)
				{
					...
					st.set("spawned", "1");
					startQuestTimer("Mob_1 Timer 1", Mob_1, player, 500);
					startQuestTimer("Mob_1 has despawned", Mob_1, player, 300000);
					AddSpawn(st, Mob_1);
					htmltext = "7-02.htm";
				}
				else
			...
			else if (event.equalsIgnoreCase("10-1"))
			{
				if (st.getInt("Quest0") == 0)
				{
					...
					startQuestTimer("Mob_3 Timer 1", Mob_3, player, 500);
					startQuestTimer("Mob_2 Timer 1", Mob_2, player, 500);
					
					startQuestTimer("Mob_3 has despawned", Mob_3, player, 59000);
					startQuestTimer("Mob_2 has despawned", Mob_2, player, 60000);
	...
	{
		QuestState st = player.getQuestState(getName());
		if (name.equalsIgnoreCase("Mob_1 Timer 1"))
		{
			// Attack player
			((Attackable) npc).addDamageHate(st.getPlayer(), 0, 99999);
			npc.getAI().tryTo(IntentionType.ATTACK, st.getPlayer(), null);
		...
		else if (name.equalsIgnoreCase("Archon Hellisha has despawned"))
		{
			if (st != null)
				st.set("spawned", "0");
			DeleteSpawn(st, npc);
		}
		else if (name.equalsIgnoreCase("Mob_3 Timer 1"))
		{
			// Search the NPC.
			...
			if (_SpawnList.containsKey(Mob_2) && _SpawnList.get(Mob_2) == player.getObjectId())
			{
				...
				npc.getAI().tryTo(IntentionType.ATTACK, Mob_2, null);
				npc.broadcastNpcSay(Text[14]);
			}
		}
		else if (name.equalsIgnoreCase("Mob_3 has despawned"))
		{
			npc.broadcastNpcSay(Text[15]);
			st.set("Quest0", "2");
			DeleteSpawn(st, npc);
		}
		else if (name.equalsIgnoreCase("Mob_2 Timer 1"))
		{
			npc.broadcastNpcSay(Text[7]);
			startQuestTimer("Mob_2 Timer 2", npc, player, 1500);
			if (st.getInt("Quest1") == 45)
				st.set("Quest1", "0");
		}
		else if (name.equalsIgnoreCase("Mob_2 Timer 2"))
		{
			npc.broadcastNpcSay(Text[8].replace("PLAYERNAME", player.getName()));
			startQuestTimer("Mob_2 Timer 3", npc, player, 10000);
		}
		else if (name.equalsIgnoreCase("Mob_2 Timer 3"))
		{
			if (st.getInt("Quest0") == 0)
			{
				startQuestTimer("Mob_2 Timer 3", npc, player, 13000);
				if (Rnd.get(2) == 0)
		...
		else if (name.equalsIgnoreCase("Mob_2 has despawned"))
		{
			...
			if (st.getInt("Quest0") == 1 || st.getInt("Quest0") == 2 || st.getInt("Quest1") > 3)
			{
				...
				startQuestTimer("Mob_2 has despawned", npc, player, 1000);
		}
			...
			if (npcId == Archon_Minion)
			{
				...
				if (party != null)
				{
					...
					for (Player player1 : party.getMembers())
					{
						...
						if (st1 != null && player1.isIn3DRadius(player, Config.PARTY_RANGE))
						{
