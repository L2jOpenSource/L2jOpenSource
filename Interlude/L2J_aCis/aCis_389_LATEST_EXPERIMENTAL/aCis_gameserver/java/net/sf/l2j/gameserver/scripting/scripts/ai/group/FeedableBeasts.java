...
import net.sf.l2j.gameserver.scripting.quests.Q655_AGrandPlanForTamingWildBeasts;
...
public class FeedableBeasts extends L2AttackableAIScript
{
	...
	public void spawnNext(Npc npc, int growthLevel, Player player, int food)
	{
		...
		else
		{
			...
			if (MAD_COW_POLYMORPH.containsKey(nextNpcId))
				startQuestTimer("polymorph Mad Cow", nextNpc, player, 10000);
	...
	private static void testCrystalOfPurity(Player player, Npc npc)
	{
		...
		if (player.isClanLeader())
			st = player.getQuestState(Q655_AGrandPlanForTamingWildBeasts.qn);
		else
		{
			...
			if (leader.isIn3DRadius(npc, Config.PARTY_RANGE))
				st = leader.getQuestState(Q655_AGrandPlanForTamingWildBeasts.qn);
		}
		...
		final String toCheck = st.get("cond");
		if (toCheck == null || !toCheck.equalsIgnoreCase("1"))
			return;
		
		// Reward clan leader with a crystal of purity. Once reached 10, trigger cond 2.
		if (st.dropItemsAlways(CRYSTAL_OF_PURITY, 1, REQUIRED_CRYSTAL_COUNT))
			st.set("cond", "2");
	}
}