...
public class AdminCreateItem implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, Player activeChar)
	{
		...
		else if (command.equals("admin_reward_all"))
		{
			try
			{
				...
				activeChar.sendMessage(players.size() + " players rewarded with " + ItemData.getInstance().getTemplate(id).getName());
			}
			catch (Exception e)
			{
	...
	{
		final Item template = ItemData.getInstance().getTemplate(id);
		if (template == null)
		{
