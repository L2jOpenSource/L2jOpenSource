...
public class AdminBuffs implements IAdminCommandHandler
{
	...
	public static void showBuffs(Player activeChar, Creature target, int page)
	{
		...
		{
			final AbstractEffect effect = effects[i];
			if (effect != null)
				StringUtil.append(sb, "<tr><td>", effect.getSkill().getName(), "</td><td>", (effect.getSkill().isToggle()) ? "toggle" : effect.getPeriod() - effect.getTime() + "s", "</td><td><a action=\"bypass -h admin_stopbuff ", target.getObjectId(), " ", effect.getSkill().getId(), "\">Remove</a></td></tr>");
		}
	...
	private static void removeBuff(Player activeChar, int objId, int skillId)
	{
		...
		if (obj instanceof Creature)
		{
			...
			for (AbstractEffect effect : target.getAllEffects())
			{
				...
				{
					effect.exit();
					activeChar.sendMessage("Removed " + effect.getSkill().getName() + " level " + effect.getSkill().getLevel() + " from " + target.getName() + " (" + objId + ")");
				}
			}
