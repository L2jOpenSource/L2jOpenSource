...
public final class RequestDropItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (player.getCast().isCastingSimultaneouslyNow())
		{
			if (player.getCast().getLastSimultaneousSkillCast() != null && player.getCast().getLastSimultaneousSkillCast().getItemConsumeId() == item.getItemId())
			{
				player.sendPacket(SystemMessageId.CANNOT_DISCARD_THIS_ITEM);
				return;
			}
