...
public class GoldenRamMercenary extends Folk
{
	...
	{
		String filename = "data/html/default/" + getNpcId() + ".htm";
		...
		if (st != null)
		{
			...
			switch (getNpcId())
			{
				...
				case 31554:
					...
					if (cond >= 2)
						filename = "data/html/default/" + getNpcId() + "-1.htm";
					break;
				...
				case 31556:
					...
					if (cond == 2)
						...
						filename = "data/html/default/" + getNpcId() + "-2.htm";
					break;
			}
		}
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if (actualCommand.contains("buff"))
		{
			if (qs != null && qs.getInt("cond") == 3)
			{
				...
				final int[] buffData = data[Integer.valueOf(st.nextToken())];
				
				final int coins = buffData[2];
				int val = 3;
				
				if (qs.getQuestItemsCount(GOLDEN_RAM) >= coins)
				{
					qs.takeItems(GOLDEN_RAM, coins);
					getAI().tryTo(IntentionType.CAST, new SkillUseHolder(SkillTable.getInstance().getInfo(buffData[0], buffData[1]), player, false, false), null);
					val = 4;
				}
