...
public class SiegeGuardAI extends AttackableAI
{
	...
	protected void peaceMode()
	{
		...
		if (_globalAggro >= 0)
		{
			...
			final Creature hated = (Creature) ((_actor.isConfused()) ? getCurrentIntention().getFirstParameter() : npc.getMostHated());
			if (hated != null)
			{
	...
	protected void combatMode()
	{
		...
		if (attackTarget == null || _attackTimeout < System.currentTimeMillis() || !actor.isIn3DRadius(attackTarget, 2000))
		{
		...
		final double dist = actor.distance2D(attackTarget);
		...
		if (willCastASpell())
		{
			...
			if (Rnd.get(100) < 10 && !defaultList.isEmpty())
			{
				for (L2Skill sk : defaultList)
				{
					if (!checkSkillCastConditions(sk) || (sk.getCastRange() + range <= dist && !canAura(sk, attackTarget)))
						continue;
