...
public final class RequestActionUse extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		{
			case 0:
				if (player.isSitting() && !player.isStandingNow())
			...
			case 21: // Change Movement Mode (pet follow/stop)
				...
				if (summon.getFollowStatus() && !player.isIn3DRadius(summon, 2000))
					return;
			...
			case 22: // Attack (pet attack)
				...
				{
					summon.getAI().tryTo(IntentionType.FOLLOW, target, _shiftPressed);
					return;
				}
				...
				if (target instanceof Door)
				{
					if (((Door) target).isAutoAttackable(player) && summon.getNpcId() != SiegeSummon.SWOOP_CANNON_ID)
						summon.getAI().tryTo(IntentionType.ATTACK, target, _shiftPressed);
				}
				...
				{
					if (summon.isInsidePeaceZone(target))
					{
	...
	private boolean useSkill(int skillId, WorldObject target)
	{
		...
		if (player == null || player.isOperating())
			return false;
		...
		summon.getAI().tryTo(IntentionType.CAST, new SkillUseHolder(skill, skill.getCastTargetFor(summon), _ctrlPressed, _shiftPressed), null);
		return true;
	}
}