...
public class Zaken extends L2AttackableAIScript
{
		...
		if (name.equalsIgnoreCase("1001"))
		{
			if (GameTimeTaskManager.getInstance().isNight())
			{
				...
				if (npc.getAI().getCurrentIntention().getType() == IntentionType.ATTACK && !_hasTeleported)
				{
					...
					if (mostHated != null && mostHated.isIn3DRadius(_zakenLocation, 1500))
						willTeleport = false;
					...
					{
						VICTIMS.clear();
						npc.getCast().doCast(FrequentSkill.ZAKEN_SELF_TELE.getSkill());
					}
				}
				...
				if (npc.getAI().getCurrentIntention().getType() == IntentionType.ATTACK && mostHated != null)
				{
					if (_hate == 0)
					{
		...
		switch (skill.getId())
		{
			...
			case 4217: // Mass Teleport ; teleport victims and targeted Player, each on a defined, random Location.
				for (Player victim : VICTIMS)
				{
					...
					{
						((Attackable) npc).stopHating(victim);
	...
	private static void callSkills(Npc npc, WorldObject target)
	{
		...
		if (chance < 1)
			npc.getCast().doCast(FrequentSkill.ZAKEN_TELE.getSkill());
		else if (chance < 2)
			npc.getCast().doCast(FrequentSkill.ZAKEN_MASS_TELE.getSkill());
		else if (chance < 4)
			npc.getCast().doCast(FrequentSkill.ZAKEN_HOLD.getSkill());
		else if (chance < 8)
			npc.getCast().doCast(FrequentSkill.ZAKEN_DRAIN.getSkill());
		else if (chance < 15)
		{
			if (target != ((Attackable) npc).getMostHated() && npc.isIn3DRadius(target, 100))
				npc.getCast().doCast(FrequentSkill.ZAKEN_MASS_DUAL_ATTACK.getSkill());
		}
		
		if (Rnd.nextBoolean() && target == ((Attackable) npc).getMostHated())
			npc.getCast().doCast(FrequentSkill.ZAKEN_DUAL_ATTACK.getSkill());
	}
