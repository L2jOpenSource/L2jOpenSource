...
public abstract class StatParser<H extends AbstractHolder> extends AbstractParser<H>
{
	...
	protected Condition parseTargetCondition(Element element)
	{
		...
		for(Iterator<Attribute> iterator = element.attributeIterator(); iterator.hasNext();)
		{
			...
			else if(name.equalsIgnoreCase("mob_corpse"))
				cond = joinAnd(cond, new ConditionTargetMonsterCorpse(Boolean.valueOf(value)));				
		}
	...
	protected Condition parsePlayerCondition(Element element)
	{
		...
		for(Iterator<Attribute> iterator = element.attributeIterator(); iterator.hasNext();)
		{
			...
			if(name.equalsIgnoreCase("residence"))
			{
				...
				cond = joinAnd(cond, new ConditionPlayerEvent(Boolean.valueOf(value)));				
				...
				cond = joinAnd(cond, new ConditionPlayerSex(Integer.parseInt(value)));
