		...
		for(Iterator<Element> iterator = rootElement.elementIterator("account"); iterator.hasNext();)
		{
			...
			for(Iterator<Element> subIterator = element.elementIterator(); subIterator.hasNext();)
			{
				...
				else if("rates".equalsIgnoreCase(subElement.getName()))
				{
					...
					double questDropRate = subElement.attributeValue("quest_drop") == null ? 1. : Double.parseDouble(subElement.attributeValue("quest_drop"));
					double questRewardRate = subElement.attributeValue("quest_reward") == null ? 1. : Double.parseDouble(subElement.attributeValue("quest_reward"));
					double siegeFameReward = subElement.attributeValue("siege_fame_reward") == null ? 1. : Double.parseDouble(subElement.attributeValue("siege_fame_reward"));
					template.setRates(new PremiumAccountRates(expRate, spRate, adenaRate, dropRate, spoilRate, questDropRate, questRewardRate, siegeFameReward));
				}
				...
				else if("skills".equalsIgnoreCase(subElement.getName()))
				{
					for(Iterator<Element> nextIterator = subElement.elementIterator("skill"); nextIterator.hasNext();)
					{
						...
						template.attachSkill(SkillHolder.getInstance().getSkill(id, level));
					}
				}
			}
