...
import l2s.gameserver.listener.inventory.*;
...
public abstract class Inventory extends ItemContainer
{
	...
	final static ListenerList<Playable> globalListeners = new ListenerList<Playable>();

	public final static boolean addGlobalListener(Listener<Playable> listener)
	{
		return globalListeners.add(listener);
	}

	public final static boolean removeGlobalListener(Listener<Playable> listener)
	{
	...
	public static final int PAPERDOLL_DECO1_HI5 = 19;
	public static final int PAPERDOLL_DECO2_HI5 = 20;
	public static final int PAPERDOLL_DECO3_HI5 = 21;
	public static final int PAPERDOLL_DECO4_HI5 = 22;
	public static final int PAPERDOLL_DECO5_HI5 = 23;
	public static final int PAPERDOLL_DECO6_HI5 = 24;
	public static final int PAPERDOLL_BELT_HI5 = 25;

	public static final int PAPERDOLL_MAX_HF = 26;
	...
	{
		public void onRefreshEquip(ItemEquipResult result, int slot, ItemInstance item)
		{
			for(Listener<Playable> listener : globalListeners.getListeners())
			{
				if(OnEquipListener.class.isInstance(listener))
				{
					((OnEquipListener) listener).onUnequip(result, slot, item, getActor());
					((OnEquipListener) listener).onEquip(result, slot, item, getActor());
				}
			}
		...
		public void onAddItem(ItemInstance item)
		{
			for(Listener<Playable> listener : globalListeners.getListeners())
				if(OnAddItemListener.class.isInstance(listener))
					((OnAddItemListener) listener).onAddItem(item, getActor());
		...
		public void onModifyItem(ItemInstance item)
		{
			for(Listener<Playable> listener : globalListeners.getListeners())
				if(OnModifyItemListener.class.isInstance(listener))
					((OnModifyItemListener) listener).onModifyItem(item, getActor());
		...
		public void onRemoveItem(ItemInstance item)
		{
			for(Listener<Playable> listener : globalListeners.getListeners())
				if(OnRemoveItemListener.class.isInstance(listener))
					((OnRemoveItemListener) listener).onRemoveItem(item, getActor());
		...
		public void onDestroyItem(ItemInstance item)
		{
			for(Listener<Playable> listener : globalListeners.getListeners())
				if(OnDestroyItemListener.class.isInstance(listener))
					((OnDestroyItemListener) listener).onDestroyItem(item, getActor());
	...
	public int getPaperdollVisualId(int slot)
	{
		...
		if(item != null)
		{
			if(slot == PAPERDOLL_GLOVES || slot == PAPERDOLL_LEGS || slot == PAPERDOLL_FEET)
			{
				ItemInstance chestItem = _paperdoll[PAPERDOLL_CHEST];
				if(chestItem != null && chestItem.getVisualId() > 0)
				{
					ItemTemplate visualItemTemplate = ItemHolder.getInstance().getTemplate(chestItem.getVisualId());
					if(visualItemTemplate != null && visualItemTemplate.getBodyPart() == ItemTemplate.SLOT_FORMAL_WEAR)
						return chestItem.getVisualId();
				}
			}

			if(slot == PAPERDOLL_LEGS)
			{
				ItemInstance chestItem = _paperdoll[PAPERDOLL_CHEST];
				if(chestItem != null && chestItem.getVisualId() > 0)
				{
					ItemTemplate visualItemTemplate = ItemHolder.getInstance().getTemplate(chestItem.getVisualId());
					if(visualItemTemplate != null && visualItemTemplate.getBodyPart() == ItemTemplate.SLOT_FULL_ARMOR)
						return chestItem.getVisualId();
				}
			}
			...
			case ItemTemplate.SLOT_L_HAND:
			{
				...
				if (newItem.getItemType() == EtcItemType.ARROW)
				{
					...
					if (rHandItemTemplate.getItemType() != WeaponType.BOW)
						return;
					if (rHandItemTemplate.getCrystalType().externalOrdinal != newItem.getCrystalType().externalOrdinal)
						return;
				}
	...
	public void refreshEquip(ItemEquipResult result)
	{
		isRefresh = true;
		for(ItemInstance item : getItems())
		{
			if(item.isEquipped())
				_listeners.onRefreshEquip(result, item.getEquipSlot(), item);
			else if(item.getItemType() == EtcItemType.RUNE)
				_listeners.onRefreshEquip(result, -1, item);
		}
		isRefresh = false;
	}
