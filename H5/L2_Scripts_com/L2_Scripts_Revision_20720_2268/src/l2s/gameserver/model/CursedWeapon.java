...
public class CursedWeapon
{
	...
	public boolean dropIt(NpcInstance attackable, Player killer, Player owner)
	{
		...
		Skill skill = SkillHolder.getInstance().getSkill(_skillId, player.getSkillLevel(_skillId));
		if(skill != null)
	...
	private Collection<Skill> getSkills(Player player)
	{
		...
		Skill skill = SkillHolder.getInstance().getSkill(_skillId, level);
		List<Skill> ret = new ArrayList<Skill>();
		ret.add(skill);
		for(AddedSkill s : skill.getAddedSkills())
			ret.add(SkillHolder.getInstance().getSkill(s.id, s.level));
		return ret;
	}
	...
	public void increaseKills()
	{
		...
		if(newLevel > oldLevel)
			player.broadcastPacket(new SocialActionPacket(player.getObjectId(), SocialActionPacket.CURSED_WEAPON_LEVEL_UP));
	}

	/**
	 * Функция для обновления скилла и стат игрока.
	 */
