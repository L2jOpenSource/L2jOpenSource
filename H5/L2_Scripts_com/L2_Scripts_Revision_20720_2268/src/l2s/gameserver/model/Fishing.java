...
import l2s.gameserver.network.l2.s2c.ExFishingHpRegenPacket;
import l2s.gameserver.network.l2.s2c.ExFishingStartPacket;
import l2s.gameserver.network.l2.s2c.ExFishingStartCombatPacket;
...
public class Fishing
{
	...
	public void startFishing()
	{
		...
		_fisher.broadcastPacket(new ExFishingStartPacket(_fisher, _fish.getType(), _fisher.getFishLoc(), isNightLure(_lureId)));
	...
	public void stopFishing()
	{
		...
		_fisher.broadcastPacket(new ExFishingEndPacket(_fisher, false));
		_fisher.broadcastCharInfo();
	...
	public void endFishing(boolean win)
	{
		...
		_fisher.setFishing(false);
		_fisher.broadcastPacket(new ExFishingEndPacket(_fisher, win));
		_fisher.broadcastCharInfo();
	...
	private class FishCombatTask extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			else
			{
				...
				ExFishingHpRegenPacket efhr = new ExFishingHpRegenPacket(_fisher, _time, _fishCurHP, _combatMode, 0, _anim, 0, _deceptiveMode);
				if(_anim != 0)
	...
	private void startFishCombat()
	{
		...
		ExFishingStartCombatPacket efsc = new ExFishingStartCombatPacket(_fisher, _time, _fish.getHP(), _combatMode, _fish.getGroup(), _deceptiveMode);
	...
	private void changeHp(int hp, int pen)
	{
		...
		_fisher.broadcastPacket(new ExFishingHpRegenPacket(_fisher, _time, _fishCurHP, _combatMode, _gooduse, _anim, pen, _deceptiveMode));
	...
	private void doDie(boolean win)
	{
		...
		if(win)
			...
			else
			{
				...
				ItemFunctions.addItem(_fisher, _fish.getId(), 1, true, "Fishing drop");
