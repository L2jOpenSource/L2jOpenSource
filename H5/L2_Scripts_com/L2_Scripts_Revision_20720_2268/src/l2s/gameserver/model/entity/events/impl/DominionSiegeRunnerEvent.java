...
public class DominionSiegeRunnerEvent extends Event
{
		...
		if(id == 1)
		{
			...
			switch(val)
			{
				...
				case -1:
					...
					break;
				case 3600:
					broadcastToWorld(new SystemMessagePacket(SystemMsg.THE_TERRITORY_WAR_WILL_END_IN_S1HOURS).addInteger(val / 3600));
					break;
				case 600:
				case 300:
				case 60:
					broadcastToWorld(new SystemMessagePacket(SystemMsg.THE_TERRITORY_WAR_WILL_END_IN_S1MINUTES).addInteger(val / 60));
					break;
				case 10:
				case 5:
				case 4:
				...
				case 2:
				case 1:
					broadcastToWorld(new SystemMessagePacket(SystemMsg.S1_SECONDS_TO_THE_END_OF_TERRITORY_WAR).addInteger(val));
					break;
			}
		}
	}
	...
	public synchronized void registerDominion(Dominion d)
	{
		...
		{	
			...
			if(castle.getOwnDate().getTimeInMillis() == 0)
				return;		
			//start a block using a first DAO values
			long terrSiegeDate = DominionDAO.getInstance().getDominionSiegeDate(d.getId());
			...
			{
				d.getSiegeDate().setTimeInMillis(terrSiegeDate);
				_startTime.setTimeInMillis(terrSiegeDate);
				reCalcNextTime(true);
			}
		}
		else
	...
	public synchronized void unRegisterDominion(Dominion d)
	{
		...
		DominionDAO.getInstance().update01(d.getId(), 0);		
