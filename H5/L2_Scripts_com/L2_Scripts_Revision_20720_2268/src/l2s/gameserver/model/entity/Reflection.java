...
import l2s.gameserver.listener.zone.impl.AirshipControllerZoneListener;
...
public class Reflection
{
			...
			switch(s.getSpawnType())
			{
				case 0: // точечный спаун, в каждой указанной точке
					for(Location loc : s.getCoords())
					{
						...
						c.setRespawnPattern(null); // TODO
						c.setAmount(s.getCount());
						c.setLoc(loc);
						c.doSpawn(true);
						if(s.getRespawnDelay() == 0 && c.getRespawnPattern() == null)
							c.stopRespawn();
						else
							c.startRespawn();
						addSpawn(c);
	...
	/**
	 * Данный метод открывает и закрывает двери в инстансах.
	 *
	 * @param doorId
	 * @param flag (1 - открыть дверь, 0 - закрыть дверь) 
	 */
	public void Castle_GateOpenCloseEx(int doorId, int flag)
	{
		DoorInstance door = _doors.get(doorId);
		if(flag == 1)
		{
