...
public final class FestivalGuideInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		{
			player.sendMessage(new CustomMessage("common.FestivalGuide.ErrorNotParticipant", player));
			return;
		}
		...
		else if(command.startsWith("Festival"))
		{
			...
			switch(val)
			{
				...
				case 2: // Festival 2 xxxx
					...
					{
						player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2FestivalGuideInstance.InProgress", player));
						return;
					}
					...
					for(Player pc : playerParty.getPartyMembers())
					{
						long count = ItemFunctions.getItemCount(pc, SevenSignsFestival.FESTIVAL_BLOOD_OFFERING);
						if(count > 0)
						{
							showChatWindow(player, 2, "m", false);
							return;
						}	
					}
					...
					{
						player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2FestivalGuideInstance.NotEnoughSSType", player));
						return;
					}
					player.sendPacket(SystemMessagePacket.removeItems(stoneType, stonesNeeded));
				...
				case 9: // Leave the Festival
					...
					if(playerParty.isLeader(player))
						...
						player.sendMessage(new CustomMessage("common.FestivalGuide.Leave", player));
					break;
				default:
