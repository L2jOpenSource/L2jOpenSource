...
public class PetInstance extends Servitor
{
		...
		public void runImpl() throws Exception
		{
			...
			else
			{
				...
				if(getCurrentFed() <= 0.10 * getMaxFed())
				{
					...
					owner.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2PetInstance.UnSummonHungryPet", owner));
					unSummon();
					return;
				}
			}
	...
	public static final PetInstance restore(ItemInstance control, NpcTemplate template, Player owner)
	{
		...
		try
		{
			...
			if(!rset.next())
			{
				if(PetDataTable.isBabyPet(template.getNpcId()) || PetDataTable.isImprovedBabyPet(template.getNpcId()) || PetDataTable.isSpecialPet(template.getNpcId()))
	...
	public PetInstance(int objectId, NpcTemplate template, Player owner, ItemInstance control, int currentLevel, long exp)
	{
		...
		if(PetDataTable.isSpecialPet(template.npcId))
		{
			_level = owner.getLevel();
	...
	public void addExpAndSp(long addToExp, long addToSp)
	{
		...
		if(old_level < _level)
		{
			owner.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2PetInstance.PetLevelUp", owner).addNumber(_level));
	...
	public int getMaxHp()
	{
		return Math.max(1, (int) calcStat(Stats.MAX_HP, _data.getHP(), null, null));
	}

	@Override
	public int getMaxMp()
	{
		return Math.max(1, (int) calcStat(Stats.MAX_MP, _data.getMP(), null, null));
	}
	...
	public void updateControlItem()
	{
		...
		controlItem.update();
		Player owner = getPlayer();
		owner.sendPacket(new InventoryUpdatePacket().addModifiedItem(owner, controlItem));
	}
