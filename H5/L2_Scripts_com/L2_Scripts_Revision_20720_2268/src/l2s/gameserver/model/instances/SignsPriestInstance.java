		...
		else if(command.startsWith("SevenSigns"))
		{
			...
			switch(val)
			{
				case 2: // Purchase Record of the Seven Signs
					...
					player.sendPacket(SystemMessagePacket.obtainItems(SevenSigns.RECORD_SEVEN_SIGNS_ID, 1, 0));
				...
				case 4: // Join a Cabal - SevenSigns 4 [0]1 x
					...
					{
						player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2SignsPriestInstance.AlreadyMember", player).addString(SevenSigns.getCabalName(cabal)));
						return;
					}
					if(player.getClassId().isOfLevel(ClassLevel.NONE))
					{
						player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2SignsPriestInstance.YouAreNewbie", player));
						break;
					}

					else if(player.getClassLevel() >= 2)
						if(Config.ALT_GAME_REQUIRE_CASTLE_DAWN)
							if(getPlayerAllyHasCastle(player))
							{
								if(cabal == SevenSigns.CABAL_DUSK)
								{
									player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2SignsPriestInstance.CastleOwning", player));
									return;
								}
							}
							else
								...
								if(cabal == SevenSigns.CABAL_DAWN)
								{
									...
									{
										ItemFunctions.deleteItem(player, SevenSigns.CERTIFICATE_OF_APPROVAL_ID, 1);
										allowJoinDawn = true;
									}
									...
									if(!allowJoinDawn)
									{
										if(Config.ALT_GAME_ALLOW_ADENA_DAWN)
											player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2SignsPriestInstance.CastleOwningCertificate", player));
										else
											player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2SignsPriestInstance.CastleOwningCertificate2", player));
										return;
									}
								}
				...
				case 6: // Contribute Seal Stones - SevenSigns 6 x
					...
					else
					{
						...
						if(!stonesFound)
						{
							player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2SignsPriestInstance.DontHaveAnySSType", player));
							return;
						}
						if(SevenSigns.getInstance().getCurrentPeriod() == SevenSigns.PERIOD_COMP_RESULTS)
						{
							player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2SignsPriestInstance.CannotContributeDuringCompRes", player));
							return;						
						}
				...
				case 7: // Exchange Ancient Adena for Adena - SevenSigns 7 xxxxxxx
					...
					if(player.getInventory().destroyItemByItemId(SevenSigns.ANCIENT_ADENA_ID, ancientAdenaConvert))
					{
						...
						player.sendPacket(SystemMessagePacket.removeItems(5575, ancientAdenaConvert));
						player.sendPacket(SystemMessagePacket.obtainItems(57, ancientAdenaConvert, 0));
					}
					break;
				case 9: // Receive Contribution Rewards
					...
					if(SevenSigns.getInstance().isSealValidationPeriod() && playerCabal == winningCabal)
					{
						...
						player.sendPacket(SystemMessagePacket.obtainItems(SevenSigns.ANCIENT_ADENA_ID, ancientAdenaReward, 0));
				...
				case 17: // Exchange Seal Stones for Ancient Adena (Type Choice) - SevenSigns 17 x
					...
					if(stoneType == 4)
					{
						...
						if(ancientAdenaReward > 0)
						{
							if(BlueStoneInstance != null)
							{
								...
								player.sendPacket(SystemMessagePacket.removeItems(SevenSigns.SEAL_STONE_BLUE_ID, bcount));
							}
							if(GreenStoneInstance != null)
							{
								player.getInventory().destroyItem(GreenStoneInstance, gcount);
								player.sendPacket(SystemMessagePacket.removeItems(SevenSigns.SEAL_STONE_GREEN_ID, gcount));
							}
							if(RedStoneInstance != null)
							{
								player.getInventory().destroyItem(RedStoneInstance, rcount);
								player.sendPacket(SystemMessagePacket.removeItems(SevenSigns.SEAL_STONE_RED_ID, rcount));
							}
							...
							player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2SignsPriestInstance.DontHaveAnySS", player));
						break;
					}
					...
					content = HtmCache.getInstance().getHtml(path, player);
				...
				case 18: // Exchange Seal Stones for Ancient Adena - SevenSigns 18 xxxx xxxxxx
					...
					if(convertCount <= totalCount && convertCount > 0)
					{
						...
						if(player.getInventory().destroyItemByItemId(convertStoneId, convertCount))
						{
							...
							ancientAdena.setCount(ancientAdenaReward);
							player.getInventory().addItem(ancientAdena);
							player.sendPacket(SystemMessagePacket.removeItems(convertStoneId, convertCount), SystemMessagePacket.obtainItems(SevenSigns.ANCIENT_ADENA_ID, ancientAdenaReward, 0));
						}
					}
					else
						player.sendMessage(new CustomMessage("l2s.gameserver.model.instances.L2SignsPriestInstance.DontHaveSSAmount", player));
					break;
				...
				case 20: // Seal Status (for when joining a cabal)
					...
					NpcHtmlMessagePacket html2 = new NpcHtmlMessagePacket(player, this);
				...
				case 22:
					...
					if(ItemFunctions.deleteItem(player, 57, adenaConvert, true))
					{
						...
						ItemFunctions.addItem(player, SevenSigns.ANCIENT_ADENA_ID, result, true, "Exchange Adena to Ancient Adena");
