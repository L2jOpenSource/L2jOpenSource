...
import l2s.gameserver.network.l2.s2c.FriendAddRequestResult;
import l2s.gameserver.network.l2.s2c.L2FriendPacket;
import l2s.gameserver.network.l2.s2c.L2FriendListPacket;
import l2s.gameserver.network.l2.s2c.L2FriendStatusPacket;
...
public class FriendList
{
	...
	public void remove(String name)
	{
		...
		if(objectId > 0)
		{
			...
			_owner.sendPacket(new SystemMessage(SystemMessage.S1_HAS_BEEN_REMOVED_FROM_YOUR_FRIEND_LIST).addString(name), new L2FriendPacket(name, false, friendChar != null, objectId));

			if(friendChar != null)
				friendChar.sendPacket(new SystemMessage(SystemMessage.S1__HAS_BEEN_DELETED_FROM_YOUR_FRIENDS_LIST).addString(_owner.getName()), new L2FriendPacket(_owner, false));
		}
		else
	...
	public void notifyFriends(boolean login)
	{
		for(Friend friend : values())
		{
			...
			friendPlayer.sendPacket(new L2FriendStatusPacket(thisFriend, login));

			friend.update(friendPlayer, login);
		}
	}

	public IStaticPacket requestFriendInvite(GameObject target)
	{
		...
		player.sendPacket(new FriendAddRequestResult(_owner.getName()));
		return null;
	}
