...
public class MacroList
{
	...
	public void registerMacro(Macro macro)
	{
		if(macro.id == 0)
		{
			...
			if(player.isHFClient())
				...
				player.sendPacket(new MacroListPacket(_revision, macro.id, MacroListPacket.Action.ADD, 1, macro));
		}
		else
		{
			...
			if(player.isHFClient())
				...
				player.sendPacket(new MacroListPacket(_revision, macro.id, MacroListPacket.Action.UPDATE, 1, macro));
		}
	}
	...
	public void deleteMacro(int id)
	{
		...
		if(player.isHFClient())
			...
			player.sendPacket(new MacroListPacket(_revision, id, MacroListPacket.Action.DELETE, 0, null));
	}
	...
	public void sendMacroses()
	{
		...
		int size = size();
		if(size == 0)
			player.sendPacket(new MacroListPacket(_revision, 0, MacroListPacket.Action.ADD, 0, null));
		else
		{
			for(Macro macro : _macroses.values())
				player.sendPacket(new MacroListPacket(_revision, 0, MacroListPacket.Action.ADD, size, macro));
		}
	}
