...
public class InstantZone
{
	...
	private final List<InstantZoneEntryType> _entryType;
	...
	public InstantZone(int id, String name, SchedulingPattern resetReuse, int reuseDelay, int sharedReuseGroup, int timelimit, boolean dispelBuffs, int minLevel, int maxLevel, int minParty, int maxParty, int timer, boolean onPartyDismiss, List<Location> tele, Location ret, IntObjectMap<DoorInfo> doors, Map<String, ZoneInfo> zones,  Map<String, SpawnInfo2> spawns, List<SpawnInfo> spawnsInfo, int collapseIfEmpty, int maxChannels, int removedItemId, int removedItemCount, boolean removedItemNecessity, int giveItemId, int givedItemCount, int requiredQuestId, boolean setReuseUponEntry, StatsSet params)
	{
		...
		_entryType = new ArrayList<InstantZoneEntryType>();

		if(getMinParty() == 1)
			_entryType.add(InstantZoneEntryType.SOLO);

		if(getMinParty() <= 9 && getMaxParty() > 1)
			_entryType.add(InstantZoneEntryType.PARTY);

		if(getMaxParty() > 9)
			_entryType.add(InstantZoneEntryType.COMMAND_CHANNEL);
			
		else if(getMaxParty() == 0)
			_entryType.add(InstantZoneEntryType.EVENT);

		if(_id >= 160 && _id <= 163)
		{
			for(InstantZoneEntryType iz : _entryType)
				System.out.println("ID: " + _id + ", ENTER TYPE: " + iz);
		}
			
		if (_entryType.isEmpty())
	...
	public InstantZoneEntryType getEntryType(Player player)
	{
		...
		if(party != null)
		{
			if(party.getCommandChannel() != null && _entryType.contains(InstantZoneEntryType.COMMAND_CHANNEL))
				return InstantZoneEntryType.COMMAND_CHANNEL;

			if(_entryType.contains(InstantZoneEntryType.PARTY))
				return InstantZoneEntryType.PARTY;
		}

		if(_entryType.contains(InstantZoneEntryType.SOLO))
			return InstantZoneEntryType.SOLO;

		if(_entryType.contains(InstantZoneEntryType.PARTY))
			return InstantZoneEntryType.PARTY;

		if(_entryType.contains(InstantZoneEntryType.COMMAND_CHANNEL))
			return InstantZoneEntryType.COMMAND_CHANNEL;
