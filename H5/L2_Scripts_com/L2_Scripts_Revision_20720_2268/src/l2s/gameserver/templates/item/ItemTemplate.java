...
public abstract class ItemTemplate extends StatTemplate
{
	...
	private final int _masterworkConvert;
	...
	private List<CapsuledItem> _capsuledItems = new ArrayList<CapsuledItem>();
	...
	protected ItemTemplate(final StatsSet set)
	{
		...
		_masterworkConvert = set.getInteger("masterwork_convert", -1);
	...
	{
		return getItemType() == WeaponType.NONE;
	}
	...
	{
		return _bodyPart == SLOT_HAIR || _bodyPart == SLOT_DHAIR || _bodyPart == SLOT_HAIRALL;
	}
	...
	public boolean testCondition(Playable player, ItemInstance instance, boolean sendMsg)
	{
		if(_condition == null)
			return true;
		...
		if(!res && sendMsg && _condition.getSystemMsg() != null)
		{
			if(_condition.getSystemMsg().size() > 0)
				player.sendPacket(new SystemMessagePacket(_condition.getSystemMsg()).addItemName(getItemId()));
			else
	...
	public boolean isBlocked(Playable playable, ItemInstance instance)
	{
		...
		if(!_condition.test(env) && _condition.getSystemMsg() == null)
			return true;
