...
public enum InstantZoneEntryType
{
	...
	private static SystemMsg checkPlayer(Player player, InstantZone instancedZone)
	{
		...
		if(instancedZone.getRemovedItemId() == -100)
		{
			...
			{
				player.reducePcBangPoints(instancedZone.getRemovedItemCount(), true);
			}
		}
		else if(instancedZone.getRemovedItemId() == -200)
		{
			...
			{
				player.getClan().setReputationScore(player.getClan().getReputationScore() - instancedZone.getRemovedItemCount());
			}
		}
		else if(instancedZone.getRemovedItemId() == -300)
		{
			...
			{
				player.setFame(player.getFame() - instancedZone.getRemovedItemCount(), "instance");
			}
		}
