	...
	public static void storeEnchantItemsOly(Player player)
	{
		...
		for (int i = 0; i < len; i++)
		{
			...
			if(isMaxEnchant(_item))
			{
				...
				player.sendPacket(new InventoryUpdatePacket().addModifiedItem(player, _item));
	...
	public static void restoreEnchantItemsOly(Player player)
	{
		...
		if(var != null)
		{
			...
			for(String item : items)
			{
				...
				player.sendPacket(new InventoryUpdatePacket().addModifiedItem(player, itemToEnchant));
				player.broadcastUserInfo(true);
				player.broadcastCharInfo();
			}
		}
