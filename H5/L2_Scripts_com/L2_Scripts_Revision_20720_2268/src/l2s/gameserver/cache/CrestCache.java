...
public class CrestCache
{
	...
	public static final int HF_LARGE_CREST_SIZE = 2176;
	...
	private final TIntIntMap _pledgeCrestLargeIdHF = new TIntIntHashMap();
	...
	private final TIntObjectMap<byte[]> _pledgeCrestLargeHF = new TIntObjectHashMap<byte[]>();
	...
	public void load()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				_pledgeCrestLargeIdHF.put(pledgeId, crestId);
				_pledgeCrestLargeHF.put(crestId, crest);
			}
	...
	public byte[] getPledgeCrestLargeHF(int crestId)
	{
		byte[] crest = null;
		...
		try
		{
			crest = _pledgeCrestLargeHF.get(crestId);
		}
		finally
		{
			readLock.unlock();
	...
	public int getPledgeCrestLargeIdHF(int pledgeId)
	{
		int crestId = 0;
		...
		try
		{
			crestId = _pledgeCrestLargeIdHF.get(pledgeId);
		}
		finally
		{
			readLock.unlock();
	...
	public void removePledgeCrestLargeHF(int pledgeId)
	{
		writeLock.lock();
		try
		{
			_pledgeCrestLargeHF.remove(_pledgeCrestLargeId.remove(pledgeId));
		}
		finally
		{
			writeLock.unlock();
	...
	public int savePledgeCrestLargeHF(int pledgeId, byte[] crest)
	{
		int crestId = getCrestId(pledgeId, crest);
