...
public class SecondaryPasswordAuth
{
	...
	private void loadPassword()
	{
		...
		if(password != null && !password.equals(""))
		{
			...
			if(wrongAttempts != null && !wrongAttempts.equals(""))
				_wrongAttempts = Integer.parseInt(wrongAttempts);
				
			else if(wrongAttempts.equals(""))
				_wrongAttempts = 0;
		}
	}
	...
	public boolean savePassword(String password)
	{
		...
		String accountName = _activeClient.getLogin();
		if(AccountVariablesDAO.getInstance().select(accountName, VAR_PWD) == null || AccountVariablesDAO.getInstance().select(accountName, VAR_PWD) == "")
	...
	{
		String accountName = _activeClient.getLogin();
		if(AccountVariablesDAO.getInstance().select(accountName, VAR_WTE) == null || AccountVariablesDAO.getInstance().select(accountName, VAR_WTE) == "")
