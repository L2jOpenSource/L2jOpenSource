	...
	protected void runImpl()
	{
		...
		try
		{
			...
			for(int i = 0; i < _count; i++)
			{
				...
				Log.LogEvent(activeChar.getName(), activeChar.getIP(), "WareHouse", "WareHouse WithDraw Item:", ""+item.getName()+" count: "+item.getCount()+" objId: "+item.getObjectId()+" from NPC " + (whkeeper == null ? "BBS" : whkeeper.getNpcId()));								
