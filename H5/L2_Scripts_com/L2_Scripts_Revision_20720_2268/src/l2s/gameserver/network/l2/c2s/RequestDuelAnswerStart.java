...
public class RequestDuelAnswerStart extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(activeChar.getOlympiadGame() != null || Olympiad.isRegistered(activeChar))
		{
			request.cancel();
			activeChar.sendActionFailed();
			activeChar.sendPacket(new SystemMessagePacket(SystemMsg.C1_CANNOT_DUEL_BECAUSE_C1_IS_PARTICIPATING_IN_THE_OLYMPIAD).addName(activeChar));
			return;
		}
		...
		switch(_response)
		{
			...
			case 1:
				...
				try
				{
					...
					Log.LogEvent(activeChar.getName(), "Duel", "StartDuel", "duel started with: "+requestor.getName()+"");
					Log.LogEvent(requestor.getName(), "Duel", "StartDuel", "duel started with: "+activeChar.getName()+"");
				}
				finally
				{
					request.done();
