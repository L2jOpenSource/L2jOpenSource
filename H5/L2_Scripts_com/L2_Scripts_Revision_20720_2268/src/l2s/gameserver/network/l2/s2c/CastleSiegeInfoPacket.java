...
public class CastleSiegeInfoPacket extends L2GameServerPacket
{
	...
	public CastleSiegeInfoPacket(Castle castle, Player player)
	{
	...
	public CastleSiegeInfoPacket(ClanHall ch, Player player)
	{
		this((Residence)ch, player);

		_startTime = ch.getSiegeEvent() != null ? (int) (ch.getSiegeDate().getTimeInMillis() / 1000) : 0;
	}
	public CastleSiegeInfoPacket(Fortress fortes, Player player)
	{
		this((Residence)fortes, player);

		_startTime = fortes.getSiegeEvent() != null ? (int) (fortes.getSiegeDate().getTimeInMillis() / 1000) : 0;
	}

	protected CastleSiegeInfoPacket(Residence residence, Player player)
	{
		_id = residence.getId();
