...
public class RequestBuyItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(!activeChar.isGM() && (merchant == null || !isValidMerchant || !activeChar.checkInteractionDistance(merchant)))
		{
			activeChar.sendActionFailed();
			return;
		}
		...
		try
		{
			...
			for(TradeItem ti : buyList)
			{
				...
				if(merchant != null)
					Log.LogEvent(activeChar.getName(), "BuyItem", "BoughItems", "bought: "+ti.getCount()+" of "+ti.getItemId()+" from NPC ID: "+merchant.getNpcId()+"");
				Log.LogItem(activeChar, Log.ShopBuy, ti.getItemId(), ti.getCount());
			}	
