...
public class ExEnchantSkillInfoDetailPacket extends L2GameServerPacket
{
	...
	public ExEnchantSkillInfoDetailPacket(int skillId, int skillLvl, int sp, int chance, int bookId, int adenaCount)
	{
		_skillId = skillId;
		_skillLvl = skillLvl;
		_sp = sp;
	...
	protected void writeImpl()
	{
		writeD(_unk); // ?
		writeD(_skillId);
		writeD(SkillUtils.convertHFSkillLevelToGODMask(_skillId, _skillLvl));
		writeQ(_sp);
		writeD(_chance);
