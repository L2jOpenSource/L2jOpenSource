...
public class PvPRewardManager
{
	...
	private static final boolean no_msg = false;//Config.DISALLOW_MSG_TO_PL;
	...
	private static boolean basicCheck(Player killed, Player killer)
	{
		...
		if(killed.getClassLevel() < Config.PVP_REWARD_MIN_PL_PROFF)
		{
			if(!no_msg)
			{
				if(killer.isLangRus())
		...
		if(Config.PVP_REWARD_ONLY_BATTLE_ZONE && !killer.isInZone(ZoneType.battle_zone))
		{
			if(!no_msg)
			{
				if(killer.isLangRus())
					killer.sendMessage("Система PvP: Вы убили игрока но разрешено убивать игроков только на боевых площадках ( не Арены, олимпиада и т.п ).");
				else
					killer.sendMessage("PvP System: You killed a player, it's allowed to kill players only on battle fields (not such as Arenas, olympiad etc.)");
			}			
			return false;
		}
		...
		if(Config.PVP_REWARD_ONLY_NOBLE_GIVE && !killed.isNoble())
		{
			if(!no_msg)
			{
				if(killer.isLangRus())
					killer.sendMessage("Система PvP: Вы убили игрока но он был не ноблесом.");	
				else
					killer.sendMessage("PvP System: You killed a player, but this players isn't noblesse");
			}		
			return false;
		}
	...
	private static boolean checkEquip(Player killed)
	{
		...
		if(weapon.getCrystalType().externalOrdinal < Config.PVP_REWARD_WEAPON_GRADE_TO_CHECK)
			return false;
	...
	{
		if(!Config.ALLOW_PVP_REWARD_SYSTEM)
			return;

		if(Config.PVP_REWARDS_ZONES.length > 0)
		{
			boolean check = false;
			for(String zone : Config.PVP_REWARDS_ZONES)
			{
				if(victim.isInZone(zone))
				{
					check = true;
					break;
				}
	...
	private static void giveItem(Player player)
	{
		...
		if(Config.PVP_REWARD_RANDOM_ONE)
		{
			...
			if(rewardId > 0 && rewardCount > 0)
				ItemFunctions.addItem(player, rewardId, rewardCount, true, "PvP reward manager");
		}
		else
		{
			for(int i = 0 ; i < Config.PVP_REWARD_REWARD_IDS.length - 1 ; i++)
			{
				...
				if(rewardId > 0 && rewardCount > 0)
					ItemFunctions.addItem(player, Config.PVP_REWARD_REWARD_IDS[i], Config.PVP_REWARD_COUNTS[i], true, "PvP reward manager");
			}
		}
	}
