...
public class CursedWeaponsManager
{
	...
	public void endOfLife(CursedWeapon cw)
	{
		if(cw.isActivated())
		{
			...
			if(player != null)
			{
				...
				player.removeSkill(SkillHolder.getInstance().getSkill(cw.getSkillId(), player.getSkillLevel(cw.getSkillId())), false);
	...
	{
		if(killer.isInOlympiadMode() || killer.isCursedWeaponEquipped() || _cursedWeapons.length == 0 || killer.getReflection() != ReflectionManager.DEFAULT || killer.getOlympiadGame() != null || Olympiad.isRegisteredInComp(killer))
			return;
