...
public class ItemAuctionManager
{
	...
	private final TIntObjectMap<ItemAuctionInstance> _managerInstances = new TIntObjectHashMap<ItemAuctionInstance>();
	...
	private void load()
	{
						...
						if("instance".equalsIgnoreCase(nb.getNodeName()))
						{
							...
							for(Node nc = nb.getFirstChild(); nc != null; nc = nc.getNextSibling())
							{
								if("item".equalsIgnoreCase(nc.getNodeName()))
								{
									...
									int auctionBidItemId = Integer.parseInt(nac.getNamedItem("bid_item_id").getNodeValue());
									...
									AuctionItem item = new AuctionItem(auctionItemId, auctionLenght, auctionBidItemId, auctionInitBid, itemId, itemCount, itemExtra);
									items.add(item);
								}
							}
		...
		{
			_log.error("ItemAuctionManager: Error while loading item_auctions.xml!", e);
		}
	}
