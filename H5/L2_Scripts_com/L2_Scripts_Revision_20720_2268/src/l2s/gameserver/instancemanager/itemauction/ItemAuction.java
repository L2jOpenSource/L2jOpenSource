...
public class ItemAuction
{
	...
	private TIntObjectMap<ItemAuctionBid> _auctionBids;
	...
	public void registerBid(Player player, long newBid)
	{
		...
		if(newBid > Config.ALT_ITEM_AUCTION_MAX_BID)
		{
			if(Config.ALT_ITEM_AUCTION_MAX_BID == 100000000000L)
				...
				player.sendMessage(new CustomMessage("common.ItemAuction.LimitExceeded", player).addNumber(Config.ALT_ITEM_AUCTION_MAX_BID));
			return;
		}
	...
	private void onPlayerBid(Player player, ItemAuctionBid bid)
	{
		...
		if(getEndingTime() - System.currentTimeMillis() <= 1000 * 60 * 10)
			...
			else if(_auctionEndingExtendState == 1 && getAndSetLastBidPlayerObjectId(player.getObjectId()) != player.getObjectId())
			{
				...
				broadcastToAllBidders(SystemMsg.BIDDER_EXISTS_AUCTION_TIME_HAS_BEEN_EXTENDED_BY_3_MINUTES);
			}
	}

	public void broadcastToAllBidders(IStaticPacket packet)
	{
	...
	public void cancelBid(Player player)
	{
		...
		synchronized (_auctionBids)
		{
			...
			{
				player.sendMessage(new CustomMessage("common.ItemAuction.CancelBid", player));
				return;
			}
	...
	{
		if(_auctionItem.getActionBidItemId() == ItemTemplate.ITEM_ID_ADENA)
		{
		...
		return player.getInventory().destroyItemByItemId(_auctionItem.getActionBidItemId(), count);
	}
	...
	private void increaseItemCount(Player player, long count)
	{
		if(_auctionItem.getActionBidItemId() == ItemTemplate.ITEM_ID_ADENA)
			...
			player.getInventory().addItem(_auctionItem.getActionBidItemId(), count);
		player.sendPacket(SystemMessagePacket.obtainItems(_auctionItem.getActionBidItemId(), count, 0));
	}
