...
public final class HandysBlockCheckerManager
{
	...
	public boolean addPlayerToArena(Player player, int arenaId)
	{
		...
		synchronized (holder)
		{
			...
			holder.broadCastPacketToTeam(new ExBlockUpSetList.AddPlayer(player, isRed));
			return true;
		}
	}
	...
	public void removePlayer(Player player, int arenaId, int team)
	{
		...
		synchronized (holder)
		{
			...
			holder.broadCastPacketToTeam(new ExBlockUpSetList.RemovePlayer(player, isRed));
	...
	public void changePlayerToTeam(Player player, int arena, int team)
	{
		...
		synchronized (holder)
		{
			...
			{
				player.sendMessage(new CustomMessage("common.Handys.TeamFull", player));
				return;
			}
			...
			holder.broadCastPacketToTeam(new ExBlockUpSetList.ChangeTeam(player, isFromRed));
		}
	}
