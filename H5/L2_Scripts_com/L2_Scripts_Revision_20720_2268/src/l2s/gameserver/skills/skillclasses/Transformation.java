...
public class Transformation extends Skill
{
	...
	public boolean checkCondition(final Creature activeChar, final Creature target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		if(player.isInFlyingTransform() && getId() == SKILL_TRANSFORM_DISPEL && Math.abs(player.getZ() - player.getLoc().correctGeoZ(player.getGeoIndex()).z) > 333)
		{
			activeChar.sendPacket(new SystemMessage(SystemMessage.S1_CANNOT_BE_USED_DUE_TO_UNSUITABLE_TERMS).addSkillName(_id, _level));
			return false;
		}

		if(!Config.ALT_USE_TRANSFORM_IN_EPIC_ZONE && getId() != SKILL_TRANSFORM_DISPEL && player.isInZone(ZoneType.epic))
		{
			activeChar.sendPacket(new SystemMessage(SystemMessage.S1_CANNOT_BE_USED_DUE_TO_UNSUITABLE_TERMS).addSkillName(_id, _level));
			return false;
		}
	...
	public void useSkill(Creature activeChar, List<Creature> targets)
	{
		if(useSummon)
		{
			...
			activeChar.getServitor().unSummon();
		}
