...
import l2s.gameserver.model.reward.RewardItemRates;
...
public abstract class RewardListInfo
{
	...
	public static String makeRewardListInfo(Player player, TIntStringHashMap tpls, NpcInstance npc, List<RewardList> rewardLists, RewardType groupType, String groupName, double penaltyMod, boolean show, int page)
	{
		if(show)
		{
				...
				for(RewardGroup g : list)
				{
					...
					if(type == RewardType.RATED_GROUPED)
					{
						if(g.isAdena())
						{
							...
							grate = npc.calcStat(Stats.RATE_ITEMS, rateAdena, player, null);
						}
						else
						{
							...
							grate = npc.calcStat(Stats.RATE_ITEMS, rateDrop, player, null);
							gpmod *= Config.DROP_CHANCE_MODIFIER;
						}
					}
					else if(type == RewardType.SWEEP)
					{
						grate = npc.calcStat(Stats.RATE_SPOIL, player.getRateSpoil(), player, null);
						gpmod *= Config.SPOIL_CHANCE_MODIFIER;
					}
					...
					double itemMaxChance = (double) RewardList.MAX_CHANCE / items.size() * Math.min(items.size(), Config.MAX_DROP_ITEMS_FROM_ONE_GROUP) / RewardList.MAX_CHANCE;
					...
					for(RewardData d : items)
					{
						...
						double irate = d.notRate() ? 1.0 : grate * RewardItemRates.getRateModifier(d.getItem());
					...
					itemMaxChance += ((normalChancesCount * itemMaxChance) - normalChancesSum) / (items.size() - normalChancesCount);
					...
					for(RewardData d : items)
					{
						...
						if(items.size() > Config.MAX_DROP_ITEMS_FROM_ONE_GROUP)
