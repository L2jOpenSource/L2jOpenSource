...
public class FantasyIsle extends Functions implements ScriptFile
{
	...
	private static String[] TEXT = new String[] {"How come people are not here... We are about to start the show.. Hmm", "Ugh, I have butterflies in my stomach.. The show starts soon...", "Thank you all for coming here tonight.", "It is an honor to have the special show today.", "Our Fantasy Isle is fully committed to your happiness.", "Now I'd like to introduce the most beautiful singer in Aden. Please welcome Leyla Mira!", "Here she comes!", "Thank you very much, Leyla. Next is", "It was very difficult to invite this first group that just came back from their world tour. Let's welcome the Fantasy Isle Circus!", "Come on ~ everyone", "Did you like it? That was so amazing.", "Now we also invited individuals with special talents.", "Let's welcome the first person here!", ";;;;;;Oh", "Okay, now here comes the next person. Come on up please.", "Oh, it looks like something great is going to happen, right?", "Oh, my ;;;;", "That's g- .. great. Now, here comes the last person.", "Now this is the end of today's show.", "How was it? I am not sure if you really enjoyed it.", "Please remember that Fantasy Isle is always planning a lot of great shows for you.", "Well, I wish I could continue all night long, but this is it for today. Thank you."};
	...
	public void start(String event)
	{
		...
		else if(event.equals("6") && temp_npc != null)
		{
			...
			temp_npc.broadcastPacket(new PlaySoundPacket(PlaySoundPacket.Type.MUSIC, "NS22_F", 0, 0, 0, 0, 0));
		...
		else if(event.equals("10") && temp_npc != null)
		{
			...
			temp_npc.broadcastPacket(new PlaySoundPacket(PlaySoundPacket.Type.MUSIC, "TP05_F", 0, 0, 0, 0, 0));
		...
		else if(event.equals("29") && temp_npc != null)
		{
			...
			temp_npc.broadcastPacket(new SocialActionPacket(temp_npc.getObjectId(), 1));
