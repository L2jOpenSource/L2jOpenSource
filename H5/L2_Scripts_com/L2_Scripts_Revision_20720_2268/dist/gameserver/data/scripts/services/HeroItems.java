...
import static l2s.gameserver.model.entity.olympiad.Olympiad.OLYMPIAD_HTML_PATH;
...
public class HeroItems extends Functions
{
	...
	public void getweapon(String[] var)
	{
		...
		if(player.isHero())
		{
			...
			for(String heroItem[] : HERO_ITEMS)
			{
				...
				if(player.getInventory().getItemByItemId(itemId) != null || player.getWarehouse().getItemByItemId(itemId) != null)
				{
					player.getInventory().destroyItemByItemId(itemId, -1);
					fileName += "monument_weapon_have.htm";
					have = true;
					break;
				}
			...
			if(!have)
			{
				ItemFunctions.addItem(player, item, 1, true, "Give hero weapon");
	...
	public String getcir()
	{
		...
		if(player.isHero())
		{
			...
			{
				ItemFunctions.addItem(player, 6842, 1, true, "Give hero circle"); //Wings of Destiny Circlet
