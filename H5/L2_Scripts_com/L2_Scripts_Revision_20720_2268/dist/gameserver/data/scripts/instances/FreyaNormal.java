...
public class FreyaNormal extends Reflection
{
	...
	private class FirstStage extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			firstStageGuardSpawn = ThreadPoolManager.getInstance().scheduleAtFixedRate(new GuardSpawnTask(1), 2000L, 90000L);
		}
	}
	...
	private class TimerToSecondStage extends RunnableImpl
	{
		...
		{
			for(Player p : getPlayers())
				p.sendPacket(new ExSendUIEventPacket(p, false, false, 60, 0, NpcString.TIME_REMAINING_UNTIL_NEXT_BATTLE));
	...
	private class SecondStage extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			secondStageGuardSpawn = ThreadPoolManager.getInstance().scheduleAtFixedRate(new GuardSpawnTask(2), 2000L, 90000L);
	...
	private class ThirdStage extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			for(Player p : getPlayers())
			{
				...
				p.sendPacket(ExChangeClientEffectInfo.STATIC_FREYA_DESTROYED);
			}
			thirdStageGuardSpawn = ThreadPoolManager.getInstance().scheduleAtFixedRate(new GuardSpawnTask(3), 2000L, 90000L);
	...
	public class ZoneListenerL implements OnZoneEnterLeaveListener
	{
		...
		{
			if(cha.isPlayer())
				cha.sendPacket(ExChangeClientEffectInfo.STATIC_FREYA_DEFAULT);
		}
