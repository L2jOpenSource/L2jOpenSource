...
public class CrystalCaverns extends Reflection
{
	...
	public void notifyTearsDead(NpcInstance npc)
	{
		...
		for(Creature c : npc.getAroundCharacters(600, 300))
			if(c.isPlayer())
				ItemFunctions.addItem(c.getPlayer(), 9697, 1, true, "On Tears dead reward"); //Clear Crystal
	}
	...
	private void notifySteamStarted()
	{
		...
		for(Player p : getPlayers())
			p.altOnMagicUseTimer(p, SkillHolder.getInstance().getSkill(5239, 1));
	...
	public void notifyNextLevel(NpcInstance npc)
	{
		...
		for(Player p : getPlayers())
		{
			p.altOnMagicUseTimer(p, SkillHolder.getInstance().getSkill(5239, level));
	...
	public void notifyKechiDead(NpcInstance npc)
	{
		...
		for(Creature c : npc.getAroundCharacters(600, 300))
			if(c.isPlayer())
				ItemFunctions.addItem(c.getPlayer(), 9696, 1, true, "On Kechi dead reward"); //Red Crystal
		if(failureTimer != null)
		{
			failureTimer.cancel(false);
			failureTimer = null;
	...
	public void notifyDarnelDead(NpcInstance npc)
	{
		...
		for(Creature c : npc.getAroundCharacters(600, 300))
			if(c.isPlayer())
				ItemFunctions.addItem(c.getPlayer(), 9695, 1, true, "On Darnel dead reward"); // Blue Crystal
	}
