...
{
	//change this only if you are running an english server!
	...
	private void reward(Party party)
	{
		for(Player member : party.getPartyMembers())
		{
			if(isLangRus)
				...
				member.sendMessage("Your group has won the GvG event, we added the group leader to the rating table.");
			member.setFame(member.getFame() + Config.GvG_FAME_REWARD, "GvG"); // fame
			ItemFunctions.addItem(member, Config.GvG_REWARD, Config.GvG_REWARD_COUNT, "GvG event reward"); // Fantasy Isle Coin
		}
	}
		...
		public void onDeath(Creature self, Creature killer)
		{
			...
			else if(self.isNpc() && killer.isPlayable()) //onKill - mob death
			{
				...
				else if(self.getNpcId() == BOSS_ID)
				{
					...
					if(isLangRus)
						broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 5000, ScreenMessageAlign.MIDDLE_CENTER, true, "Охранник Сокровищ Геральда погиб от руки " + killer.getName()));
					else
						broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 5000, ScreenMessageAlign.MIDDLE_CENTER, true, "The guardian of the tresure has been killed by " + killer.getName()));
					end();
				}
			}
		}
	...
	private void removePlayer(Player player, boolean legalQuit)
	{
		...
		player.teleToLocation(Location.findPointToStay(GvG.RETURN_LOC, 0, 150, ReflectionManager.DEFAULT.getGeoIndex()), ReflectionManager.DEFAULT);
	...
	private void teamWithdraw(Party party)
	{
		...
		if(isLangRus)
			broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 4000, ScreenMessageAlign.MIDDLE_CENTER, true, "Команда соперника покинула поле боя в полном составе. Конец сражения."));
		else
			broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 4000, ScreenMessageAlign.MIDDLE_CENTER, true, "The opponents group has left the battle zone. End of the battle."));
		end();
	}
	...
	public class BossSpawn extends RunnableImpl
	{
		...
		{
			if(isLangRus)
				broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 5000, ScreenMessageAlign.MIDDLE_CENTER, true, "Появился Охранник Сокровищ Геральда"));
			else
				broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 5000, ScreenMessageAlign.MIDDLE_CENTER, true, "The guardian of treasures Geralda has spotet in this area!"));
	...
	public class CountingDown extends RunnableImpl
	{
		...
		{
			if(isLangRus)
				broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 4000, ScreenMessageAlign.MIDDLE_CENTER, true, "До конца сражения осталась 1 минута"));
			else
				broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 4000, ScreenMessageAlign.MIDDLE_CENTER, true, "Until the end of the battle 1 minute remains"));
		}
	}
	...
	public class BattleEnd extends RunnableImpl
	{
		...
		{
			if(isLangRus)
				broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 4000, ScreenMessageAlign.BOTTOM_RIGHT, true, "Время битвы истекло. Телепортация через 1 минуту."));
			else
				broadCastPacketToBothTeams(new ExShowScreenMessage(NpcString.NONE, 4000, ScreenMessageAlign.BOTTOM_RIGHT, true, "Battle time is up. Teleporting in 1 minute."));
			end();
		}
	}
	...
	private class TeleportListener implements OnTeleportListener
	{
		...
		public void onTeleport(Player player, int x, int y, int z, Reflection reflection)
		{
			...
			if(isLangRus)
				...
				player.sendMessage("You've left the battle zone, and has been disqualified.");
		}
	}
