...
public class LafiLakfi extends DefaultAI
{
	...
	private static final Skill s_display_bug_of_fortune1 = SkillHolder.getInstance().getSkill(6045, 1);
	private static final Skill s_display_jackpot_firework = SkillHolder.getInstance().getSkill(5778, 1);
		...
		if(_nextEat < System.currentTimeMillis())
		{
			...
			if(closestItem != null && closestItem.getCount() >= Config.MIN_ADENA_TO_EAT)
			{
				...
				if(i_ai2 == 2 && getFirstSpawned(actor))
				{
					...
					actor.doDie(actor);
					actor.deleteMe();
					addTimer(1500, 0, null, Config.TIME_IF_NOT_FEED*60000); //config me
				}
