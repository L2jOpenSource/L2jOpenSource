...
public class Baium extends DefaultAI
{
	...
	{
		return getActor().getReflection() == ReflectionManager.DEFAULT;
	}
	...
	protected void onEvtAttacked(Creature attacker, int damage)
	{
		...
		if(_firstTimeAttacked)
		{
			...
			if(attacker == null)
				return;
			if(attacker.isPlayer() && attacker.getServitor() != null)
				attacker.getServitor().doDie(actor);
	...
	protected boolean maybeMoveToHome(boolean force)
	{
		...
		if(actor.getReflection() == ReflectionManager.DEFAULT)
		{
			if(actor != null && BaiumManager.getZone() != null && !BaiumManager.getZone().checkIfInZone(actor))
				teleportHome();
		}
		return false;
	}
