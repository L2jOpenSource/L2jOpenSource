...
public class FreyaStandHard extends Fighter
{
	...
	protected void thinkAttack()
	{
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkill(Skill_EternalBlizzard, 1), actor, true);
		...
		if(!actor.isCastingNow() && !actor.isMoving && _iceballReuseTimer < System.currentTimeMillis())
		{
			...
			{
				actor.doCast(SkillHolder.getInstance().getSkill(Skill_IceBall, 1), topDamager, true);
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkill(Skill_SummonElemental, 1), actor, true);
			for(NpcInstance guard : getActor().getAroundNpc(800, 100))
				guard.altOnMagicUseTimer(guard, SkillHolder.getInstance().getSkill(Skill_SummonElemental, 1));
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkill(Skill_SelfNova, 1), actor, true);
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkill(Skill_ReflectMagic, 1), actor, true);
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkill(Skill_IceStorm, 1), actor, true);
		...
		if(!actor.isCastingNow() && !actor.isMoving && _deathsentenceReuseTimer < System.currentTimeMillis())
		{
			...
			{
				actor.doCast(SkillHolder.getInstance().getSkill(Skill_DeathSentence, 1), randomHated, true);
		...
		{
			actor.doCast(SkillHolder.getInstance().getSkill(Skill_Anger, 1), actor, true);
