...
public class _108_JumbleTumbleDiamondFuss extends QuestScript
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 30523)
		{
			...
			else if(cond == 7 && st.getQuestItemsCount(GEM_BOX1) > 0)
			{
				...
				st.giveItems(COAL_PIECE, 1, false, false);
				st.setCond(8);
			}
			...
			else if(cond == 12 && st.getQuestItemsCount(STAR_DIAMOND) > 0)
			{
				...
				st.giveItems(SILVERSMITH_HAMMER, 1, false, false);
				...
				if(st.getPlayer().getClassId().isOfLevel(ClassLevel.NONE) && !st.getPlayer().getVarBoolean("p1q3"))
				{
					...
					st.giveItems(5789, 6000, false, false); // newbie ss
				}
				st.finishQuest();
			}
		}
		else if(npcId == 30516)
		{
			if(cond == 1 && st.getQuestItemsCount(GOUPHS_CONTRACT) > 0)
			{
				...
				st.giveItems(REEPS_CONTRACT, 1, false, false);
		...
		else if(npcId == 30529)
		{
			if(cond == 3 && st.getQuestItemsCount(ELVEN_WINE) > 0)
			{
				...
				st.giveItems(BRONPS_DICE, 1, false, false);
		...
		else if(npcId == 30526)
		{
			...
			else if(cond == 6 && st.getQuestItemsCount(BRONPS_CONTRACT) > 0 && st.getQuestItemsCount(AQUAMARINE) >= 10 && st.getQuestItemsCount(CHRYSOBERYL) >= 10)
			{
				...
				st.giveItems(GEM_BOX1, 1, false, false);
				st.setCond(7);
			}
			else if(cond == 7 && st.getQuestItemsCount(GEM_BOX1) > 0)
			...
			else if(cond == 8 && st.getQuestItemsCount(COAL_PIECE) > 0)
			{
				...
				st.giveItems(BRONPS_LETTER, 1, false, false);
				st.setCond(9);
			}
		...
		else if(npcId == 30521)
		{
			if(cond == 9 && st.getQuestItemsCount(BRONPS_LETTER) > 0)
			{
				...
				st.giveItems(BERRY_TART, 1, false, false);
				st.setCond(10);
			}
			...
			if(cond == 10 && st.getQuestItemsCount(BERRY_TART) > 0)
			{
				...
				st.giveItems(BAT_DIAGRAM, 1, false, false);
				st.setCond(11);
			}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 20323 || npcId == 20324)
		{
			if(cond == 5 && st.getQuestItemsCount(BRONPS_CONTRACT) > 0)
			{
				...
				{
					st.giveItems(AQUAMARINE, 1, true, true);
				...
				{
					st.giveItems(CHRYSOBERYL, 1, true, true);
			...
			if(cond == 11 && st.getQuestItemsCount(BAT_DIAGRAM) > 0 && st.getQuestItemsCount(STAR_DIAMOND) == 0)
				if(Rnd.chance(50))
				{
					...
					st.giveItems(STAR_DIAMOND, 1, false, false);
					st.setCond(12);
				}
		return null;
	}
