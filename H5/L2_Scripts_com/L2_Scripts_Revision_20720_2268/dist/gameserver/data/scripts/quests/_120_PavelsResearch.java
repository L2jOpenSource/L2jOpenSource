...
public class _120_PavelsResearch extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(16);
			st.giveItems(Enigma, 1, false, false);
		}
		...
		else if(event.equalsIgnoreCase("32041-32.htm"))
		{
			...
			st.giveItems(EarPhoenix, 1, false, false);
			st.giveItems(ADENA_ID, 783729, true, true);
		...
		{
			st.set("talk", "1");
			st.playSound(SOUND_ED_DRONE_02);
		}
		...
		{
			st.setCond(15);
			st.giveItems(Report, 1, false, false);
		...
		{
			st.setCond(6);
			st.giveItems(Flower, 1, false, false);
		}
		...
		{
			st.setCond(23);
			st.giveItems(Heart, 1, false, false);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Stones)
		{
			if(cond == 0)
			{
				if(st.getPlayer().getLevel() >= 70 && st.getPlayer().isQuestCompleted(114))
