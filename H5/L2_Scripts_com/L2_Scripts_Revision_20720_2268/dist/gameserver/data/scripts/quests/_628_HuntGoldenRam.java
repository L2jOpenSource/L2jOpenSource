...
public class _628_HuntGoldenRam extends QuestScript
{
	//Npcs
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("31554-03a.htm"))
		{
			if(st.getQuestItemsCount(CHITIN) >= 100 && st.getCond() == 1)
			{
				...
				st.giveItems(RECRUIT, 1, false, false);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(cond == 2)
		{
			if(chitin1 >= 100 && chitin2 >= 100)
			{
				...
				st.giveItems(SOLDIER, 1, false, false);
				st.getPlayer().updateRam();
				st.finishQuest();
			}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond >= 1 && 21507 < npcId && npcId < 21513)
		{
			...
			{
				st.giveItems(CHITIN, 1, true, true);
			...
			{
				st.giveItems(CHITIN2, 1, true, true);
