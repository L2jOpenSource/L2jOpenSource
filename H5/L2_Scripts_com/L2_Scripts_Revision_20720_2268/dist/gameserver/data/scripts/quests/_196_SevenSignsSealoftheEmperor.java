...
public class _196_SevenSignsSealoftheEmperor extends QuestScript
{
	// NPCs
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("shunaiman_q196_2.htm"))
		{
			...
			st.giveItems(SacredSwordofEinhasad, 1, false, false);
			st.giveItems(ElmoredenHolyWater, 1, false, false);
		}
		else if(event.equalsIgnoreCase("courtmagician_q196_2.htm"))
		{
			st.playSound(SOUND_ITEMGET);
			st.giveItems(CourtMagiciansMagicStaff, 1, false, false);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == IasonHeine)
		{
			if(cond == 0)
			{
				if(player.getLevel() >= 79 && player.isQuestCompleted(195))
