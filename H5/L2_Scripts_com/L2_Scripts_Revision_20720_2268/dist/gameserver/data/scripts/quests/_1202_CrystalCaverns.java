...
public class _1202_CrystalCaverns extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("meet"))
		{
			...
			st.giveItems(PRISON_KEY, 1, false, false);
		...
		else if(event.equalsIgnoreCase("out"))
		{
			if(player.getParty() != null)
			{
				...
				for(Player pl : player.getParty().getPartyMembers())
					pl.teleToLocation(149361, 172327, -945, ReflectionManager.DEFAULT);
			}
			else
				player.teleToLocation(149361, 172327, -945, ReflectionManager.DEFAULT);
			return null;
		}
		return event;
	}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == DARNEL)
		{
			...
			st.giveItems(Rnd.chance(50) ? WHITE_SEED_OF_EVIL_SHARD : BLACK_SEED_OF_EVIL_SHARD, seedsCount, false, false);
		}
		else if(npcId == KECHI)
		{
