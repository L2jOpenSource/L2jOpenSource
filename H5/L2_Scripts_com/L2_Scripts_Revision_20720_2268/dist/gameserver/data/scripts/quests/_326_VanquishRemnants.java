...
public class _326_VanquishRemnants extends QuestScript
{
	//NPC
			...
			else if(cond == 1)
			{
				if(st.getQuestItemsCount(RedCrossBadge) + st.getQuestItemsCount(BlueCrossBadge) + st.getQuestItemsCount(BlackCrossBadge) >= 100)
				{
					if(st.getQuestItemsCount(BlackLionMark) == 0)
					{
						...
						st.giveItems(BlackLionMark, 1, false, false);
					}
					else
				...
				st.giveItems(ADENA_ID, (st.getQuestItemsCount(RedCrossBadge) * 46) + (st.getQuestItemsCount(BlueCrossBadge) * 54) + (st.getQuestItemsCount(BlackCrossBadge) * 68), true, true);
