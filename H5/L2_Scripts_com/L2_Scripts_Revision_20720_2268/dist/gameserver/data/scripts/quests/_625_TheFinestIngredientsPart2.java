...
public class _625_TheFinestIngredientsPart2 extends QuestScript
{
	// NPCs
	...
	{
		int cond = st.getCond();
		if(event.equalsIgnoreCase("jeremy_q0625_0104.htm"))
		{
			if(st.getQuestItemsCount(Soy_Sauce_Jar) == 0)
				return "jeremy_q0625_0102.htm";
			st.setCond(1);
			st.takeItems(Soy_Sauce_Jar, 1);
			st.giveItems(Food_for_Bumbalump, 1, false, false);
		}
		else if(event.equalsIgnoreCase("jeremy_q0625_0301.htm") && cond == 3)
		{
			st.finishQuest();
		...
		else if(event.equalsIgnoreCase("yetis_table_q0625_0201.htm") && cond == 1)
		{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(st.getCond() == 1 || st.getCond() == 2)
		{
			if(st.getQuestItemsCount(Food_for_Bumbalump) > 0)
				st.takeItems(Food_for_Bumbalump, 1);
			st.giveItems(Special_Yeti_Meat, 1, false, false);
			st.setCond(3);
		}
