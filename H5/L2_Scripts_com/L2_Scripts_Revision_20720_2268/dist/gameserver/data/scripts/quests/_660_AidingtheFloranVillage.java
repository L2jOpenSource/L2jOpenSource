...
public class _660_AidingtheFloranVillage extends QuestScript
{
	// NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30291-05.htm"))
		{
			...
			{
				st.giveItems(ADENA_ID, (EYES * 100) + (SCALE * 100) + (SHARD * 100) + 9000, true, true);
			...
			{
				st.giveItems(ADENA_ID, (EYES * 100) + (SCALE * 100) + (SHARD * 100), true, true);
				st.takeItems(WATCHING_EYES, -1);
				st.takeItems(DELU_LIZARDMAN_SCALE, -1);
				st.takeItems(ROUGHLY_HEWN_ROCK_GOLEM_SHARD, -1);
			}
		...
		else if(event.equalsIgnoreCase("30291-11.htm"))
		{
			if(EYES + SCALE + SHARD >= 99)
			{
				...
				if(Rnd.chance(80))
				{
					st.giveItems(ADENA_ID, 13000, true, true);
					st.giveItems(SCROLL_ENCANT_ARMOR, 1, false, false);
				}
				else
		...
		else if(event.equalsIgnoreCase("30291-12.htm"))
		{
			if(EYES + SCALE + SHARD >= 199)
			{
				...
				if(luck < 9)
				{
					...
					st.giveItems(SCROLL_ENCHANT_WEAPON, 1, false, false);
				else
		...
		else if(event.equalsIgnoreCase("30291-13.htm"))
		{
			if(EYES + SCALE + SHARD >= 499)
			{
				...
				if(Rnd.chance(80))
				{
					st.giveItems(ADENA_ID, 45000, true, true);
					st.giveItems(SCROLL_ENCHANT_WEAPON, 1, false, false);
				}
				else
					st.giveItems(ADENA_ID, 5000, true, true);
				st.playSound(SOUND_ITEMGET);
			}
			else
				htmltext = "30291-14.htm";
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(st.getCond() == 2)
			...
			{
				st.giveItems(WATCHING_EYES, 1, true, true);
				st.playSound(SOUND_ITEMGET);
			}
			else if(npcId == ROUGH_HEWN_ROCK_GOLEM && chance < 75)
			{
				st.giveItems(ROUGHLY_HEWN_ROCK_GOLEM_SHARD, 1, true, true);
				st.playSound(SOUND_ITEMGET);
			}
			else if(npcId == 20781 | npcId == 21104 | npcId == 21107 | npcId == 21105 && chance < 67)
			{
				st.giveItems(DELU_LIZARDMAN_SCALE, 1, true, true);
				st.playSound(SOUND_ITEMGET);
			}
		return null;
	}
