...
public class _336_CoinOfMagic extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(COIN_DIAGRAM, 1, false, false);
			st.setCond(1);
		}
	...
	private String promote(QuestState st)
	{
		...
		else
		{
			...
			if(h == 6)
			{
				...
				st.giveItems(3811 + grade, 1, false, false);
			...
			else
			{
				if(st.getQuestItemsCount(COIN_DIAGRAM) > 0)
				{
					if(st.getQuestItemsCount(KALDIS_COIN) > 0)
					{
						...
						st.giveItems(MEMBERSHIP_3, 1, false, false);
						st.set("grade", "3");
						st.setCond(4);
						st.playSound(SOUND_FANFARE_MIDDLE);
