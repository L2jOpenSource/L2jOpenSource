...
public class _212_TrialOfDuty extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
		{
			...
			{
				st.giveItems(7562, 45, false, false);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 30109)
		{
			...
			else if(cond == 18 && st.getQuestItemsCount(LETTER_OF_DUSTIN_ID) > 0)
			{
				htmltext = "hannavalt_q0212_05.htm";
				st.takeItems(LETTER_OF_DUSTIN_ID, -1);
				st.giveItems(MARK_OF_DUTY_ID, 1, false, false);
		...
		else if(npcId == 30653)
		{
			if(cond == 1)
			{
				...
				if(st.getQuestItemsCount(OLD_KNIGHT_SWORD_ID) == 0)
					st.giveItems(OLD_KNIGHT_SWORD_ID, 1, false, false);
				st.setCond(2);
			}
		...
		else if(npcId == 30654)
		{
			...
			else if(cond == 6 && st.getQuestItemsCount(TALIANUSS_REPORT_ID) > 0)
			{
				...
				st.setCond(7);
				st.giveItems(MIRROR_OF_ORPIC_ID, 1, false, false);
			}
		...
		else if(npcId == 30656 && cond == 8 && st.getQuestItemsCount(MIRROR_OF_ORPIC_ID) > 0)
		{
			...
			st.giveItems(TEAR_OF_CONFESSION_ID, 1, false, false);
			st.setCond(9);
		}
		else if(npcId == 30655)
		{
			...
			else if(cond == 12 && st.getQuestItemsCount(MILITAS_ARTICLE_ID) >= 20)
			{
				...
				st.giveItems(TEAR_OF_LOYALTY_ID, 1, false, false);
				st.setCond(13);
			}
			else if(cond == 13)
		...
		else if(npcId == 30116)
		{
			...
			else if(cond == 15)
			{
				...
				st.giveItems(SAINTS_ASHES_URN_ID, 1, false, false);
				st.setCond(16);
			}
			else if(cond == 17 && st.getQuestItemsCount(LETTER_OF_WINDAWOOD_ID) > 0)
			{
				...
				st.giveItems(LETTER_OF_DUSTIN_ID, 1, false, false);
				st.setCond(18);
			}
			else if(cond == 16)
			...
			if(cond == 16 && st.getQuestItemsCount(SAINTS_ASHES_URN_ID) > 0)
			{
				...
				st.giveItems(LETTER_OF_WINDAWOOD_ID, 1, false, false);
				st.setCond(17);
			}
			else if(cond == 17)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			st.giveItems(KNIGHTS_TEAR_ID, 1, false, false);
			st.setCond(3);
		}
		...
		{
			st.giveItems(MILITAS_ARTICLE_ID, 1, true, true);
			...
			{
				st.giveItems(ATEBALTS_SKULL_ID, 1, false, false);
				st.playSound(SOUND_ITEMGET);
			}
			else if(st.getQuestItemsCount(ATEBALTS_RIBS_ID) == 0)
			{
				st.giveItems(ATEBALTS_RIBS_ID, 1, false, false);
				st.playSound(SOUND_ITEMGET);
			}
			else if(st.getQuestItemsCount(ATEBALTS_SHIN_ID) == 0)
			{
				st.giveItems(ATEBALTS_SHIN_ID, 1, false, false);
				st.setCond(15);
				st.playSound(SOUND_ITEMGET);
			}
		return null;
