...
public class _211_TrialOfChallenger extends QuestScript
{
	// Npcs
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
		{
			...
			{
				st.giveItems(7562, 61, false, false);
		...
		else if(event.equalsIgnoreCase("30647_1"))
		{
			...
			{
				st.giveItems(SCROLL_OF_SHYSLASSY_ID, 1, false, false);
				if(Rnd.chance(22))
				{
					...
					if(n > 90)
					{
						st.giveItems(MITHRIL_SCALE_GAITERS_MATERIAL_ID, 1, false, false);
						st.giveItems(BRIGANDINE_GAUNTLET_PATTERN_ID, 1, false, false);
						st.giveItems(MANTICOR_SKIN_GAITERS_PATTERN_ID, 1, false, false);
						st.giveItems(GAUNTLET_OF_REPOSE_OF_THE_SOUL_PATTERN_ID, 1, false, false);
						st.giveItems(IRON_BOOTS_DESIGN_ID, 1, false, false);
					}
					else if(n > 70)
					{
						st.giveItems(TOME_OF_BLOOD_PAGE_ID, 1, false, false);
						st.giveItems(ELVEN_NECKLACE_BEADS_ID, 1, false, false);
					}
					else if(n > 40)
						st.giveItems(WHITE_TUNIC_PATTERN_ID, 1, false, false);
					else
						st.giveItems(IRON_BOOTS_DESIGN_ID, 1, false, false);
				}
				else
				{
					...
					st.takeItems(BROKEN_KEY_ID, -1);
					st.giveItems(ADENA_ID, Rnd.get(1000) + 1, true, true);
				}
			}
			else
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Kash)
		{
			...
			else if(cond == 2)
			{
				if(st.getQuestItemsCount(SCROLL_OF_SHYSLASSY_ID) >= 1)
				{
					...
					st.giveItems(LETTER_OF_KASH_ID, 1, false, false);
					st.setCond(3);
				}
			}
			else if(cond >= 7)
		...
		else if(npcId == Martien)
		{
			...
			{
				if(st.getQuestItemsCount(WATCHERS_EYE1_ID) == 0)
		...
		else if(npcId == ChestOfShyslassys)
		{
			if(cond == 2)
		...
		else if(npcId == Raldo)
		{
			...
			else if(cond == 10)
			{
				...
				st.takeItems(BROKEN_KEY_ID, -1);
				st.giveItems(MARK_OF_CHALLENGER_ID, 1, false, false);
