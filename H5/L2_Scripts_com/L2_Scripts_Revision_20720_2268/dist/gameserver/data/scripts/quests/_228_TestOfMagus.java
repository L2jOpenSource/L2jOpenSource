...
public class _228_TestOfMagus extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(RukalsLetter, 1, false, false);
			st.setCond(1);
		...
		else if(event.equalsIgnoreCase("30391-02.htm"))
		{
			...
			st.giveItems(ParinasLetter, 1, false, false);
			st.setCond(2);
		}
		else if(event.equalsIgnoreCase("30612-02.htm"))
		{
			st.takeItems(ParinasLetter, -1);
			st.giveItems(LilacCharm, 1, false, false);
			st.setCond(3);
		}
		else if(event.equalsIgnoreCase("30629-10.htm"))
		{
			...
			st.giveItems(ScoreOfElements, 1, false, false);
			st.setCond(5);
		}
		else if(event.equalsIgnoreCase("30412-02.htm"))
		{
			st.giveItems(SylphCharm, 1, false, false);
			st.playSound(SOUND_MIDDLE);
		}
		else if(event.equalsIgnoreCase("30409-03.htm"))
		{
			st.giveItems(SerpentCharm, 1, false, false);
			st.playSound(SOUND_MIDDLE);
		}
		return htmltext;
	}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Rukal)
		{
			...
			else if(st.getQuestItemsCount(MarkOfMagus) != 0)
			{
			...
			else if(cond == 6)
			{
				...
				st.giveItems(MarkOfMagus, 1, false, false);
		...
		else if(npcId == Salamander && cond == 5)
		{
			if(st.getQuestItemsCount(ToneOfFire) == 0)
			{
				if(st.getQuestItemsCount(SalamanderCharm) == 0)
				{
					...
					st.giveItems(SalamanderCharm, 1, false, false);
					st.playSound(SOUND_MIDDLE);
				}
				...
				else
				{
					...
					st.giveItems(ToneOfFire, 1, false, false);
		...
		else if(npcId == Sylph && cond == 5)
		{
			if(st.getQuestItemsCount(ToneOfWind) == 0)
			{
				...
				else
				{
					...
					st.giveItems(ToneOfWind, 1, false, false);
		...
		else if(npcId == Snake && cond == 5)
		{
			if(st.getQuestItemsCount(ToneOfEarth) == 0)
			{
				...
				else
				{
					...
					st.giveItems(ToneOfEarth, 1, false, false);
			...
			if(st.getQuestItemsCount(ToneOfWater) == 0)
			{
				if(st.getQuestItemsCount(UndineCharm) == 0)
				{
					...
					st.giveItems(UndineCharm, 1, false, false);
					st.playSound(SOUND_MIDDLE);
				}
				...
				else
				{
					...
					st.giveItems(ToneOfWater, 1, false, false);
