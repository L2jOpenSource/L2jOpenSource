...
public class _281_HeadForTheHills extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(ADENA_ID, st.getQuestItemsCount(HillsOfGoldMonsterClaw) * 50, true, true);
		...
		else if(event.equalsIgnoreCase("soe"))
		{
			if(st.getQuestItemsCount(HillsOfGoldMonsterClaw) >= 50)
			{
				...
				st.giveItems(ScrollOfEscape, 5, false, false);
				tryGiveOneTimeRevard(st);
				htmltext = "zerstorer_morsell_q0281_06.htm";
			}
			else
