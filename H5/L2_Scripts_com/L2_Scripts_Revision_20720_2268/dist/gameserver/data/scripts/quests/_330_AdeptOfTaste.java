...
public class _330_AdeptOfTaste extends QuestScript
{
	// NPCs
	...
	{
		if(event.equalsIgnoreCase("30469_03.htm"))
		{
			if(st.getQuestItemsCount(Ingredient_List) == 0)
				st.giveItems(Ingredient_List, 1, false, false);
			st.setCond(1);
		}
		else if(event.equalsIgnoreCase("30062_05.htm"))
		{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Jonas)
		{
			...
			else
			{
				if(Has_Ingredient_List)
				{
					...
					st.giveItems(Jonass_Steak_Dishes[(int) spec_ingredients_count], 1, false, false);
						...
						for(int i = Miriens_Reviews.length; i > 0; i--)
							if(st.getQuestItemsCount(Miriens_Reviews[i - 1]) > 0)
							{
								...
								if(adena_rewards[i - 1] > 0)
									st.giveItems(ADENA_ID, adena_rewards[i - 1], false, false);
								if(rewards[i - 1] > 0)
									st.giveItems(rewards[i - 1], 1, false, false);
								st.finishQuest();
			...
			if(all_ingredients_count == 0)
			{
				...
				for(int i = Jonass_Steak_Dishes.length; i > 0; i--)
					if(st.getQuestItemsCount(Jonass_Steak_Dishes[i - 1]) > 0)
					{
						...
						st.giveItems(Miriens_Reviews[i - 1], 1, false, false);
		...
		if(npcId == Sonia)
		{
			...
			{
				st.giveItems(Sonias_Botany_Book, 1, false, false);
				return "30062_01.htm";
			}
		}
		...
		if(npcId == Glyvka)
		{
			...
			if(st.getQuestItemsCount(Glyvkas_Botany_Book) > 0)
			{
				...
				return "30067_07.htm";
			{
				st.giveItems(Glyvkas_Botany_Book, 1, false, false);
				return "30067_01.htm";
			}
		}
		...
		if(npcId == Rollant)
		{
			...
			if(st.getQuestItemsCount(Rollants_Creature_Book) > 0)
			{
				if(!has_meat)
				{
					...
					st.playSound(SOUND_MIDDLE);
					st.giveItems(Meat_of_Monster_Eye, 1, false, false);
					return "30069_03.htm";
				}
			}
			else if(has_meat)
			...
			{
				st.giveItems(Rollants_Creature_Book, 1, false, false);
				return "30069_01.htm";
			}
		}
		...
		if(npcId == Jacob)
		{
			...
			{
				st.giveItems(Jacobs_Insect_Book, 1, false, false);
				return "30073_01.htm";
			}
		}
		...
		if(npcId == Pano)
		{
			...
			if(st.getQuestItemsCount(Panos_Contract) > 0)
			{
				if(!has_potato)
				{
					...
					st.playSound(SOUND_MIDDLE);
					st.giveItems(Dionian_Potato, 1, false, false);
					return "30078_03.htm";
				}
			}
			else if(has_potato)
			...
			{
				st.giveItems(Panos_Contract, 1, false, false);
				return "30078_01.htm";
			}
		}
	...
	private static void Root2Sap(QuestState st, int sap_id)
	{
		...
		st.playSound(SOUND_MIDDLE);
		st.giveItems(sap_id, 1, false, false);
	}
	...
	private static void Moss2Bundle(QuestState st, int bundle_id)
	{
		...
		st.playSound(SOUND_MIDDLE);
		st.giveItems(bundle_id, 1, false, false);
	}
	...
	private static void Nectar2Honey(QuestState st, int honey_id)
	{
		...
		st.playSound(SOUND_MIDDLE);
		st.giveItems(honey_id, 1, false, false);
	}
}