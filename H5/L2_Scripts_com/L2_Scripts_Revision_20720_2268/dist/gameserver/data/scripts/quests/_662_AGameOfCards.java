...
public class _662_AGameOfCards extends QuestScript
{
	// NPCs
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30845_03.htm") && st.getQuestItemsCount(RED_GEM) >= 50)
			return "30845_04.htm";
	...
	private static class CardGame
	{
		...
		public CardGame(int _player_id)
		{
			...
			for(int i = 0; i < cards.length; i++)
				cards[i] = "<a action=\"bypass -h npc_%objectId%_QuestEvent 662 card" + i + "\">?</a>";
		}
		...
		private String finish(QuestState st)
		{
			...
			if(cmatches.length == 1)
			{
				if(cmatches[0] == 5)
				{
					...
					st.giveItems(ZIGGOS_GEMSTONE, 43, false, false);
					st.giveItems(Enchant_Weapon_S, 3, false, false);
					st.giveItems(Enchant_Weapon_A, 1, false, false);
				}
				else if(cmatches[0] == 4)
				{
					...
					st.giveItems(Enchant_Weapon_S, 2, false, false);
					st.giveItems(Enchant_Weapon_C, 2, false, false);
				}
				...
				else if(cmatches[0] == 2)
				{
					...
					st.giveItems(Enchant_Armor_D, 2, false, false);
				}
			}
				...
				if(cmatches[0] == 3 || cmatches[1] == 3)
				{
					...
					st.giveItems(Enchant_Weapon_A, 1, false, false);
					st.giveItems(Enchant_Weapon_B, 2, false, false);
					st.giveItems(Enchant_Weapon_D, 1, false, false);
				}
				else
				{
					txt = "Hmmm...? This is... Two pairs? You got lucky this time, but I wonder if it'll last. Here's your prize.";
					st.giveItems(Enchant_Weapon_C, 1, false, false);
				}
			...
			if(st.getQuestItemsCount(RED_GEM) >= 50)
				result += "<br><br><a action=\"bypass -h npc_%objectId%_QuestEvent 662 30845_10.htm\">Play Again!</a>";
			result += html_footer;
			Games.remove(player_id);
			return result;
		}
