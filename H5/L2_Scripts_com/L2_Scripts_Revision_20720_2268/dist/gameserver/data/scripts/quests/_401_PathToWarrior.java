...
public class _401_PathToWarrior extends QuestScript
{
	int AURON = 30010;
	int SIMPLON = 30253;
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("401_3"))
		{
			...
			st.giveItems(RUSTED_BRONZE_SWORD3_ID, 1, false, false);
			st.setCond(5);
		}
		else if(event.equalsIgnoreCase("1"))
		{
			...
			{
				st.setCond(1);
				st.giveItems(EINS_LETTER_ID, 1, false, false);
		...
		else if(event.equalsIgnoreCase("30253_1"))
		{
			...
			st.giveItems(WARRIOR_GUILD_MARK_ID, 1, false, false);
			st.setCond(2);
		}
		return htmltext;
	}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == SIMPLON && st.getQuestItemsCount(WARRIOR_GUILD_MARK_ID) > 0)
		{
			...
			else if(st.getQuestItemsCount(RUSTED_BRONZE_SWORD1_ID) >= 10)
			{
				...
				st.giveItems(RUSTED_BRONZE_SWORD2_ID, 1, false, false);
				st.giveItems(SIMPLONS_LETTER_ID, 1, false, false);
				st.setCond(4);
			...
			else if(st.getQuestItemsCount(POISON_SPIDER_LEG2_ID) > 19)
			{
				...
				{
					st.giveItems(MEDALLION_OF_WARRIOR_ID, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == TRACKER_SKELETON || npcId == TRACKER_SKELETON_LD)
		{
			...
			{
				st.giveItems(RUSTED_BRONZE_SWORD1_ID, 1, true, true);
			...
			{
				st.giveItems(POISON_SPIDER_LEG2_ID, 1, true, true);
				if(st.getQuestItemsCount(POISON_SPIDER_LEG2_ID) >= 20)
				{
					st.setCond(6);
				}
