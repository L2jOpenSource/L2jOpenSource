...
public class _233_TestOfWarspirit extends QuestScript
{
	// NPCs
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		if(event.equalsIgnoreCase("30510-05.htm"))
		{
			...
			{
				//st.giveItems(Dimensional_Diamond, 92, false, false); NOT GIVING?
				st.getPlayer().setVar("dd3", "1", -1);
			}
			st.setCond(1);
		}
		else if(event.equalsIgnoreCase("30630-04.htm"))
			st.giveItems(ORIMS_CONTRACT, 1, false, false);
		else if(event.equalsIgnoreCase("30682-02.htm"))
			st.giveItems(PEKIRONS_TOTEM, 1, false, false);
		else if(event.equalsIgnoreCase("30515-02.htm"))
			st.giveItems(MANAKIAS_TOTEM, 1, false, false);
		else if(event.equalsIgnoreCase("30507-02.htm"))
			st.giveItems(RACOYS_TOTEM, 1, false, false);
		else if(event.equalsIgnoreCase("30030-04.htm"))
			st.giveItems(VIVIANTES_LETTER, 1, false, false);
		else if(event.equalsIgnoreCase("30649-03.htm") && st.getQuestItemsCount(WARSPIRIT_TOTEM) > 0)
		{
			...
			st.giveItems(MARK_OF_WARSPIRIT, 1, false, false);
			if(!st.getPlayer().getVarBoolean("prof2.3"))
			{
				st.addExpAndSp(894888, 61408);
				st.giveItems(ADENA_ID, 161806, true, true); // FIXED
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Somak)
		{
			...
			else
			{
				if(st.getQuestItemsCount(VENDETTA_TOTEM) > 0)
				{
					...
					st.giveItems(WARSPIRIT_TOTEM, 1, false, false);
					st.giveItems(BRAKIS_REMAINS2, 1, false, false);
					st.giveItems(HERMODTS_REMAINS2, 1, false, false);
					st.giveItems(KIRUNAS_REMAINS2, 1, false, false);
					st.giveItems(TONARS_REMAINS2, 1, false, false);
				...
				st.giveItems(VENDETTA_TOTEM, 1, false, false);
				st.playSound(SOUND_MIDDLE);
				return "30510-07.htm";
			}
		}
		...
		if(npcId == Orim)
		{
			if(st.getQuestItemsCount(ORIMS_CONTRACT) > 0)
			{
				...
				st.giveItems(BRAKIS_REMAINS1, 1, false, false);
				st.playSound(SOUND_MIDDLE);
				return "30630-06.htm";
			}
		...
		if(npcId == Pekiron)
		{
			if(st.getQuestItemsCount(PEKIRONS_TOTEM) > 0)
			{
				...
				st.giveItems(TONARS_REMAINS1, 1, false, false);
				st.playSound(SOUND_MIDDLE);
				return "30682-04.htm";
			}
		...
		if(npcId == Manakia)
		{
			if(st.getQuestItemsCount(MANAKIAS_TOTEM) > 0)
			{
				...
				for(int drop_id : Medusa_Drops)
					if(st.getQuestItemsCount(drop_id) == 0)
						st.takeItems(drop_id, -1);
				st.giveItems(HERMODTS_REMAINS1, 1, false, false);
				st.playSound(SOUND_MIDDLE);
				return "30515-04.htm";
			}
			...
			if(st.getQuestItemsCount(RACOYS_TOTEM) > 0)
			{
				...
				if(st.getQuestItemsCount(VIVIANTES_LETTER) == 0)
				{
					...
					for(int drop_id : Noble_Ant_Drops)
						if(st.getQuestItemsCount(drop_id) == 0)
							st.takeItems(drop_id, -1);
					st.giveItems(KIRUNAS_REMAINS1, 1, false, false);
					st.playSound(SOUND_MIDDLE);
					return "30507-06.htm";
				}
			}
		...
		if(npcId == Sarien)
			if(st.getQuestItemsCount(RACOYS_TOTEM) > 0)
			{
				if(st.getQuestItemsCount(INSECT_DIAGRAM_BOOK) == 0 && st.getQuestItemsCount(VIVIANTES_LETTER) > 0)
				{
					...
					st.giveItems(INSECT_DIAGRAM_BOOK, 1, false, false);
					st.playSound(SOUND_MIDDLE);
					return "30436-01.htm";
				}
	...
	public String onKill(NpcInstance npc, QuestState qs)
	{
		...
		{
			qs.giveItems(PORTAS_EYE, 1, true, true);
		...
		{
			qs.giveItems(EXCUROS_SCALE, 1, true, true);
		...
		{
			qs.giveItems(MORDEOS_TALON, 1, true, true);
		...
		else if((npcId == Noble_Ant || npcId == Noble_Ant_Leader) && qs.getQuestItemsCount(RACOYS_TOTEM) > 0)
		{
			...
			if(drops.size() > 0 && Rnd.chance(30))
			{
				...
				qs.giveItems(drop_id, 1, false, false);
		...
		{
			qs.giveItems(HERMODTS_SKULL, 1, false, false);
			...
			{
				qs.giveItems(TAMLIN_ORC_HEAD, 1, true, true);
