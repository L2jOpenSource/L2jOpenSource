...
public class _404_PathToWizard extends QuestScript
{
	//npc
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
		{
			...
			if(st.getQuestItemsCount(WIND_FEATHER_ID) < 1)
			{
				...
				st.giveItems(WIND_FEATHER_ID, 1, false, false);
				st.setCond(6);
			}
		return htmltext;
	}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == PARINA)
		{
			...
			else if(st.getQuestItemsCount(FLAME_EARING_ID) < 1 | st.getQuestItemsCount(WIND_BANGEL_ID) < 1 | st.getQuestItemsCount(WATER_NECKLACE_ID) < 1 | st.getQuestItemsCount(EARTH_RING_ID) < 1)
				htmltext = "parina_q0404_05.htm";
			else if(st.getQuestItemsCount(FLAME_EARING_ID) > 0 && st.getQuestItemsCount(WIND_BANGEL_ID) > 0 && st.getQuestItemsCount(WATER_NECKLACE_ID) > 0 && st.getQuestItemsCount(EARTH_RING_ID) > 0)
			{
				...
				if(st.getPlayer().getClassId().isOfLevel(ClassLevel.NONE))
				{
					if(st.getQuestItemsCount(BEAD_OF_SEASON_ID) < 1)
						st.giveItems(BEAD_OF_SEASON_ID, 1, false, false);
		...
		else if(npcId == FLAME_SALAMANDER)
		{
			...
			{
				st.giveItems(MAP_OF_LUSTER_ID, 1, false, false);
			...
			else if(cond == 3 && st.getQuestItemsCount(MAP_OF_LUSTER_ID) > 0 && st.getQuestItemsCount(KEY_OF_FLAME_ID) > 0)
			{
				...
				if(st.getQuestItemsCount(FLAME_EARING_ID) < 1)
					st.giveItems(FLAME_EARING_ID, 1, false, false);
		...
		else if(npcId == WIND_SYLPH)
		{
			...
			{
				st.giveItems(BROKEN_BRONZE_MIRROR_ID, 1, false, false);
			...
			else if(cond > 0 && st.getQuestItemsCount(BROKEN_BRONZE_MIRROR_ID) > 0 && st.getQuestItemsCount(WIND_FEATHER_ID) > 0)
			{
				...
				if(st.getQuestItemsCount(WIND_BANGEL_ID) < 1)
					st.giveItems(WIND_BANGEL_ID, 1, false, false);
		...
		else if(npcId == WATER_UNDINE)
		{
			...
			{
				st.giveItems(RAMAS_DIARY_ID, 1, false, false);
			...
			else if(cond == 9 && st.getQuestItemsCount(RAMAS_DIARY_ID) > 0 && st.getQuestItemsCount(SPARKLE_PEBBLE_ID) > 1)
			{
				...
				if(st.getQuestItemsCount(WATER_NECKLACE_ID) < 1)
					st.giveItems(WATER_NECKLACE_ID, 1, false, false);
			...
			{
				st.giveItems(RUST_GOLD_COIN_ID, 1, false, false);
			...
			else if(cond == 12 && st.getQuestItemsCount(RUST_GOLD_COIN_ID) > 0 && st.getQuestItemsCount(RED_SOIL_ID) > 0)
			{
				...
				if(st.getQuestItemsCount(EARTH_RING_ID) < 1)
					st.giveItems(EARTH_RING_ID, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == RATMAN_WARRIOR)
		{
			if(cond == 2)
			{
				st.giveItems(KEY_OF_FLAME_ID, 1, false, false);
				st.setCond(3);
			}
		}
		else if(npcId == WATER_SEER)
		{
			...
			{
				st.giveItems(SPARKLE_PEBBLE_ID, 1, true, true);
			...
			if(cond == 11)
			{
				st.giveItems(RED_SOIL_ID, 1, false, false);
				st.setCond(12);
			}
		return null;
	}
