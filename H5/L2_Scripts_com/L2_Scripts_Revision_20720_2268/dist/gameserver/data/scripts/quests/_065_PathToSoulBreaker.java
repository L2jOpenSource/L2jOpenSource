...
public class _065_PathToSoulBreaker extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("32213-02.htm"))
		{
			...
			{
				st.giveItems(DD, 47, false, false);
		...
		if(event.equalsIgnoreCase("32332-01.htm"))
		{
			...
			st.giveItems(Sealed_Doc, 1, false, false);
			st.setCond(13);
			st.unset("id");
			Despawn_Katenar();
		}
		...
		if(event.equalsIgnoreCase("32138-09.htm"))
		{
			...
			st.giveItems(Kekropus_Rec, 1, false, false);
			st.setCond(17);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Vitus)
		{
			...
			else if(st.getQuestItemsCount(SB_Certificate) > 0)
			{
			...
			else if(cond == 17)
			{
				...
				if(!st.getPlayer().getVarBoolean("prof2.1"))
				{
					...
					st.giveItems(ADENA_ID, 71194, true, true);
					st.getPlayer().setVar("prof2.1", "1", -1);
				}
				st.giveItems(SB_Certificate, 1, false, false);
				st.finishQuest();
			}
		}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			st.giveItems(Wyrm_Heart, 1, true, true);
