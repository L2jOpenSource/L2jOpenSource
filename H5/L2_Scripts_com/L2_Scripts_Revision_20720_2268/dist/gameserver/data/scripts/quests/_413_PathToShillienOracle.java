...
public class _413_PathToShillienOracle extends QuestScript
{
	//npc
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
		{
			...
			st.setCond(1);
			st.giveItems(SIDRAS_LETTER1_ID, 1, false, false);
		}
		...
		else if(event.equalsIgnoreCase("30377_1"))
		{
			...
			st.giveItems(BLANK_SHEET1_ID, 5, false, false);
		...
		else if(event.equalsIgnoreCase("30375_3"))
		{
			...
			st.giveItems(PENITENTS_MARK_ID, 1, false, false);
			st.playSound(SOUND_ITEMGET);
			st.setCond(5);
		}
		return htmltext;
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == SIDRA)
		{
			...
			else if(cond == 7 && st.getQuestItemsCount(ANDARIEL_BOOK_ID) > 0 && st.getQuestItemsCount(GARMIEL_BOOK_ID) > 0)
			{
				...
				{
					st.giveItems(ORB_OF_ABYSS_ID, 1, false, false);
		...
		else if(npcId == TALBOT)
		{
			...
			else if(cond == 3 && st.getQuestItemsCount(BLOODY_RUNE1_ID) > 4)
			{
				...
				st.giveItems(GARMIEL_BOOK_ID, 1, false, false);
				st.giveItems(PRAYER_OF_ADON_ID, 1, false, false);
				st.playSound(SOUND_ITEMGET);
				st.setCond(4);
			}
			else if(cond > 3 && cond < 7)
			...
			else if(cond == 6 && st.getQuestItemsCount(ASHEN_BONES_ID) > 9)
			{
				...
				st.giveItems(ANDARIEL_BOOK_ID, 1, false, false);
				st.playSound(SOUND_ITEMGET);
				st.setCond(7);
			}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == DARK_SUCCUBUS)
			...
			{
				st.giveItems(BLOODY_RUNE1_ID, 1, false, false);
		...
		for(int i : ASHEN_BONES_DROP)
			...
			{
				st.giveItems(ASHEN_BONES_ID, 1, true, true);
