...
public class _292_BrigandsSweep extends QuestScript
{
	// NPCs
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Spiron)
		{
			...
			{
				st.takeItems(SuspiciousContract, -1);
				st.giveItems(ADENA_ID, 120, true, true);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(st.getQuestItemsCount(SuspiciousContract) == 0 && Rnd.chance(Chance))
			...
			{
				st.giveItems(SuspiciousMemo, 1, false, false);
				st.playSound(SOUND_ITEMGET);
			}
			else
			{
				st.takeItems(SuspiciousMemo, -1);
				st.giveItems(SuspiciousContract, 1, false, false);
