...
public class _355_FamilyHonor extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equals("galicbredo_q0355_07.htm"))
		{
			...
			st.giveItems(WORK_OF_BERONA, count, false, false);
		}
		...
		else if(event.equals("appraise"))
		{
			...
			else if(appraising < 40)
			{
				...
				st.takeItems(WORK_OF_BERONA, 1);
				st.giveItems(STATUE_REPLICA, 1, false, false);
			}
			else if(appraising < 60)
			{
				htmltext = "patrin_q0355_04.htm";
				st.takeItems(WORK_OF_BERONA, 1);
				st.giveItems(STATUE_ORIGINAL, 1, false, false);
			}
			else if(appraising < 80)
			{
				htmltext = "galicbredo_q0355_10.htm";
				st.takeItems(WORK_OF_BERONA, 1);
				st.giveItems(STATUE_FORGERY, 1, false, false);
			}
			else if(appraising < 100)
			{
				htmltext = "galicbredo_q0355_11.htm";
				st.takeItems(WORK_OF_BERONA, 1);
				st.giveItems(STATUE_PROTOTYPE, 1, false, false);
			}
		}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		if(st.getCond() == 1)
		{
			...
			{
				st.giveItems(GALFREDOS_BUST, 1, true, true);
				st.playSound(SOUND_ITEMGET);
			}
			if(Rnd.chance(CHANCE_FOR_GODDESS_BUST))
				st.giveItems(BUST_OF_ANCIENT_GODDESS, 1, true, true);
		}
		return null;
	}
}