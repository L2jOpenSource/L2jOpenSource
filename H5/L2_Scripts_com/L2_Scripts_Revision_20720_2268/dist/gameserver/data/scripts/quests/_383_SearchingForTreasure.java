...
public class _383_SearchingForTreasure extends QuestScript
{
	// Items
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30890-02b.htm"))
		{
			...
			{
				st.takeItems(1661, 1);
				st.giveItems(ADENA_ID, 500 + Rnd.get(5) * 300, true, true);
				int count = 0;
				while(count < 1)
					for(rewardInfo reward : rewards)
					{
						...
						{
							st.giveItems(id, Rnd.get(qty) + 1, false, false);
							count += 1;
						}
						if(count < 2)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == PIRATES_CHEST && st.getCond() == 2)
