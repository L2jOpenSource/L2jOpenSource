...
public class _335_TheSongOfTheHunter extends QuestScript
{
	// NPCs
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		if(event.equalsIgnoreCase("30744_03.htm"))
		{
			if(st.getQuestItemsCount(_1st_Test_Instructions) == 0)
				st.giveItems(_1st_Test_Instructions, 1, false, false);
			st.setCond(1);
		}
		else if(event.equalsIgnoreCase("30744_09.htm"))
		{
			...
			if(st.getQuestItemsCount(_2nd_Test_Instructions) == 0)
			{
				...
				st.giveItems(_2nd_Test_Instructions, 1, false, false);
			}
		}
		else if(event.equalsIgnoreCase("30744_16.htm"))
		{
			...
			{
				st.giveItems(ADENA_ID, 20000, true, true);
				event = "30744_17.htm";
			}
			st.finishQuest();
		}
		else if(event.equalsIgnoreCase("30746_03.htm"))
		{
			...
			if(st.getQuestItemsCount(Cybellins_Dagger) == 0)
				st.giveItems(Cybellins_Dagger, 1, false, false);
			if(st.getQuestItemsCount(Cybellins_Request) == 0)
				st.giveItems(Cybellins_Request, 1, false, false);
			st.takeAllItems(q_blood_crystal);
			st.playSound(SOUND_MIDDLE);
			st.giveItems(q_blood_crystal[1], 1, false, false);
		}
		...
		else if(event.startsWith("30745-request-"))
		{
			...
			if(!isValidRequest(request_id))
				return null;
			st.giveItems(request_id, 1, false, false);
		...
		if(npcId == Grey)
		{
			...
			else
			{
				if(st.getQuestItemsCount(_1st_Test_Instructions) > 0)
				{
					...
					st.giveItems(_1st_Circle_Hunter_License, 1, false, false);
					st.setCond(2);
					return "30744_06.htm";
				}
				...
				if(st.getQuestItemsCount(_2nd_Test_Instructions) > 0)
				{
					...
					st.giveItems(_2nd_Circle_Hunter_License, 1, false, false);
					st.setCond(3);
					return "30744_12.htm";
				}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(st.getQuestItemsCount(_1st_Circle_Hunter_License) > 0 || st.getQuestItemsCount(_2nd_Circle_Hunter_License) > 0)
		{
			if(st.getQuestItemsCount(Cybellins_Request) > 0 && st.getPlayer().getActiveWeaponTemplate() != null && st.getPlayer().getActiveWeaponTemplate().getItemId() == Cybellins_Dagger)
			{
				int Blood_Crystal_Level = Get_Blood_Crystal_Level(st);
					...
					for(int lizardmen_id : q_blood_crystal_lizardmen)
						if(npcId == lizardmen_id)
							if(Rnd.chance(50))
							{
								...
								st.giveItems(q_blood_crystal[Blood_Crystal_Level + 1], 1, false, false);
							}
							else
							{
								st.takeAllItems(q_blood_crystal);
								st.giveItems(q_blood_crystal[0], 1, false, false);
							}
			}
	...
	private static boolean Blood_Crystal2Adena(QuestState st, int Blood_Crystal_Level)
	{
		if(Blood_Crystal_Level < 2)
			return false;
		st.takeAllItems(q_blood_crystal);
		st.giveItems(ADENA_ID, 3400 * (int) Math.pow(2, Blood_Crystal_Level - 2), true, true);
		return true;
	}
	...
	private static String FormatList(QuestState st, Request[] requests)
	{
		...
		for(int i = 0; i <= 5; i++)
		{
			String s = "<a action=\"bypass -h npc_%objectId%_QuestEvent 335 30745-request-" + requests[listpacked[i]].request_id + "\">" + requests[listpacked[i]].text + "</a>";
	...
	public static class Request
	{
		...
		public boolean Complete(QuestState st)
		{
			...
			st.playSound(SOUND_MIDDLE);
			st.giveItems(Laurel_Leaf_Pin, 1, false, false);
			st.giveItems(ADENA_ID, reward_adena, true, true);
			st.unset("list");
			return true;
		}
	}
