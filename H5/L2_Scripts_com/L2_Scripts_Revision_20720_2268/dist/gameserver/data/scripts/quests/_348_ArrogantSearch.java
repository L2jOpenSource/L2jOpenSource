...
public class _348_ArrogantSearch extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(29);
			st.giveItems(WHITE_FABRIC_2, 10, false, false);
		}
		if(event.equals("30864-10a.htm"))
		{
			if(st.getQuestItemsCount(WHITE_FABRIC_2) < 10)
				st.giveItems(WHITE_FABRIC_2, 10 - st.getQuestItemsCount(WHITE_FABRIC_2), false, false);
		...
		if(event.equals("30864-10b.htm"))
		{
			if(st.getQuestItemsCount(BLOODED_FABRIC) > 1)
			{
				...
				st.giveItems(ADENA_ID, count * 5000, true, true); // с потолка
				htmltext = "30864-10.htm";
			}
			else
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == HANELLIN)
		{
			...
			{
				st.setCond(5);
				st.giveItems(HANELLINS_FIRST_LETTER, 1, false, false);
				st.giveItems(HANELLINS_SECOND_LETTER, 1, false, false);
				st.giveItems(HANELLINS_THIRD_LETTER, 1, false, false);
			...
			else if(cond == 22)
			{
				...
				if(st.getInt("companions") == 0)
				{
					...
					st.giveItems(WHITE_FABRIC_1, 1, false, false);
				}
				else
				{
					st.setCond(23);
					htmltext = "not implemented yet";
					st.giveItems(WHITE_FABRIC_0, 3, false, false);
				}
			}
			...
			else if(cond == 26 && st.getQuestItemsCount(BLOODED_FABRIC) < 1)
			{
				if(st.getQuestItemsCount(WHITE_FABRIC_2) < 1)
					st.giveItems(WHITE_FABRIC_2, 1, false, false);
				htmltext = "30864-07a.htm";
			}
		...
		// Other NPCs follow:
			...
			if(ARK_OWNERS.containsKey(npcId))
			{
				...
				if(st.getQuestItemsCount(ARK_OWNERS.get(npcId)[0]) == 1)
				{
					...
					st.getPlayer().sendPacket(new RadarControlPacket(0, 1, ARK_OWNERS.get(npcId)[2], ARK_OWNERS.get(npcId)[3], ARK_OWNERS.get(npcId)[4]));
				}
			...
			else if(ARKS.containsKey(npcId))
			{
				...
				// the player has the key and doesn't have the contents, give the contents
				{
					...
					st.giveItems(ARKS.get(npcId)[2], 1, false, false);
				}
			}
				...
				{
					st.giveItems(FIRST_KEY_OF_ARK, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(drop != null)
		{
			...
			if(st.getCond() >= cond && st.getQuestItemsCount(item) < max && (take == 0 || st.getQuestItemsCount(take) > 0) && Rnd.chance(chance))
			{
				...
				{
					QuestState FatesWhisper = st.getPlayer().getQuestState(234);
