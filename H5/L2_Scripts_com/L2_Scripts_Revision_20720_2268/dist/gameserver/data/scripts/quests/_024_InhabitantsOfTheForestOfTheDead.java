...
public class _024_InhabitantsOfTheForestOfTheDead extends QuestScript
{
	// Список NPC
	...
	public String onTalk(NpcInstance npc, QuestState qs)
	{
		...
		if(npcId == DORIAN)
		{
			if(cond == 0)
			{
				if(qs.getPlayer().isQuestCompleted(23))
			...
			else if(cond == 7 && qs.getQuestItemsCount(LIDIA_HAIR_PIN) == 0)
			{
				...
				qs.giveItems(LIDIA_HAIR_PIN, 1, false, false);
				qs.setCond(8);
			}
		}
	...
	public String onEvent(String event, QuestState qs, NpcInstance npc)
	{
		if(event.startsWith("seePlayer"))
		{
			...
			{
				qs.giveItems(BROKEN_SILVER_CROSS_OF_EINHASAD, 1, false, false);
		...
		{
			qs.giveItems(FLOWER_BOUQUET, 1, false, false);
			qs.setCond(1);
		}
		...
		{
			qs.giveItems(SILVER_CROSS_OF_EINHASAD, 1, false, false);
			qs.setCond(3);
		}
		...
		{
			qs.giveItems(SUSPICIOUS_TOTEM_DOLL1, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState qs)
	{
		...
		if(cond == 9)
		{
			if(ArrayUtils.contains(MOBS, npcId) && Rnd.chance(70))
			{
				qs.giveItems(SUSPICIOUS_TOTEM_DOLL, 1, false, false);
				qs.setCond(10);
			}
		}
		return null;
	...
	public void onAbort(QuestState st)
	{
		if(st.getQuestItemsCount(LIDIA_HAIR_PIN) == 0)
			st.giveItems(LIDIA_HAIR_PIN, 1, false, false);
	}
}
