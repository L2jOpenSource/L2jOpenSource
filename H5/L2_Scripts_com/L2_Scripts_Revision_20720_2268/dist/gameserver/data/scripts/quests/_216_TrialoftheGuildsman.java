...
public class _216_TrialoftheGuildsman extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(1);
			st.giveItems(VALKONS_RECOMMEND, 1, false, false);
			st.takeItems(ADENA_ID, 2000);
			if(!st.getPlayer().getVarBoolean("dd1"))
			{
				st.giveItems(DIMENSION_DIAMOND, 85, false, false);
		...
		else if(event.equalsIgnoreCase("30103_3") || event.equalsIgnoreCase("30103_4"))
		{
			...
			st.giveItems(MARK_OF_GUILDSMAN, 1, false, false);
			if(!st.getPlayer().getVarBoolean("prof2.1"))
			{
				st.addExpAndSp(1029478, 66768);
				st.giveItems(57, 187606, true, true);
		...
		else if(event.equalsIgnoreCase("blacksmith_alltran_q0216_03.htm"))
		{
			...
			st.giveItems(ALLTRANS_INSTRUCTIONS, 1, false, false);
			st.giveItems(RP_JOURNEYMAN_RING, 1, false, false);
			st.giveItems(ALLTRANS_RECOMMEND1, 1, false, false);
			st.giveItems(ALLTRANS_RECOMMEND2, 1, false, false);
			st.setCond(5);
		}
		else if(event.equalsIgnoreCase("warehouse_keeper_norman_q0216_04.htm"))
		{
			st.takeItems(ALLTRANS_RECOMMEND1, -1);
			st.giveItems(NORMANS_INSTRUCTIONS, 1, false, false);
			st.giveItems(NORMANS_RECEIPT, 1, false, false);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == VALKON)
		{
			...
			else if(st.getQuestItemsCount(MARK_OF_GUILDSMAN) > 0)
			{
		...
		else if(npcId == NORMAN && cond >= 5)
		{
			...
			else if(st.getQuestItemsCount(ALLTRANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(NORMANS_LIST) > 0)
			{
				if(st.getQuestItemsCount(GRAY_BONE_POWDER) >= 70 && st.getQuestItemsCount(GRANITE_WHETSTONE) >= 70 && st.getQuestItemsCount(RED_PIGMENT) >= 70 && st.getQuestItemsCount(BRAIDED_YARN) >= 70)
				{
					...
					st.giveItems(JOURNEYMAN_GEM, 7, false, false);
		...
		else if(npcId == DUNING && cond >= 5)
		{
			...
			if(st.getQuestItemsCount(ALLTRANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(ALLTRANS_RECOMMEND2) > 0)
				...
				else
				{
					...
					st.giveItems(JOURNEYMAN_DECO_BEADS, 7, false, false);
					if(st.getQuestItemsCount(JOURNEYMAN_DECO_BEADS) == 7 && st.getQuestItemsCount(JOURNEYMAN_GEM) >= 7)
						st.setCond(6);
				}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 5 && (npcId == 20079 || npcId == 20080 || npcId == 20081))
			if(Rnd.chance(33) && st.getQuestItemsCount(ALLTRANS_INSTRUCTIONS) > 0 && st.getQuestItemsCount(PINTERS_INSTRUCTIONS) > 0)
			{
				...
				{
					st.giveItems(AMBER_BEAD, 5, true, true);
					if(st.getQuestItemsCount(AMBER_BEAD) >= 70)
				...
				{
					st.giveItems(AMBER_LUMP, 5, true, true);
