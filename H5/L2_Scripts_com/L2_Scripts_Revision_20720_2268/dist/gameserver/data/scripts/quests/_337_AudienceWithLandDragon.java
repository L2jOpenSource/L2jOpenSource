...
public class _337_AudienceWithLandDragon extends QuestScript
{
	//npc
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
		{
			...
			st.setCond(1);
			st.set("guard", "0");
			st.giveItems(FEATHER_OF_GABRIELLE_ID, 1, false, false);
		...
		else if(event.equalsIgnoreCase("15"))
		{
			...
			st.giveItems(HERALD_OF_SLAYER_ID, 1, false, false);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == KAIENA && cond == 1 && step < 4)
		{
			...
			else if(st.getQuestItemsCount(STALKER_HORN_ID) > 0 && st.getQuestItemsCount(DRAKE_TALON_ID) > 0)
			{
				...
				st.giveItems(MARK_OF_WATCHMAN_ID, 1, false, false);
				st.set("step", "3");
		...
		else if(npcId == GILMORE && step < 17)
		{
			...
			else
			{
				...
				st.giveItems(PORTAL_STONE_ID, 1, false, false);
				st.finishQuest();
			}
		return htmltext;
	}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		for(int[] element : DROPLIST)
			...
			{
				st.giveItems(element[2], element[5], true, true);
