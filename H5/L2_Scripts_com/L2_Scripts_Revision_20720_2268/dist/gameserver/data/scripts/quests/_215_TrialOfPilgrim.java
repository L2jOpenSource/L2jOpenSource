...
public class _215_TrialOfPilgrim extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equals("1"))
		{
			...
			st.setCond(1);
			st.giveItems(VOUCHER_OF_TRIAL_ID, 1, false, false);
		}
		if(!st.getPlayer().getVarBoolean("dd1"))
		{
			st.giveItems(7562, 49, false, false);
		...
		else if(event.equals("30649_1"))
		{
			...
			st.giveItems(SPIRIT_OF_FLAME_ID, 1, false, false);
		...
		else if(event.equals("30650_1"))
		{
			if(st.getQuestItemsCount(ADENA_ID) >= 100000)
			{
				...
				st.giveItems(BOOK_OF_GERALD_ID, 1, false, false);
		...
		else if(event.equals("30652_1"))
		{
			...
			st.giveItems(BOOK_OF_DARKNESS_ID, 1, false, false);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == 30648 && st.getCond() == 17 && st.getQuestItemsCount(BOOK_OF_SAGE_ID) > 0)
		{
			...
			st.giveItems(MARK_OF_PILGRIM_ID, 1, false, false);
		...
		else if(npcId == 30550 && st.getCond() == 5 && st.getQuestItemsCount(SPIRIT_OF_FLAME_ID) > 0)
		{
			...
			st.giveItems(TAG_OF_RUMOR_ID, 1, false, false);
			st.setCond(6);
		}
		...
		else if(npcId == 30651 && st.getCond() == 6 && st.getQuestItemsCount(TAG_OF_RUMOR_ID) > 0)
		{
			...
			st.giveItems(GREY_BADGE_ID, 1, false, false);
		...
		else if(npcId == 30036 && st.getCond() == 9)
		{
			...
			st.giveItems(PICTURE_OF_NAHIR_ID, 1, false, false);
			st.setCond(10);
		}
		...
		else if(npcId == 30036 && st.getCond() == 11)
		{
			...
			st.giveItems(STATUE_OF_EINHASAD_ID, 1, false, false);
		...
		else if(npcId == 30612 && st.getCond() == 16)
		{
			...
			st.giveItems(BOOK_OF_SAGE_ID, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 27116)
		{
			if(st.getCond() == 3 && st.getQuestItemsCount(ESSENSE_OF_FLAME_ID) == 0)
				if(Rnd.chance(30))
				{
					st.giveItems(ESSENSE_OF_FLAME_ID, 1, false, false);
					st.setCond(4);
				}
		}
		else if(npcId == 27117)
		{
			...
			{
				st.giveItems(HAIR_OF_NAHIR_ID, 1, false, false);
				st.setCond(11);
			}
		}
		else if(npcId == 27118)
			if(st.getCond() == 13 && st.getQuestItemsCount(DEBRIS_OF_WILLOW_ID) == 0)
				if(Rnd.chance(20))
				{
					st.giveItems(DEBRIS_OF_WILLOW_ID, 1, false, false);
					st.setCond(14);
				}
		return null;
	}
