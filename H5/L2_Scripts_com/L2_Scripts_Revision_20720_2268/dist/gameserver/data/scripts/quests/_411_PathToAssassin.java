...
public class _411_PathToAssassin extends QuestScript
{
	//npc
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
		{
			...
			{
				st.setCond(1);
				st.giveItems(SHILENS_CALL_ID, 1, false, false);
		...
		else if(event.equalsIgnoreCase("30419_1"))
		{
			...
			st.giveItems(ARKENIAS_LETTER_ID, 1, false, false);
			st.setCond(2);
		}
		else if(event.equalsIgnoreCase("30382_1"))
		{
			...
			st.giveItems(LEIKANS_NOTE_ID, 1, false, false);
			st.setCond(3);
		}
		return htmltext;
	}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == TRISKEL)
		{
			...
			else if(cond == 7)
			{
				...
				{
					st.giveItems(IRON_HEART_ID, 1, false, false);
		...
		else if(npcId == ARKENIA)
		{
			...
			else if(cond == 6 && st.getQuestItemsCount(SHILENS_TEARS_ID) > 0)
			{
				...
				st.giveItems(ARKENIA_RECOMMEND_ID, 1, false, false);
				st.setCond(7);
			}
			else if(cond == 7)
			...
			else if(cond == 4 && st.getQuestItemsCount(ONYX_BEASTS_MOLAR_ID) > 9)
			{
				...
				st.giveItems(LEIKANS_KNIFE_ID, 1, false, false);
				st.setCond(5);
			}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == CALPICO)
		{
			...
			{
				st.giveItems(SHILENS_TEARS_ID, 1, false, false);
				st.setCond(6);
			}
		}
			...
			{
				st.giveItems(ONYX_BEASTS_MOLAR_ID, 1, true, true);
