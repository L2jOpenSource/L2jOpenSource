...
public class _415_PathToOrcMonk extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(Pomegranate, 1, false, false);
			st.setCond(1);
		}
		return htmltext;
	}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Urutu)
		{
			...
			else if(cond == 8)
			{
				...
				st.giveItems(GantakisLetterOfRecommendation, 1, false, false);
		...
		else if(npcId == Rosheek)
		{
			if(cond == 1)
			{
				...
				st.giveItems(LeatherPouch1st, 1, false, false);
			...
			else if(cond == 3)
			{
				...
				st.giveItems(LeatherPouch2st, 1, false, false);
				st.setCond(4);
			}
			else if(cond == 4)
			...
			else if(cond == 5)
			{
				...
				st.giveItems(LeatherPouch3st, 1, false, false);
			...
			else if(cond == 7)
			{
				...
				st.giveItems(FierySpiritScroll, 1, false, false);
				st.giveItems(RosheeksLetter, 1, false, false);
		...
		else if(npcId == Kasman)
		{
			if(cond == 9)
			{
				...
				st.giveItems(Fig, 1, false, false);
			...
			else if(cond == 13)
			{
				...
				{
					st.giveItems(KhavatariTotem, 1, false, false);
			...
			{
				st.takeItems(Fig, -1);
				st.giveItems(LeatherPouch4st, 1, false, false);
			...
			else if(cond == 12)
			{
				...
				st.giveItems(IronWillScroll, 1, false, false);
				st.giveItems(TorukusLetter, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 3 && st.getQuestItemsCount(LeatherPouchFull1st) == 0)
		{
			...
			st.giveItems(LeatherPouchFull1st, 1, false, false);
		}
		else if(cond == 5 && st.getQuestItemsCount(LeatherPouchFull2st) == 0)
		{
			...
			st.giveItems(LeatherPouchFull2st, 1, false, false);
		}
		else if(cond == 7 && st.getQuestItemsCount(LeatherPouchFull3st) == 0)
		{
			...
			st.giveItems(LeatherPouchFull3st, 1, false, false);
		}
		else if(cond == 11 && st.getQuestItemsCount(RatmanFang) >= 3 && st.getQuestItemsCount(LangkLizardmanTooth) >= 3 && st.getQuestItemsCount(FelimLizardmanTooth) >= 3 && st.getQuestItemsCount(VukuOrcTusk) >= 3)
		{
			...
			st.giveItems(LeatherPouchFull4st, 1, false, false);
			st.setCond(12);
		}
		return null;
	}
