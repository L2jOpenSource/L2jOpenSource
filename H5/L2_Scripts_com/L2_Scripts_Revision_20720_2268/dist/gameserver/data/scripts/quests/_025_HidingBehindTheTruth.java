...
public class _025_HidingBehindTheTruth extends QuestScript
{
	// Список NPC
	...
	public String onEvent(String event, QuestState qs, NpcInstance npc)
	{
		...
		{
			if(!qs.getPlayer().isQuestCompleted(24))
			{
				qs.setCond(1);
				return "31349-02.htm";
			}
		...
		else if(event.equalsIgnoreCase("31348-08.htm"))
		{
			if(qs.getCond() == 4)
			{
				...
				if(qs.getQuestItemsCount(GEMSTONE_KEY) == 0)
					qs.giveItems(GEMSTONE_KEY, 1, false, false);
			}
		...
		else if(event.equalsIgnoreCase("31522-04.htm"))
		{
			...
			if(qs.getQuestItemsCount(MAP_FOREST_OF_DEADMAN) == 0)
				qs.giveItems(MAP_FOREST_OF_DEADMAN, 1, false, false);
		}
		...
		{
			qs.set("id", "8");
			qs.giveItems(CONTRACT, 1, false, false);
		}
		...
		else if(event.equalsIgnoreCase("End1"))
		{
			if(qs.getCond() != 17)
				return "31532-24.htm";
			qs.giveItems(RING_OF_BLESSING, 2, false, false);
			qs.giveItems(EARRING_OF_BLESSING, 1, false, false);
		...
		else if(event.equalsIgnoreCase("End2"))
		{
			...
			qs.giveItems(NECKLACE_OF_BLESSING, 1, false, false);
			qs.giveItems(EARRING_OF_BLESSING, 1, false, false);
			qs.addExpAndSp(572277, 53750);
			qs.finishQuest();
			return "31522-16.htm";
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		switch(npcId)
		{
			...
			case MYSTERIOUS_WIZARD:
				...
				{
					st.setCond(3);
					st.giveItems(SUSPICIOUS_TOTEM_DOLL_2, 1, false, false);
					return "31522-01.htm";
				}
				if(cond == 3)
					return "31522-02.htm";
			...
			case COFFIN:
				...
				{
					st.setCond(13);
					st.giveItems(LIDIAS_DRESS, 1, false, false);
					return "31536-01.htm";
				}
				if(cond == 13)
					return "31531-03.htm";
	...
	public String onKill(NpcInstance npc, QuestState qs)
	{
		...
		{
			qs.giveItems(SUSPICIOUS_TOTEM_DOLL_3, 1, false, false);
			qs.setCond(8);
		}
