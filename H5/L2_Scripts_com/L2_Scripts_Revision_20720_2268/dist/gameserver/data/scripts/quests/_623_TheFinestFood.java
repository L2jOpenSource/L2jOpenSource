...
public class _623_TheFinestFood extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("623_3"))
		{
			...
			{
				case 1:
					st.giveItems(57, 25000, true, true);
					st.giveItems(6849,1, false, false);
					break;
				case 2:
					st.giveItems(57, 65000, true, true);
					st.giveItems(6847, 1, false, false);
					break;
				case 3:
					st.giveItems(57, 25000, true, true);
					st.giveItems(6851, 1, false, false);
					break;
				case 4:
					st.giveItems(ADENA_ID, 73000, true, true);
					st.addExpAndSp(230000, 1820);	
					break;	
			}
			st.finishQuest();
		}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 1) // Like off C4 PTS AI (убрали  && Rnd.chance(50))
			if(npcId == HOT_SPRINGS_BUFFALO)
			{
				...
				{
					st.giveItems(BUFFALO_MEAT, 1, true, true);
					if(st.getQuestItemsCount(BUFFALO_MEAT) >= 100)
					{
						if(summ(st) >= 300)
							st.setCond(2);
			...
			else if(npcId == HOT_SPRINGS_FLAVA)
			{
				...
				{
					st.giveItems(LEAF_OF_FLAVA, 1, true, true);
					if(st.getQuestItemsCount(LEAF_OF_FLAVA) >= 100)
					{
						if(summ(st) >= 300)
							st.setCond(2);
				...
				{
					st.giveItems(ANTELOPE_HORN, 1, true, true);
					if(st.getQuestItemsCount(ANTELOPE_HORN) >= 100)
					{
						if(summ(st) >= 300)
							st.setCond(2);
