...
public class _343_UndertheShadowoftheIvoryTower extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30834-08.htm"))
		{
			if(orbs > 0)
			{
				st.giveItems(ADENA_ID, orbs * 120, true, true);
				st.takeItems(ORB, -1);
			}
			else
		...
		else if(event.equalsIgnoreCase("30934-04.htm"))
		{
			if(st.getInt("playing") > 0)
			{
				if(random1 == 0)
				{
					...
					st.giveItems(ORB, 10, false, false);
				}
				else if(random1 == 1)
				...
				else
				{
					...
					st.giveItems(ORB, 20, false, false);
				}
				st.unset("playing");
			}
			else
		...
		else if(event.equalsIgnoreCase("30935-05.htm"))
		{
			if(orbs >= 10)
			{
				if(random2 == 0)
				{
					...
					{
						st.unset("toss");
						st.giveItems(ORB, 150, false, false);
		...
		else if(event.equalsIgnoreCase("30935-06.htm"))
		{
			if(orbs >= 10)
			{
				...
				if(toss == 1)
					...
					st.giveItems(ORB, 30, false, false);
				else if(toss == 3)
					st.giveItems(ORB, 70, false, false);
				else if(toss == 4)
					st.giveItems(ORB, 150, false, false);
			}
			else
			...
			if(st.getQuestItemsCount(ECTOPLASM) > 0)
			{
				...
				if(random <= 119)
					st.giveItems(955, 1, false, false);
				else if(random <= 169)
					st.giveItems(951, 1, false, false);
				else if(random <= 329)
					st.giveItems(2511, Rnd.get(200) + 401, false, false);
				else if(random <= 559)
					st.giveItems(2510, Rnd.get(200) + 401, false, false);
				else if(random <= 561)
					st.giveItems(316, 1, false, false);
				else if(random <= 578)
					st.giveItems(630, 1, false, false);
				else if(random <= 579)
					st.giveItems(188, 1, false, false);
				else if(random <= 581)
					st.giveItems(885, 1, false, false);
				else if(random <= 582)
					st.giveItems(103, 1, false, false);
				else if(random <= 584)
					st.giveItems(917, 1, false, false);
				else
