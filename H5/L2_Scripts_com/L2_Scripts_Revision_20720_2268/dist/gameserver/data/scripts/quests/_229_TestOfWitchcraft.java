 ...
 */
public class _229_TestOfWitchcraft extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(OrimsDiagram, 1, false, false);
			if(!st.getPlayer().getVarBoolean("dd3"))
			{
				st.giveItems(7562, 122, false, false);
		...
		else if(event.equalsIgnoreCase("30630-14.htm"))
		{
			...
			else
			{
				...
				if(st.getQuestItemsCount(Brimstone_1st) == 0)
					st.giveItems(Brimstone_1st, 1, false, false);
				st.setCond(4);
				st.set("id", "1");
		...
		else if(event.equalsIgnoreCase("30630-16.htm"))
		{
			...
			st.giveItems(OrimsInstructions, 1, false, false);
			st.giveItems(Orims1stLetter, 1, false, false);
			st.giveItems(Orims2stLetter, 1, false, false);
			st.setCond(6);
		}
		else if(event.equalsIgnoreCase("30110-08.htm"))
		{
			st.takeItems(Orims2stLetter, 1);
			st.giveItems(SoultrapCrystal, 1, false, false);
			st.giveItems(IkersAmulet, 1, false, false);
		...
		else if(event.equalsIgnoreCase("30417-03.htm"))
		{
			...
			st.giveItems(SirVaspersLetter, 1, false, false);
		}
		else if(event.equalsIgnoreCase("30633-02.htm"))
		{
			...
			else
			{
				...
				if(st.getQuestItemsCount(Brimstone_2nd) == 0)
					st.giveItems(Brimstone_2nd, 1, false, false);
		...
		else if(event.equalsIgnoreCase("30630-22.htm"))
		{
			...
			if(!st.getPlayer().getVarBoolean("prof2.3"))
			{
				...
				st.giveItems(ADENA_ID, 372154, true, true);
				st.getPlayer().setVar("prof2.3", "1", -1);
			}
			st.giveItems(MarkOfWitchcraft, 1, false, false);
			st.finishQuest();
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Orim)
		{
			...
			else if(st.getQuestItemsCount(MarkOfWitchcraft) != 0)
			{
		...
		else if(npcId == Iker)
		{
			if(cond == 2)
			{
				...
				else if(st.getQuestItemsCount(Aklantoth_1stGem) == 0 && st.getQuestItemsCount(IkersList) > 0)
				{
					...
					st.giveItems(Aklantoth_1stGem, 1, false, false);
		...
		else if(npcId == Vasper)
		{
			if(cond == 6)
			{
				...
				else if(st.getQuestItemsCount(VadinsSanctions) != 0)
				{
					...
					st.giveItems(SwordOfBinding, 1, false, false);
		...
		else if(npcId == Vadin)
		{
			if(cond == 6)
			{
				if(st.getQuestItemsCount(SirVaspersLetter) != 0)
				{
					...
					st.giveItems(VadinsCrucifix, 1, false, false);
				}
				...
				else if(st.getQuestItemsCount(TamlinOrcAmulet) >= 20)
				{
					...
					st.giveItems(VadinsSanctions, 1, false, false);
				}
				else if(st.getQuestItemsCount(VadinsSanctions) > 0)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 2 && st.getQuestItemsCount(LeopoldsJournal) > 0 && npcId == SkeletalMercenary)
		{
			if(st.getQuestItemsCount(Aklantoth_4stGem) == 0 && Rnd.chance(50))
				st.giveItems(Aklantoth_4stGem, 1, false, false);
			if(st.getQuestItemsCount(Aklantoth_5stGem) == 0 && Rnd.chance(50))
				st.giveItems(Aklantoth_5stGem, 1, false, false);
			if(st.getQuestItemsCount(Aklantoth_6stGem) == 0 && Rnd.chance(50))
				st.giveItems(Aklantoth_6stGem, 1, false, false);
		...
		else if(cond == 9 && npcId == DrevanulPrinceZeruel)
		{
			if(st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == SwordOfBinding)
			{
				...
				st.giveItems(PurgatoryKey, 1, false, false);
				st.giveItems(ZeruelBindCrystal, 1, false, false);
				st.unset("id");
				st.setCond(10);
