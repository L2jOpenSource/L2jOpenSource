...
public class _195_SevenSignsSecretRitualofthePriests extends QuestScript
{
	// NPCs
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(2);
			st.giveItems(GuardsoftheDawnIdentityCard, 1, false, false);
		}
		else if(event.equalsIgnoreCase("raymond_q195_3.htm"))
		{
			...
			SkillHolder.getInstance().getSkill(6204, 1).getEffects(player, player, false, false);
			st.setCond(3);
		}
		...
		else if(event.equalsIgnoreCase("open_door"))
		{
			...
			{
				int deviceId = npc.getParameter("device_id", 0);
				if(deviceId == 1)
				{
					ref.openDoor(door1);
					ref.openDoor(door2);
		...
		{
			st.giveItems(EmperorShunaimansContract, 1, false, false);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == ClaudiaAthebaldt)
		{
			if(cond == 0)
			{
				if(player.getLevel() >= 79 && player.isQuestCompleted(194))
