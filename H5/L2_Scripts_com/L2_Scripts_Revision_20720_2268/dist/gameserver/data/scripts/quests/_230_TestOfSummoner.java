...
public class _230_TestOfSummoner extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(CRYSTAL_OF_PROGRESS1_ID, 1, false, false); // give Starting Crystal
		...
		{
			st.giveItems(CRYSTAL_OF_PROGRESS2_ID, 1, false, false);
		...
		{
			st.giveItems(CRYSTAL_OF_PROGRESS3_ID, 1, false, false);
		...
		{
			st.giveItems(CRYSTAL_OF_PROGRESS4_ID, 1, false, false);
		...
		{
			st.giveItems(CRYSTAL_OF_PROGRESS5_ID, 1, false, false);
		...
		{
			st.giveItems(CRYSTAL_OF_PROGRESS6_ID, 1, false, false);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 30634)
		{
			if(cond == 0)
			{
				if(st.getPlayer().getClassId() == ClassId.WIZARD || st.getPlayer().getClassId() == ClassId.ELVEN_WIZARD || st.getPlayer().getClassId() == ClassId.DARK_WIZARD)
				{
			...
			else
			{
				...
				{ // step 2 means already talkd with lara
					...
					{ // finished all battles... the player is able to earn the marks
						...
						st.giveItems(MARK_OF_SUMMONER_ID, 1, false, false);
						if(!st.getPlayer().getVarBoolean("prof2.3"))
						{
							st.addExpAndSp(1664494, 114220);
							st.giveItems(ADENA_ID, 300960, true, true);
		...
		{ // anything realated to Lara below
			...
			else
			{
				...
				{// if( you have enough materials, receive your Beginner Arcanas, List 1 - 5
					...
					st.giveItems(BEGINNERS_ARCANA_ID, 2, false, false);
				...
				if(i[0] == npcId)
				{
					...
					if(step > 1)
						...
						{ // damn.. you won the batlle.. here are the arcanas
							...
							st.giveItems(SUMMONERS[npcId - 30635][1], 1, false, false);// give arcana
	...
	public String onDeath(Creature killer, Creature victim, QuestState st)
	{
		...
		if(victim == st.getPlayer() || victim == st.getPlayer().getServitor())
			if(npcId >= 27102 && npcId <= 27107)
			{
				...
				{
					st.set(var, "4");
					st.giveItems(defeat, 1, false, false);
				}
			}
		return null;
	}
	...
	{ // on the first attack, the stat is in battle... anytime gives crystal and set stat
		...
		if(npcId >= 27102 && npcId <= 27107)
		{
			...
			{
				st.set(var, "3");
				st.giveItems(progress, 1, false, false);
			...
			if(servitor == null || servitor.isPet())
				st.giveItems(i[2], 1, false, false);
		}
		return null;
	}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{ // if a summon dies
			...
			if(st.getInt(var) == 3)
			{
				...
				{
					st.set(var, "6");
					st.giveItems(victory, 1, false, false); // if he wons without cheating, set stat won and give victory crystal
					st.playSound(SOUND_MIDDLE);
				}
				else
