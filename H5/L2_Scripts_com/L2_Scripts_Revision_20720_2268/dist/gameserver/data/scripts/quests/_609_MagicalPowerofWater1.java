...
public class _609_MagicalPowerofWater1 extends QuestScript
{
	// NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equals("quest_accept"))
		{
			...
			if(st.getCond() == 2)
				...
				{
					st.takeItems(THIEF_KEY, 1);
					st.giveItems(STOLEN_GREEN_TOTEM, 1, false, false);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == ASEFA)
		{
			...
			else if(cond == 2 && proval == 1)
			{
				...
				npc.doCast(SkillHolder.getInstance().getSkill(4548, 1), st.getPlayer(), true);
				st.set("proval", "0");
			}
			else if(cond == 3 && st.getQuestItemsCount(STOLEN_GREEN_TOTEM) >= 1)
			{
				...
				st.giveItems(GREEN_TOTEM, 1, false, false);
				st.giveItems(DIVINE_STONE_OF_WISDOM, 1, false, false);
				st.finishQuest();
			}
		}
	...
	public String onAttack(NpcInstance npc, QuestState st)
	{
		...
		{
			npc.doCast(SkillHolder.getInstance().getSkill(4547, 1), st.getPlayer(), true);
			st.set("proval", "1");
		}
		return null;
	}
