 ...
 */
public class _066_CertifiedArbalester extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(KamaelInquisitorTraineeMark, 1, false, false);
			st.setCond(10);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Rindy)
		{
			...
			else if(st.getQuestItemsCount(KamaelInquisitorMark) > 0)
			{
		...
		else if(npcId == Selsia)
		{
			...
			else if(cond == 20)
			{
				...
				if(!st.getPlayer().getVarBoolean("prof2.1"))
				{
					...
					st.giveItems(ADENA_ID, 77666, true, true);
					st.getPlayer().setVar("prof2.1", "1", -1);
				}
				st.giveItems(KamaelInquisitorMark, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			st.giveItems(ResearchOnTheGiantsAndTheAncientRace, 1, false, false);
			st.setCond(20);
		}
		return null;
	}
