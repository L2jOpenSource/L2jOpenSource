...
public abstract class SagasSuperclass extends QuestScript
{
	protected int id = 0;
	...
	protected static int[] Quests = { 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100 };
	...
	private void FinishQuest(QuestState st, Player player)
	{
		...
		st.giveItems(ADENA_ID, 5000000, true, true);
		st.giveItems(6622, 1, true, true);
		st.finishQuest();
		player.setClassId(getClassId(player), false, true);
		player.broadcastCharInfo();
		Cast(GameObjectsStorage.getByNpcId(NPC[0]), player, 4339, 1);
	}

	public SagasSuperclass(QuestPartyType partyType, QuestRepeatType repeatType)
	{
	...
	public void giveHallishaMark(QuestState st)
	{
		...
		if(st.getQuestItemsCount(Items[3]) < 700)
			st.giveItems(Items[3], Rnd.get(1, 4), true, true); // freya change
		else
		{
	...
	public static void process_step_15to16(QuestState st)
	{
		...
		if(st.getQuestItemsCount(Resonance_Amulet) == 0)
			st.giveItems(Resonance_Amulet, 1, false, false);
		st.setCond(16);
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(1);
			st.giveItems(Items[10], 1, false, false);
			htmltext = "0-03.htm";
		}
		...
		else if(event.equalsIgnoreCase("1-4"))
		{
			...
			st.giveItems(Items[1], 1, false, false);
			htmltext = "1-06.htm";
		}
		...
		else if(event.equalsIgnoreCase("2-2"))
		{
			...
			st.giveItems(Items[4], 1, false, false);
			htmltext = "2-06.htm";
		}
		...
		else if(event.equalsIgnoreCase("3-8"))
		{
			...
			st.giveItems(Items[7], 1, false, false);
			htmltext = "3-08.htm";
		}
		...
		{
			st.giveItems(Items[9], 1, false, false);
			st.setCond(18);
			htmltext = "4-011.htm";
		}
		...
		else if(event.equalsIgnoreCase("5-1"))
		{
			...
			Cast(GameObjectsStorage.getByNpcId(NPC[5]), player, 4546, 1);
			htmltext = "5-02.htm";
		}
		else if(event.equalsIgnoreCase("6-1"))
		{
			...
			Cast(GameObjectsStorage.getByNpcId(NPC[6]), player, 4546, 1);
			htmltext = "6-03.htm";
		}
		...
		else if(event.equalsIgnoreCase("7-2"))
		{
			...
			Cast(GameObjectsStorage.getByNpcId(NPC[7]), player, 4546, 1);
			htmltext = "7-06.htm";
		}
		else if(event.equalsIgnoreCase("8-1"))
		{
			...
			Cast(GameObjectsStorage.getByNpcId(NPC[8]), player, 4546, 1);
			htmltext = "8-02.htm";
		}
		else if(event.equalsIgnoreCase("9-1"))
		{
			...
			Cast(GameObjectsStorage.getByNpcId(NPC[9]), player, 4546, 1);
			htmltext = "9-03.htm";
		}
		...
		else if(event.equalsIgnoreCase("10-2"))
		{
			...
			Cast(GameObjectsStorage.getByNpcId(NPC[10]), player, 4546, 1);
			htmltext = "10-06.htm";
		}
		...
		else if(cond == 3)
		{
			...
			else if(npcId == 31537)
			{
				if(st.getQuestItemsCount(7546) == 0)
				{
					...
					st.giveItems(7546, 1, false, false);
					return null;
				}
				else
		...
		else if(isGuardianAngels(npcId))
		{
			...
			if(st1 != null)
				if(st1.getCond() == 6)
					...
					{
						st1.giveItems(((SagasSuperclass) st1.getQuest()).Items[5], 1, false, false);
						st1.setCond(7);
					}
		}
		else
				...
				if(st2 != null)
				{
					if(!player.isInParty())
						if(st == st2)
						{
							...
							st.giveItems(Items[6], 1, false, false);
							st.setCond(9);
						}
