...
public class _408_PathToElvenwizard extends QuestScript
{
	//npc
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
		{
			...
			{
				st.setCond(1);
				st.giveItems(FERTILITY_PERIDOT_ID, 1, false, false);
		...
		else if(event.equalsIgnoreCase("408_1"))
		{
			...
			{
				st.giveItems(ROGELLIAS_LETTER_ID, 1, false, false);
		...
		else if(event.equalsIgnoreCase("408_4"))
		{
			if(st.getQuestItemsCount(ROGELLIAS_LETTER_ID) > 0)
			{
				...
				st.giveItems(CHARM_OF_GRAIN_ID, 1, false, false);
		...
		else if(event.equalsIgnoreCase("408_2"))
		{
			...
			{
				st.giveItems(APPETIZING_APPLE_ID, 1, false, false);
		...
		else if(event.equalsIgnoreCase("408_5"))
		{
			if(st.getQuestItemsCount(APPETIZING_APPLE_ID) > 0)
			{
				...
				st.giveItems(SAP_OF_WORLD_TREE_ID, 1, false, false);
			...
			{
				st.giveItems(IMMORTAL_LOVE_ID, 1, false, false);
		...
		if(npcId == ROSELLA)
		{
			...
			else if(st.getQuestItemsCount(LUCKY_POTPOURI_ID) > 0)
			{
				...
				else if(st.getQuestItemsCount(MAGICAL_POWERS_RUBY_ID) > 0 && st.getQuestItemsCount(NOBILITY_AMETHYST_ID) > 0 && st.getQuestItemsCount(PURE_AQUAMARINE_ID) > 0)
				{
					...
					{
						st.giveItems(ETERNITY_DIAMOND_ID, 1, false, false);
		...
		else if(npcId == GREENIS && cond > 0)
		{
			if(st.getQuestItemsCount(ROGELLIAS_LETTER_ID) > 0)
				...
				else
				{
					...
					st.giveItems(MAGICAL_POWERS_RUBY_ID, 1, false, false);
		...
		else if(npcId == THALIA && cond > 0)
		{
			if(st.getQuestItemsCount(APPETIZING_APPLE_ID) > 0)
				...
				else
				{
					...
					st.giveItems(PURE_AQUAMARINE_ID, 1, false, false);
			...
			if(st.getQuestItemsCount(IMMORTAL_LOVE_ID) > 0)
			{
				...
				st.giveItems(LUCKY_POTPOURI_ID, 1, false, false);
				...
				else
				{
					...
					st.giveItems(NOBILITY_AMETHYST_ID, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == PINCER_SPIDER)
		{
			...
			{
				st.giveItems(RED_DOWN_ID, 1, true, true);
				if(st.getQuestItemsCount(RED_DOWN_ID) < 5)
		...
		else if(npcId == DRYAD_ELDER)
		{
			...
			{
				st.giveItems(GOLD_LEAVES_ID, 1, true, true);
				if(st.getQuestItemsCount(GOLD_LEAVES_ID) < 5)
					st.playSound(SOUND_ITEMGET);
				else
					st.playSound(SOUND_MIDDLE);
			...
			{
				st.giveItems(AMETHYST_ID, 1, true, true);
				if(st.getQuestItemsCount(AMETHYST_ID) < 2)
					st.playSound(SOUND_ITEMGET);
				else
					st.playSound(SOUND_MIDDLE);
