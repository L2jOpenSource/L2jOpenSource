...
public class _236_SeedsOfChaos extends QuestScript
{
	// NPCs
	...
	{
		int cond = st.getCond();
		if(event.equalsIgnoreCase("32138_02b.htm"))
			st.setCond(1);
		else if(event.equalsIgnoreCase("31522_02.htm") && cond == 1)
			st.setCond(2);
		else if(event.equalsIgnoreCase("32236_08.htm") && cond == 13)
			st.setCond(14);
		else if(event.equalsIgnoreCase("32138_09.htm") && cond == 14)
			st.setCond(15);
		else if(event.equalsIgnoreCase("32237_11.htm") && cond == 16)
			st.setCond(17);
		else if(event.equalsIgnoreCase("32239_12.htm") && cond == 17)
			st.setCond(18);
		else if(event.equalsIgnoreCase("32237_13.htm") && cond == 18)
			st.setCond(19);
		else if(event.equalsIgnoreCase("32239_14.htm") && cond == 19)
			st.setCond(20);
		...
		else if(event.equalsIgnoreCase("31522-ready") && (cond == 3 || cond == 4) && st.getInt("echo") == 1)
		{
			if(cond == 3)
				st.setCond(4);
			if(!KATENAR_SPAWNED)
		...
		else if(event.equalsIgnoreCase("32238-harkil") && (cond == 5 || cond == 13))
		{
		...
		else if(event.equalsIgnoreCase("32236-hunt") && cond == 5)
		{
			st.setCond(12);
			return "32236_06.htm";
		}
		else if(event.equalsIgnoreCase("32333_02.htm") && cond == 4)
		{
			st.setCond(5);
			st.unset("echo");
		}
		else if(event.equalsIgnoreCase("32190_02.htm") && (cond == 15 || cond == 16))
		{
			if(cond == 15)
				st.setCond(16);
			st.getPlayer().teleToLocation(-119534, 87176, -12593);
		}
		else if(event.equalsIgnoreCase("32237_15.htm") && cond == 20)
		{
			st.giveItems(SCROLL_ENCHANT_WEAPON_A, 1, false, false);
			st.finishQuest();
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == KEKROPUS)
		{
			if(cond == 0)
			{
				...
				if(st.getPlayer().isQuestCompleted(234))
					return "32138_02.htm";
		...
		if(npcId == KATENAR)
			return cond < 4 ? NO_QUEST_DIALOG : cond == 4 ? "32333_01.htm" : "32333_02.htm";

		if(npcId == ROCK)
			return cond == 5 || cond == 13 ? "32238-01.htm" : "32238-00.htm";

		if(npcId == MAO)
			return cond >= 15 ? "32190_01.htm" : NO_QUEST_DIALOG;
	...
	public String onKill(NpcInstance npc, QuestState qs)
	{
		...
		if(IsInIntArray(npcId, NEEDLE_STAKATO_DRONES))
		{
			...
			{
				qs.giveItems(BLACK_ECHO_CRYSTAL, 1, false, false);
				qs.setCond(3);
			}
		}
			...
			{
				qs.giveItems(SHINING_MEDALLION, 1, false, false);
