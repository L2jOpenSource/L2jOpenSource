 ...
 */
public class _225_TestOfTheSearcher extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(LuthersLetter, 1, false, false);
			st.setCond(1);
			if(!st.getPlayer().getVarBoolean("dd3"))
			{
				//st.giveItems(7562, 82, false, false); NOT GIVEN?
		...
		else if(event.equalsIgnoreCase("30291-07.htm"))
		{
			...
			st.giveItems(LambertsMap, 1, false, false);
			st.giveItems(AlexsLetter, 1, false, false);
			st.giveItems(AlexsOrder, 1, false, false);
			st.setCond(8);
		}
		else if(event.equalsIgnoreCase("30420-01a.htm"))
		{
			st.takeItems(WineCatalog, -1);
			st.giveItems(TyrasContract, 1, false, false);
			st.setCond(10);
		}
		else if(event.equalsIgnoreCase("30730-01d.htm"))
		{
			st.takeItems(OldOrder, -1);
			st.giveItems(JaxsDiary, 1, false, false);
			st.setCond(14);
		}
		else if(event.equalsIgnoreCase("30627-01a.htm"))
		{
			...
			if(isQuest == null)
			{
				if(st.getQuestItemsCount(RustedKey) == 0)
					st.giveItems(RustedKey, 1, false, false);
				st.addSpawn(Chest);
		...
		else if(event.equalsIgnoreCase("30628-01a.htm"))
		{
			...
			st.giveItems(GoldBar, 20, false, false);
			st.setCond(18);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Luther)
		{
			...
			else if(st.getQuestItemsCount(MarkOfSearcher) != 0)
			{
			...
			else if(cond == 19)
			{
				...
				if(!st.getPlayer().getVarBoolean("prof2.3"))
				{
					...
					st.giveItems(ADENA_ID, 161806, true, true);
					st.getPlayer().setVar("prof2.3", "1", -1);
				}
				st.takeItems(AlexsRecommend, -1);
				st.giveItems(MarkOfSearcher, 1, false, false);
				st.finishQuest();
			}
		}
		else if(npcId == Alex)
		{
			if(cond == 1)
			{
				...
				st.giveItems(AlexsWarrant, 1, false, false);
				st.setCond(2);
			}
			else if(cond == 2)
			...
			else if(cond == 18)
			{
				...
				st.giveItems(AlexsRecommend, 1, false, false);
		...
		else if(npcId == Leirynn)
		{
			if(cond == 2)
			{
				...
				st.giveItems(Leirynns1stOrder, 1, false, false);
				st.setCond(3);
			}
			else if(cond == 3)
			...
			else if(cond == 4)
			{
				...
				st.giveItems(Leirynns2ndOrder, 1, false, false);
				st.setCond(5);
			}
			else if(cond == 5)
			...
			else if(cond == 6)
			{
				...
				st.giveItems(LeirynnsReport, 1, false, false);
		...
		else if(npcId == Borys)
		{
			if(cond == 8)
			{
				...
				st.giveItems(WineCatalog, 1, false, false);
			...
			else if(cond == 12)
			{
				...
				st.giveItems(OldOrder, 1, false, false);
		...
		else if(npcId == Tyra)
		{
			...
			else if(cond == 11)
			{
				...
				st.giveItems(MalrukianWine, 1, false, false);
		...
		else if(npcId == Jax)
		{
			...
			else if(cond == 15)
			{
				...
				st.giveItems(CombinedMap, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 5 && npcId == DeluChiefKalkis)
		{
			if(st.getQuestItemsCount(StrangeMap) == 0)
				st.giveItems(StrangeMap, 1, false, false);
			if(st.getQuestItemsCount(ChiefKalkisFang) == 0)
				st.giveItems(ChiefKalkisFang, 1, false, false);
			st.setCond(6);
		}
		else if(cond == 14)
		{
