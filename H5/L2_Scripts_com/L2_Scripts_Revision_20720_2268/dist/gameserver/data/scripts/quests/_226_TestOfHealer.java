...
public class _226_TestOfHealer extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
		{
			...
			{
				st.giveItems(7562, 104, false, false);
				st.getPlayer().setVar("dd3", "1", -1);
			}
			st.setCond(COND1);
			st.giveItems(REPORT_OF_PERRIN_ID, 1, false, false);
		}
		...
		else if(event.equalsIgnoreCase("30473_2"))
		{
			...
			st.giveItems(ADENA_ID, 266980, true, true);
			st.giveItems(MARK_OF_HEALER_ID, 1, false, false);
		...
		else if(event.equalsIgnoreCase("30428_1"))
		{
			...
			if(st.getQuestItemsCount(ADENA_ID) >= 100000)
			{
				...
				st.giveItems(PICTURE_OF_WINDY_ID, 1, false, false);
				st.setCond(COND7);
			}
			else
		...
		else if(event.equalsIgnoreCase("30660-03.htm"))
		{
			...
			st.giveItems(WINDYS_PEBBLES_ID, 1, false, false);
			st.setCond(COND8);
		}
		...
		else if(event.equalsIgnoreCase("30665_1"))
		{
			...
			st.giveItems(CRISTINAS_LETTER_ID, 1, false, false);
			st.setCond(COND22);
		}
		return htmltext;
	}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Bandellos)
		{
			if(cond == 0)
			{
				if(st.getPlayer().getClassId() == ClassId.KNIGHT || st.getPlayer().getClassId() == ClassId.CLERIC || st.getPlayer().getClassId() == ClassId.ORACLE || st.getPlayer().getClassId() == ClassId.ELVEN_KNIGHT)
				{
			...
			else if(cond == COND23)
			{
				if(st.getQuestItemsCount(GOLDEN_STATUE_ID) == 0)
				{
					...
					if(!st.getPlayer().getVarBoolean("prof2.3"))
					{
						...
						st.giveItems(ADENA_ID, 133490, true, true);
		...
		else if(npcId == FatherGupu)
		{
			...
			else if(cond == COND8)
			{
				...
				st.giveItems(GOLDEN_STATUE_ID, 1, false, false);
		...
		else if(npcId == Sorius)
		{
			if(cond == COND9)
			{
				...
				st.giveItems(ORDER_OF_SORIUS_ID, 1, false, false);
				st.setCond(COND10);
			}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			if(d[2] != 0)
				st.giveItems(d[2], 1, false, false);
			st.setCond(d[1]);
		}
		return null;
	}
