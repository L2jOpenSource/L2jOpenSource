 ...
 */
public class _132_MatrasCuriosity extends QuestScript
{
	// npc
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("32245-02.htm"))
		{
			...
			{
				st.giveItems(Rough_Ore_of_Fire, 1, false, false);
				st.giveItems(Rough_Ore_of_Water, 1, false, false);
				st.giveItems(Rough_Ore_of_Earth, 1, false, false);
				st.giveItems(Rough_Ore_of_Wind, 1, false, false);
				st.giveItems(Rough_Ore_of_Darkness, 1, false, false);
				st.giveItems(Rough_Ore_of_Divinity, 1, false, false);
		...
		{
			st.giveItems(ADENA_ID, 31210, true, true);
			st.finishQuest();
			return null;
		}
		return htmltext;
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		if(st.getCond() == 1)
		{
			if(npc.getNpcId() == Ranku && st.getQuestItemsCount(Rankus_Blueprint) < 1)
			{
				...
				st.giveItems(Rankus_Blueprint, 1, false, false);
			}
			if(npc.getNpcId() == Demon_Prince && st.getQuestItemsCount(Demon_Princes_Blueprint) < 1)
			{
				st.playSound(SOUND_ITEMGET);
				st.playSound(SOUND_MIDDLE);
				st.giveItems(Demon_Princes_Blueprint, 1, false, false);
			}
