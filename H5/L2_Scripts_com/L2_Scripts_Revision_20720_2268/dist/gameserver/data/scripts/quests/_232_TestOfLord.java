...
public class _232_TestOfLord extends QuestScript
{
	// NPCs
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(ORDEAL_NECKLACE, 1, false, false);
		...
		if(event.equalsIgnoreCase("30565-12.htm") && st.getQuestItemsCount(IMMORTAL_FLAME) > 0)
		{
			...
			st.giveItems(MARK_OF_LORD, 1, false, false);
			if(!st.getPlayer().getVarBoolean("prof2.3"))
			{
				st.addExpAndSp(894888, 61408); // FIXME: цифра приблизительная
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Varkees && ORDEAL_NECKLACE_COUNT > 0)
		{
			...
			st.giveItems(HUGE_ORC_FANG, 1, false, false);
			if(cond1Complete(st))
		...
		if(npcId == Manakia && ORDEAL_NECKLACE_COUNT > 0)
			if(VARKEES_CHARM_COUNT > 0 && HUGE_ORC_FANG_COUNT == 0)
			{
				if(MANAKIAS_AMULET_COUNT == 0)
				{
					...
					{
						st.giveItems(MANAKIAS_ORDERS, 1, false, false);
						return "30515-01.htm";
					}
					...
					st.giveItems(MANAKIAS_AMULET, 1, false, false);
					st.playSound(SOUND_MIDDLE);
					return "30515-03.htm";
				}
		...
		if(npcId == Tantus)
		{
			if(AXE_OF_CEREMONY_COUNT == 0)
			{
				...
				st.giveItems(AXE_OF_CEREMONY, 1, false, false);
				if(cond1Complete(st))
					st.setCond(2, SOUND_JACKPOT);
				else
					st.playSound(SOUND_MIDDLE);
		...
		if(npcId == Hatos)
		{
			if(SWORD_INTO_SKULL_COUNT == 0)
			{
				...
				st.giveItems(SWORD_INTO_SKULL, 1, false, false);
				if(cond1Complete(st))
					st.setCond(2, SOUND_JACKPOT);
				else
					st.playSound(SOUND_MIDDLE);
			...
			if(HATOS_CHARM_COUNT > 0 && SWORD_INTO_SKULL_COUNT == 0)
			{
				if(URUTU_BLADE_COUNT == 0)
				{
					if(SUMARIS_LETTER_COUNT > 0)
						return "30564-02.htm";
					st.giveItems(SUMARIS_LETTER, 1, false, false);
					st.playSound(SOUND_MIDDLE);
					return "30564-01.htm";
				}
		...
		if(npcId == Somak)
			if(SWORD_INTO_SKULL_COUNT == 0)
			{
				if(URUTU_BLADE_COUNT == 0 && HATOS_CHARM_COUNT > 0 && SUMARIS_LETTER_COUNT > 0)
				{
					...
					st.giveItems(URUTU_BLADE, 1, false, false);
					st.playSound(SOUND_MIDDLE);
					return "30510-01.htm";
				}
				if(URUTU_BLADE_COUNT > 0 && HATOS_CHARM_COUNT > 0 && SUMARIS_LETTER_COUNT == 0)
		...
		if(npcId == Takuna)
		{
			if(HANDIWORK_SPIDER_BROOCH_COUNT == 0)
			{
				...
				st.giveItems(HANDIWORK_SPIDER_BROOCH, 1, false, false);
				if(cond1Complete(st))
					st.setCond(2, SOUND_JACKPOT);
				else
					st.playSound(SOUND_MIDDLE);
		...
		if(npcId == Chianta)
		{
			...
			if(MONSTEREYE_WOODCARVING_COUNT == 0)
			{
				...
				st.giveItems(MONSTEREYE_WOODCARVING, 1, false, false);
				if(cond1Complete(st))
					st.setCond(2, SOUND_JACKPOT);
				else
					st.playSound(SOUND_MIDDLE);
		...
		if(npcId == Ancestor_Martankus)
		{
			...
			if(MARTANKUS_CHARM_COUNT > 0)
			{
				if(cond == 5 || st.getQuestItemsCount(RAGNA_CHIEF_NOTICE) > 0 && st.getQuestItemsCount(RAGNA_ORC_HEAD) > 0)
				{
					...
					st.giveItems(IMMORTAL_FLAME, 1, false, false);
					st.playSound(SOUND_MIDDLE);
					return "30649-06.htm";
				}
				return "30649-05.htm";
	...
	public String onKill(NpcInstance npc, QuestState qs)
	{
		...
		for(int item_id : _drop.itemList)
		{
			...
			{
				qs.giveItems(item_id, 1, true, true);
