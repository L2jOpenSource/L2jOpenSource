...
public class _064_CertifiedBerserker extends QuestScript
{
	// NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("32207-01a.htm"))
		{
			...
			{
				st.giveItems(Dimenional_Diamonds, 48, false, false);
		...
		{
			st.setCond(13);
			st.giveItems(H_LETTER, 1, false, false);
		...
		else if(event.equalsIgnoreCase("32215-05a.htm"))
		{
			...
			st.giveItems(T_REC, 1, false, false);
		}
		else if(event.equalsIgnoreCase("32207-03a.htm"))
		{
			if(!st.getPlayer().getVarBoolean("prof2.1"))
			{
				...
				st.giveItems(ADENA_ID, 63104, true, true);
				st.getPlayer().setVar("prof2.1", "1", -1);
			}
			st.giveItems(OrkurusRecommendation, 1, false, false);
			st.finishQuest();
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == ORKURUS)
		{
			...
			else if(st.getQuestItemsCount(OrkurusRecommendation) != 0)
			{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 2)
			if(npcId == BREKA_ORC || npcId == BREKA_ORC_ARCHER || npcId == BREKA_ORC_SHAMAN || npcId == BREKA_ORC_OVERLORD || npcId == BREKA_ORC_WARRIOR)
				...
				{
					st.giveItems(BREKA_ORC_HEAD, 1, true, true);
