...
public class _340_SubjugationofLizardmen extends QuestScript
{
	// NPCs
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30385-6.htm") && cond == 1 && st.getQuestItemsCount(TRADE_CARGO) >= 30)
		{
			st.setCond(2);
		...
		{
			st.setCond(6);
			st.giveItems(EVIL_SPIRIT_OF_DARKNESS, 1, false, false);
		}
		else if(event.equalsIgnoreCase("30037-4.htm") && cond == 6 && st.getQuestItemsCount(EVIL_SPIRIT_OF_DARKNESS) > 0)
		{
			st.setCond(7);
		...
		{
			st.giveItems(ADENA_ID, 14700, true, true);
			st.finishQuest();
		}
		else if(event.equalsIgnoreCase("30385-7.htm") && cond == 1 && st.getQuestItemsCount(TRADE_CARGO) >= 30)
		{
			st.takeItems(TRADE_CARGO, -1);
			st.giveItems(ADENA_ID, 4090, true, true);
			st.finishQuest();
		}
