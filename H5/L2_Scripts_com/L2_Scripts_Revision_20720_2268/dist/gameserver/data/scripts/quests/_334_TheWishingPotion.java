...
public class _334_TheWishingPotion extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(1);
			st.giveItems(ALCHEMY_TEXT_ID, 1, false, false);
		}
		else if(event.equalsIgnoreCase("30738-06.htm"))
		{
			if(st.getQuestItemsCount(WISH_POTION_ID) == 0)
			{
				...
				if(st.getQuestItemsCount(POTION_RECIPE_1_ID) == 0)
					st.giveItems(POTION_RECIPE_1_ID, 1, false, false);
				if(st.getQuestItemsCount(POTION_RECIPE_2_ID) == 0)
					st.giveItems(POTION_RECIPE_2_ID, 1, false, false);
		...
		else if(event.equalsIgnoreCase("30738-10.htm"))
		{
			if(checkIngr(st))
			{
				...
				if(st.getQuestItemsCount(MATILDS_ORB_ID) == 0)
					st.giveItems(MATILDS_ORB_ID, 1, false, false);
				st.giveItems(WISH_POTION_ID, 1, false, false);
				st.finishQuest();
			}
			else
		...
		else if(event.equalsIgnoreCase("30738-17.htm"))
		{
			if(st.getQuestItemsCount(WISH_POTION_ID) >= 1)
			{
				...
				if(WISH_CHANCE <= 33)
				{
					...
					st.giveItems(ADENA_ID, ((Rnd.get(10) + 1) * 1000000L), true, true);
				else
				{
					st.addSpawn(GRIMA);
					st.addSpawn(GRIMA);
		...
		else if(event.equalsIgnoreCase("30738-18.htm"))
		{
			if(st.getQuestItemsCount(WISH_POTION_ID) >= 1)
			{
				...
				int WISH_CHANCE = Rnd.get(100) + 1;
				if(WISH_CHANCE <= 33)
					st.giveItems(CERTIFICATE_OF_ROYALTY_ID, 1, false, false);
				else if(WISH_CHANCE >= 66)
					st.giveItems(ANCIENT_CROWN_ID, 1, false, false);
				else
					st.addSpawn(SANCHES);
			}
			else
		...
		//WISH 4
			...
			if(st.getQuestItemsCount(WISH_POTION_ID) >= 1)
			{
				...
				int WISH_CHANCE = Rnd.get(100) + 1;
				if(WISH_CHANCE <= 33)
					st.giveItems(SPELLBOOK_ICEBOLT_ID, 1, false, false);
				else if(WISH_CHANCE <= 66)
					st.giveItems(SPELLBOOK_BATTLEHEAL_ID, 1, false, false);
				else
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == TORAI)
		{
			if(st.getQuestItemsCount(FORBIDDEN_LOVE_SCROLL_ID) >= 1)
			{
				...
				st.giveItems(ADENA_ID, 500000, true, true);
		...
		else if(npcId == WISDOM_CHEST)
		{
			...
			{
				st.giveItems(HEART_OF_PAAGRIO_ID, 1, false, false);
				st.getPlayer().getTarget().decayMe();
				htmltext = "30743-06.htm";
			}
			else
		...
		else if(npcId == RUPINA)
		{
			...
			if(DROP_CHANCE <= DROP_CHANCE_NECKLACE_OF_GRACE_ID)
				st.giveItems(NECKLACE_OF_GRACE_ID, 1, false, false);
			else
				st.giveItems(SCROLL_OF_ESCAPE_ID, 1, false, false);
			st.getPlayer().getTarget().decayMe();
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == SUCCUBUS_OF_SEDUCTION && DROP_CHANCE <= DROP_CHANCE_FORBIDDEN_LOVE_SCROLL_ID)
		{
			...
			st.giveItems(FORBIDDEN_LOVE_SCROLL_ID, 1, false, false);
		}
		else if(npcId == GRIMA && DROP_CHANCE <= DROP_CHANCE_GOLD_BAR_ID)
		{
			st.playSound(SOUND_ITEMGET);
			st.giveItems(GOLD_BAR_ID, Rnd.get(5) + 1, true, true);
		}
		...
		{
			if(DROP_CHANCE <= 25)
				st.giveItems(DEMONS_BOOTS_ID, 1, true, true);
			else if(DROP_CHANCE <= 50)
				st.giveItems(DEMONS_GLOVES_ID, 1, true, true);
			else if(DROP_CHANCE <= 75)
				st.giveItems(DEMONS_STOCKINGS_ID, 1, true, true);
			else
				st.giveItems(DEMONS_TUNIC_ID, 1, true, true);
			st.playSound(SOUND_ITEMGET);
		}
		return null;
	}
