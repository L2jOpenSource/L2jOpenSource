 ...
 * @reworked by Bonux
**/
public final class _231_TestOfTheMaestro extends QuestScript
{
	// NPC's
	...
	// Reward's
	private static final int EXP_REWARD = 2085244; // EXP reward count
	private static final int SP_REWARD = 141240; // SP reward count
	private static final int ADENA_REWARD = 372154; // Adena reward count
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("30531-04.htm"))
		{
			...
			if(!st.getPlayer().getVarBoolean("dd3"))
			{
				...
				st.giveItems(DIMENSIONAL_DIAMOND, 23, false, false);
		...
		else if(event.equalsIgnoreCase("30556-05.htm"))
		{
			...
			{
				st.giveItems(BROKEN_TELEPORT_DEVICE, 1, false, false);
		...
		{
			st.giveItems(PAINT_OF_KAMURU, 1, false, false);
		}
		else if(event.equalsIgnoreCase("30673-04.htm"))
		{
			...
			{
				st.giveItems(REPORT_OF_CRUMA, 1, false, false);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		switch(npcId)
		{
			case IRON_GATES_LOCKIRIN:
			{
				if(cond == 0)
				{
					if(st.getPlayer().getClassId() == ClassId.ARTISAN)
					{
				...
				else if(cond == 2)
				{
					if(st.haveQuestItem(RECOMMENDATION_OF_BALANKI) && st.haveQuestItem(RECOMMENDATION_OF_FILAUR) && st.haveQuestItem(RECOMMENDATION_OF_ARIN))
					{
						...
						st.giveItems(ADENA_ID, ADENA_REWARD, true, true);
						st.giveItems(MARK_OF_MAESTRO, 1, false, false);
				...
				if(cond == 1)
				{
					...
					else if(subCond == 2)
					{
						...
						else
						{
							...
							st.giveItems(RECOMMENDATION_OF_BALANKI, 1, false, false);
			...
			case GRAY_PILLAR_MEMBER_FILAUR:
			{
				if(cond == 1)
				{
					if(subCond == 1 && !st.haveQuestItem(RECOMMENDATION_OF_FILAUR))
					{
						...
						st.giveItems(ARCHITECTURE_OF_CRUMA, 1, false, false);
						st.set("sub_cond", 4);
					}
					else if(subCond == 4)
					{
						...
						else if(st.haveQuestItem(REPORT_OF_CRUMA) && !st.haveQuestItem(ARCHITECTURE_OF_CRUMA))
						{
							...
							st.giveItems(RECOMMENDATION_OF_FILAUR, 1, false, false);
			...
			case BLACK_ANVILS_ARIN:
			{
				if(cond == 1)
				{
					if(subCond == 1 && !st.haveQuestItem(RECOMMENDATION_OF_ARIN))
					{
						...
						st.giveItems(PAINT_OF_TELEPORT_DEVICE, 1, false, false);
						st.set("sub_cond", 3);
					}
					else if(subCond == 3)
					{
						...
						else if(st.haveQuestItem(TELEPORT_DEVICE, 5))
						{
							...
							st.giveItems(RECOMMENDATION_OF_ARIN, 1, false, false);
			...
			case MASTER_TOMA:
			{
				if(cond == 1)
				{
					if(subCond == 3)
					{
						...
						{
							st.giveItems(TELEPORT_DEVICE, 5, false, false);
			...
			case CHIEF_CROTO:
			{
				if(cond == 1)
				{
					...
					else if(st.haveQuestItem(NECKLACE_OF_KAMUTU))
					{
						...
						st.giveItems(LETTER_OF_SOLDER_DERACHMENT, 1, false, false);
			...
			case RESEARCHER_LORAIN:
			{
				if(cond == 1)
				{
					if(subCond == 4)
					{
						...
						{
							st.giveItems(INGREDIENTS_OF_ANTIDOTE, 1, false, false);
