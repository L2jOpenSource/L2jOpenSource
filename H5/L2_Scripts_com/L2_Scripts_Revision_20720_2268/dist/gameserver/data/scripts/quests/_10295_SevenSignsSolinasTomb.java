 ...
 */
public class _10295_SevenSignsSolinasTomb extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getQuestItemsCount(StaffofBlessing) == 0)
				st.giveItems(StaffofBlessing, 1, false, false);
			else
		...
		{
			if(st.getQuestItemsCount(ScrollofAbstinence) == 0)
				st.giveItems(ScrollofAbstinence, 1, false, false);
			else
				htmltext = "atlar_q10295_0.htm";
		}
		...
		{
			if(st.getQuestItemsCount(SwordofHolySpirit) == 0)
				st.giveItems(SwordofHolySpirit, 1, false, false);
			else
				htmltext = "atlar_q10295_0.htm";
		}
		...
		{
			if(st.getQuestItemsCount(ShieldofSacrifice) == 0)
				st.giveItems(ShieldofSacrifice, 1, false, false);
			else
				htmltext = "atlar_q10295_0.htm";
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == ErisEvilThoughts)
		{
			if(cond == 0)
			{
				if(player.getLevel() >= 81 && player.isQuestCompleted(10294))
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(ArrayUtils.contains(SolinaGuardians, npcId) && checkGuardians(player, SolinaGuardians))
		{
			...
			player.broadcastPacket(new EventTriggerPacket(21100100, false));
			player.broadcastPacket(new EventTriggerPacket(21100102, true));
		}
