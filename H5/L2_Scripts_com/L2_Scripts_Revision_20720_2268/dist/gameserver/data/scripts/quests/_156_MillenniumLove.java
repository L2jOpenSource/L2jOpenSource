...
public class _156_MillenniumLove extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(LILITHS_LETTER, 1, false, false);
			st.setCond(1);
		}
		else if(event.equals("156_1"))
		{
			...
			{
				st.giveItems(THEONS_DIARY, 1, false, false);
				st.setCond(2);
			}
		...
		if(npcId == 30368)
		{
			...
			else if(cond == 2 && st.getQuestItemsCount(THEONS_DIARY) >= 1)
			{
				...
				if(st.getPlayer().getClassId().isMage())
					st.giveItems(GR_COMP_PACKAGE_SPS, 1, false, false);
				else
					st.giveItems(GR_COMP_PACKAGE_SS, 1, false, false);
				st.giveItems(57,3000, true, true);
