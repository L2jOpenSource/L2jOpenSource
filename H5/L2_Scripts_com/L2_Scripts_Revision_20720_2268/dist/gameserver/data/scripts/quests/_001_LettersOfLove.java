...
public class _001_LettersOfLove extends QuestScript
{
	...
	public String onEvent(String event, QuestState qs, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("quest_accept"))
		{
			...
			qs.setCond(1);
			qs.giveItems(DARINGS_LETTER, 1, false, false);
		}
		return htmltext;
	}
		...
		switch(npcId)
		{
			case DARIN:
				...
				else if(cond == 2 && st.getQuestItemsCount(ROXXY_KERCHIEF) >= 1)
				{
					...
					st.giveItems(DARINGS_RECEIPT, 1, false, false);
					st.setCond(3);
				}
				else if(cond == 3)
			...
			case ROXXY:
				if(cond == 1 && st.getQuestItemsCount(ROXXY_KERCHIEF) == 0 && st.getQuestItemsCount(DARINGS_LETTER) > 0)
				{
					...
					st.giveItems(ROXXY_KERCHIEF, 1, false, false);
					st.setCond(2);
				}
			...
			case BAULRO:
				if(cond == 3 && st.getQuestItemsCount(DARINGS_RECEIPT) >= 1)
				{
					...
					st.giveItems(BAULS_POTION, 1, false, false);
					st.setCond(4);
				}
				else if(cond == 4)
