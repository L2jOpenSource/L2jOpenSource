...
public class _372_LegacyOfInsolence extends QuestScript
{
	// NPCs
	...
	{
		st.giveItems(recipe_id, 1, false, false);
	}
	...
	private static boolean check_and_reward(QuestState st, int[] items_range, int[] reward)
	{
		...
		else if(Rnd.chance(Two_Recipes_Reward_Chance))
		{
			...
			st.giveItems(ADENA_ID, 4000, true, true);
		else
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("30844-6.htm"))
			...
			st.finishQuest();
		else if(event.equalsIgnoreCase("30839-exchange"))
