...
public class _354_ConquestofAlligatorIsland extends QuestScript
{
	//npc
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30895-05.htm"))
		{
			if(amount > 0)
				if(amount > 99)
				{
					st.giveItems(ADENA_ID, amount * 300, true, true);
					st.takeItems(ALLIGATOR_TOOTH, -1);
					st.playSound(SOUND_ITEMGET);
					int random = Rnd.get(RANDOM_REWARDS.length);
					st.giveItems(RANDOM_REWARDS[random][0], RANDOM_REWARDS[random][1], false, false);
					htmltext = "30895-05b.htm";
				}
				else
				{
					st.giveItems(ADENA_ID, amount * 100, true, true);
					st.takeItems(ALLIGATOR_TOOTH, -1);
					st.playSound(SOUND_ITEMGET);
		...
		{
			st.giveItems(PIRATES_TREASURE_MAP, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			st.giveItems(ALLIGATOR_TOOTH, 1, true, true);
			st.playSound(SOUND_ITEMGET);
		}
		if(Rnd.chance(CHANCE2) && st.getQuestItemsCount(TORN_MAP_FRAGMENT) < 10)
		{
			st.giveItems(TORN_MAP_FRAGMENT, 1, true, true);
