...
public class _234_FatesWhisper extends QuestScript
{
	// items
	...
	private final static int SEAL_ANGEL1 = 20831;
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("31002-05b.htm"))
		{
			...
			st.giveItems(HALLATES_INFERNIUM_SCEPTER, 1, false, false);
		else if(event.equalsIgnoreCase("31028-02.htm"))
			st.giveItems(KERNONS_INFERNIUM_SCEPTER, 1, false, false);
		else if(event.equalsIgnoreCase("31029-02.htm"))
			st.giveItems(GOLCONDAS_INFERNIUM_SCEPTER, 1, false, false);
		...
		else if(event.equalsIgnoreCase("30182-01c.htm"))
		{
			...
			st.giveItems(INFERNIUM_VARNISH, 1, false, false);
		}
		...
		{
			st.setCond(COND7);
			st.giveItems(PIPETTE_KNIFE, 1, false, false);
		}
		else if(event.equalsIgnoreCase("30833-01c.htm"))
		{
			st.setCond(COND8);
			st.giveItems(WhiteCloth, 30, false, false);
		}
		...
		if(newweapon != 0)
		{
			...
			{
				st.giveItems(newweapon, 1, false, false);
				st.giveItems(STAR_OF_DESTINY, 1, false, false);
				htmltext = "make.htm";
				st.finishQuest();
			}
			else
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		{
			st.giveItems(REIRIAS_SOUL_ORB, 1, false, false);
		...
		{
			st.giveItems(REORINS_HAMMER, 1, false, false);
			...
			else if((cond == COND7 || cond == COND9) && st.getQuestItemsCount(RED_PIPETTE_KNIFE) >= 1)
			{
				...
				st.giveItems(REORINS_MOLD, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == COND8 && (npcId == GUARDIAN_ANGEL || npcId == SEAL_ANGEL || npcId == SEAL_ANGEL1))
		{
			long count = st.getQuestItemsCount(BloodStainedCloth);
			if(st.getQuestItemsCount(WhiteCloth) > 0 && count < 30 && Rnd.chance(33))
			{
				st.giveItems(BloodStainedCloth, 1, true, true);
				if(count >= 29)
				{
	...
	public String onAttack(NpcInstance npc, QuestState st)
	{
		...
		if((cond == COND7 || cond == COND9) && npc.getNpcId() == BAIUM && st.getQuestItemsCount(PIPETTE_KNIFE) >= 1 && st.getQuestItemsCount(RED_PIPETTE_KNIFE) == 0 && st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == PIPETTE_KNIFE)
		{
			...
			st.giveItems(RED_PIPETTE_KNIFE, 1, false, false);
