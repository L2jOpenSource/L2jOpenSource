...
public class _999_T1Tutorial extends QuestScript
{
	...
	public String onEvent(String event, final QuestState st, NpcInstance npc)
	{
		...
		else
		{
			...
			if(st.getQuestItemsCount(e.item) > 0 && st.getInt("onlyone") == 0)
			{
				...
				{
					st.giveItems(e.gift1, e.count1, false, false);
				...
				else if(classId == e.classId2)
				{
					if(e.gift2 != 0)
					{
						st.giveItems(e.gift2, e.count2, false, false);
						st.playTutorialVoice("tutorial_voice_026");
					}
				}
				st.set("step", "3");
			...
			if(t.npcTyp == 1)
			{
				...
				else if(step == 1 && st.getQuestItemsCount(t.item) == 0 && Ex <= 2)
				{
					if(st.getQuestItemsCount(BLUE_GEM) > 0)
					{
						...
						st.giveItems(t.item, 1, false, false);
						st.set("step", "2");
						qs.set("Ex", "3");
						st.startQuestTimer("TimerEx_NewbieHelper", 30000);
						qs.set("ucMemo", "3");
						...
						{
							st.playTutorialVoice("tutorial_voice_027");
							st.giveItems(SPIRITSHOT_NOVICE, 100, false, false);
						...
						{
							st.playTutorialVoice("tutorial_voice_026");
							st.giveItems(SOULSHOT_NOVICE, 200, false, false);
