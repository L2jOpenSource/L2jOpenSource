...
public class _373_SupplierOfReagents extends QuestScript
{
	// Quest items
	...
	{
		String html = NO_QUEST_DIALOG;
		int stone = st.getInt("mixing");
		int ingredient = st.getInt("ingredient");
		int catalyst = st.getInt("catalyst");
		if(page == null)
		{
			html = "<html>" + "<body>Alchemists Mixing Urn:" + "<br><table border=0 width=300><tr>" + "<tr><td width=50%>" + "<a action=\"bypass -h npc_%objectId%_QuestEvent 373 U_M_MACT\">MACT Mixing Stone</a></td><td></td></tr>" + "<tr><td><a action=\"bypass -h npc_%objectId%_QuestEvent 373 U_I_IACT\">IACT Ingredients</a></td><td>(current: INGR)</td></tr>" + "<tr><td><a action=\"bypass -h npc_%objectId%_QuestEvent 373 U_C_CACT\">CACT Catalyst</a></td><td>(current: CATA)</td></tr>" + "<tr><td><a action=\"bypass -h npc_%objectId%_QuestEvent 373 31149-5.htm\">Select Temperature</a></td>" + "<td>(current: TEMP)</td></tr><tr><td><a action=\"bypass -h npc_%objectId%_QuestEvent 373 31149-6.htm\">Mix Ingredients</a></td><td></td></tr></table></body></html>";
		...
		else
		{
			...
			for(int i = 0; i < ITEMS.length; i++)
			{
				...
				if(item >= 6011 && item <= 6031 || item >= 6320 && item <= 6321)
					...
					{
						amt += 1;
						html += "<tr><td height=45><img src=icon." + ITEMS[i][1] + " height=32 width=32></td><td width=180>" + ITEMS[i][2] + "</td><td><button value=X1 action=\"bypass -h npc_%objectId%_QuestEvent 373 x_1_" + page[1] + "_" + str(item) + "\" width=40 height=15 fore=sek.cbui92><button value=X10 action=\"bypass -h npc_%objectId%_QuestEvent 373 x_2_" + page[1] + "_" + str(item) + "\" width=40 height=15 fore=sek.cbui92></td></tr>";
					}
			}
			if(amt == 0)
			{
				html += "<tr><td>You don't have any material that could be used with this Urn. Read the Mixing Manual.</td></tr>";
				html += "</table><center><a action=\"bypass -h npc_%objectId%_QuestEvent 373 urn\">Back</a></center></body></html>";
			}
		}
		return html;
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("30166-4.htm"))
		{
			...
			st.set("temp", "0");
			st.set("mixing", "0");
			st.giveItems(6317, 1, false, false);
			st.giveItems(5904, 1, false, false);
		}
		...
		else if(event.startsWith("U"))
		{
			...
			if(s_event[1].equals("M"))
			{
				if(s_event[2].equals("Insert"))
				{
					...
					if(st.getInt("mixing") != 0)
					{
						st.set("mixing", "0");
						st.set("temp", "0");
						st.giveItems(MIXING_STONE1, 1, false, false);
			...
			else if(s_event[2].equals("Retrieve"))
			{
				...
				if(item > 0 && qty > 0)
				{
					st.giveItems(item, qty, false, false);
			...
			if(st.getInt("mixing") > 0)
			{
				...
				if(temp != 0)
				{
					...
					if(item > 0)
					{
						...
						if(item == MIMIRS_ELIXIR)
						{
							QuestState mimirs = st.getPlayer().getQuestState(235);
							if(mimirs != null)
							{
								chance = 100;
								qty = 1;
