...
public class _224_TestOfSagittarius extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equals("1"))
		{
			...
			{
				//st.giveItems(7562, 96, false, false); not given?
				st.getPlayer().setVar("dd3", "1", -1);
			}
			st.giveItems(BERNARDS_INTRODUCTION_ID, 1, false, false);
		}
		...
		else if(event.equals("30626_2"))
		{
			...
			st.giveItems(LETTER_OF_HAMIL1_ID, 1, false, false);
			st.setCond(2);
		}
		...
		else if(event.equals("30626_4"))
		{
			...
			st.giveItems(LETTER_OF_HAMIL2_ID, 1, false, false);
			st.setCond(5);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == 30626 && st.getCond() == 8)
		{
			...
			st.giveItems(LETTER_OF_HAMIL3_ID, 1, false, false);
			st.setCond(9);
		}
		...
		else if(npcId == 30626 && st.getCond() == 14 && st.getQuestItemsCount(TALISMAN_OF_KADESH_ID) > 0)
		{
			...
			st.giveItems(MARK_OF_SAGITTARIUS_ID, 1, false, false);
		...
		else if(npcId == 30717 && st.getCond() == 11 && st.getQuestItemsCount(STAKATO_CHITIN_ID) > 0 && st.getQuestItemsCount(MITHRIL_CLIP_ID) > 0 && st.getQuestItemsCount(ST_BOWSTRING_ID) > 0 && st.getQuestItemsCount(MANASHENS_HORN_ID) > 0)
		{
			...
			st.giveItems(CRESCENT_MOON_BOW_ID, 1, false, false);
			st.giveItems(WOODEN_ARROW_ID, 10, false, false);
			st.setCond(12);
		}
		return htmltext;
	}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 20079 || npcId == 20080 || npcId == 20081 || npcId == 20084 || npcId == 20086 || npcId == 20089 || npcId == 20090)
		{
			...
			{
				st.giveItems(HUNTERS_RUNE1_ID, 1, true, true);
		...
		else if(npcId == 20269 || npcId == 20270)
		{
			...
			{
				st.giveItems(HUNTERS_RUNE2_ID, 1, true, true);
				if(st.getQuestItemsCount(HUNTERS_RUNE2_ID) >= 10)
				{
					st.takeItems(HUNTERS_RUNE2_ID, 10);
					st.giveItems(TALISMAN_OF_SNAKE_ID, 1, false, false);
					st.setCond(7);
				}
				else
					st.playSound(SOUND_ITEMGET);
		...
		else if(npcId == 20230 || npcId == 20232 || npcId == 20234)
		{
			...
			{
				st.giveItems(STAKATO_CHITIN_ID, 1, false, false);
		...
		else if(npcId == 20563)
		{
			...
			{
				st.giveItems(MANASHENS_HORN_ID, 1, false, false);
		...
		else if(npcId == 20233)
		{
			...
			{
				st.giveItems(ST_BOWSTRING_ID, 1, false, false);
		...
		else if(npcId == 20551)
		{
			...
			{
				st.giveItems(MITHRIL_CLIP_ID, 1, false, false);
		...
		else if(npcId == 20577 || npcId == 20578 || npcId == 20579 || npcId == 20580 || npcId == 20581 || npcId == 20582)
		{
			if(st.getCond() == 13)
				...
				{
					st.giveItems(BLOOD_OF_LIZARDMAN_ID, 1, false, false);
					st.playSound(SOUND_ITEMGET);
				}
		}
		else if(npcId == 27090)
			if(st.getCond() == 13 && st.getQuestItemsCount(TALISMAN_OF_KADESH_ID) == 0)
				...
				{
					st.giveItems(TALISMAN_OF_KADESH_ID, 1, false, false);
					st.setCond(14);
				}
				else
					st.addSpawn(27090);
