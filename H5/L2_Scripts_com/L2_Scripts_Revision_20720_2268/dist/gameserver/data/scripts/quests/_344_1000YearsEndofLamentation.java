...
public class _344_1000YearsEndofLamentation extends QuestScript
{
		...
		else if(event.equalsIgnoreCase("30754-06.htm") && cond == 1)
		{
			...
			else
			{
				if(Rnd.get((int) (SPECIAL / st.getRateQuestsReward())) >= amount)
					st.giveItems(ADENA_ID, amount * 60, true, true);
				else
				{
			...
			else
			{
				...
				if(random < 25)
				{
					...
					st.giveItems(OLD_KEY, 1, false, false);
				}
				else if(random < 50)
				{
					htmltext = "30754-13.htm";
					st.giveItems(OLD_HILT, 1, false, false);
				}
				else if(random < 75)
				{
					htmltext = "30754-14.htm";
					st.giveItems(OLD_TOTEM, 1, false, false);
				}
				else
					st.giveItems(CRUCIFIX, 1, false, false);
			}
		return htmltext;
	}
		...
		else if(cond == 2)
		{
			...
			if(npcId == GILMORE)
			{
				...
				if(mission == 1)
					...
					st.giveItems(4044, 1, false, false);
				else if(mission == 3)
					st.giveItems(4043, 1, false, false);
				else if(mission == 4)
					st.giveItems(4042, 1, false, false);
				if(amt > 0)
				{
					st.unset("amount");
					st.giveItems(ADENA_ID, (amt * 50) + bonus, true, true);
				}
	...
	private boolean rewards(QuestState st, int npcId)
	{
		...
		if(npcId == ORVEN && st.getQuestItemsCount(CRUCIFIX) > 0)
		{
			...
			state = true;
			if(chance < 50)
				st.giveItems(1875, 19, false, false);
			else if(chance < 70)
				st.giveItems(952, 5, false, false);
			else
				st.giveItems(2437, 1, false, false);
		}
		else if(npcId == GARVARENTZ && st.getQuestItemsCount(OLD_TOTEM) > 0)
		{
			...
			state = true;
			if(chance < 45)
				st.giveItems(1882, 70, false, false);
			else if(chance < 95)
				st.giveItems(1881, 50, false, false);
			else
				st.giveItems(191, 1, false, false);
		}
		else if(npcId == KAIEN && st.getQuestItemsCount(OLD_HILT) > 0)
		{
			...
			state = true;
			if(chance < 50)
				st.giveItems(1874, 25, false, false);
			else if(chance < 75)
				st.giveItems(1887, 10, false, false);
			else if(chance < 99)
				st.giveItems(951, 1, false, false);
			else
				st.giveItems(133, 1, false, false);
		}
		else if(npcId == RODEMAI && st.getQuestItemsCount(OLD_KEY) > 0)
		{
			...
			state = true;
			if(chance < 40)
				st.giveItems(1879, 55, false, false);
			else if(chance < 90)
				st.giveItems(951, 1, false, false);
			else
				st.giveItems(885, 1, false, false);
		}
		return state;
	}
}