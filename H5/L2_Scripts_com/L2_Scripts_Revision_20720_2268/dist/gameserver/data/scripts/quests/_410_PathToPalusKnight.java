...
public class _410_PathToPalusKnight extends QuestScript
{
	//npc
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("1"))
		{
			...
			st.giveItems(PALLUS_TALISMAN_ID, 1, false, false);
		}
		...
		else if(event.equalsIgnoreCase("30329_2"))
		{
			...
			st.giveItems(VIRGILS_LETTER_ID, 1, false, false);
			st.setCond(3);
		}
		else if(event.equalsIgnoreCase("30422_1"))
		{
			...
			st.giveItems(MORTE_TALISMAN_ID, 1, false, false);
			st.setCond(4);
		}
		else if(event.equalsIgnoreCase("30422_2"))
		{
			...
			st.giveItems(COFFIN_ETERNAL_REST_ID, 1, false, false);
			st.setCond(6);
		}
		return htmltext;
	}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == VIRGIL)
		{
			...
			else if(st.getQuestItemsCount(COFFIN_ETERNAL_REST_ID) > 0)
			{
				...
				{
					st.giveItems(GAZE_OF_ABYSS_ID, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == LYCANTHROPE)
		{
			...
			{
				st.giveItems(LYCANTHROPE_SKULL_ID, 1, true, true);
		...
		else if(npcId == POISON_SPIDER)
		{
			...
			{
				st.giveItems(PREDATOR_CARAPACE_ID, 1, false, false);
			...
			{
				st.giveItems(TRIMDEN_SILK_ID, 1, true, true);
				if(st.getQuestItemsCount(TRIMDEN_SILK_ID) > 4)
				{
