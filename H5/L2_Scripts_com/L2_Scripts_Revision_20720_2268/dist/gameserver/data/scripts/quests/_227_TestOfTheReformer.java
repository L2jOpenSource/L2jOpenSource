 ...
 */
public class _227_TestOfTheReformer extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(BookOfReform, 1, false, false);
			if(!st.getPlayer().getVarBoolean("dd3"))
			{
				st.giveItems(7562, 60, false, false);
		...
		else if(event.equalsIgnoreCase("30118-06.htm"))
		{
			...
			st.giveItems(LetterOfIntroduction, 1, false, false);
			st.setCond(4);
		}
		else if(event.equalsIgnoreCase("30666-04.htm"))
		{
			st.takeItems(LetterOfIntroduction, -1);
			st.giveItems(SlasLetter, 1, false, false);
			st.setCond(5);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Pupina)
		{
			...
			else if(st.getQuestItemsCount(MarkOfReformer) != 0)
			{
		...
		else if(npcId == Sla)
		{
			...
			else if(cond == 10)
			{
				...
				st.giveItems(Greetings, 3, false, false);
			...
			else if(cond == 20)
			{
				...
				st.giveItems(MarkOfReformer, 1, false, false);
				if(!st.getPlayer().getVarBoolean("prof2.3"))
				{
					st.addExpAndSp(1252844, 85972);
					st.giveItems(ADENA_ID, 226528, true, true);
		...
		else if(npcId == Katari)
		{
			...
			else if(cond == 9)
			{
				...
				st.giveItems(KatarisLetter, 1, false, false);
		...
		else if(npcId == OlMahumPilgrimNPC)
		{
			if(cond == 7)
			{
				st.giveItems(OlMahumMoney, 1, false, false);
		...
		else if(npcId == Kakan)
		{
			...
			else if(cond == 13)
			{
				...
				st.giveItems(KakansLetter, 1, false, false);
		...
		else if(npcId == Nyakuri)
		{
			...
			{
				st.takeItems(Greetings, 1);
				st.giveItems(NyakurisLetter, 1, false, false);
			...
			{
				st.takeItems(Greetings, -1);
				st.giveItems(UndeadList, 1, false, false);
			...
			else if(cond == 19)
			{
				...
				st.giveItems(RamussLetter, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == NamelessRevenant && (cond == 1 || cond == 2))
		{
			if(st.getQuestItemsCount(RippedDiary) < 6)
				st.giveItems(RippedDiary, 1, false, false);
		...
		else if(npcId == Aruraune)
		{
			...
			if(cond == 2)
			{
				if(st.getQuestItemsCount(HugeNail) == 0)
					st.giveItems(HugeNail, 1, false, false);
				st.setCond(3);
		...
		else if(npcId == OlMahumBetrayer)
		{
			...
			if(cond == 8)
			{
				if(st.getQuestItemsCount(LetterOfBetrayer) == 0)
					st.giveItems(LetterOfBetrayer, 1, false, false);
				st.setCond(9);
			}
		}
