...
public class _311_ExpulsionOfEvilSpirits extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("soulores"))
		{
			if(st.getQuestItemsCount(SoulCoreContainingEvilSpirit) >= 10)
			{
				...
				st.giveItems(ProtectionSoulsPendant, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 1 && ArrayUtils.contains(MOBS, npcId))
		{
			...
			{
				st.giveItems(SoulCoreContainingEvilSpirit, 1, true, true);
				st.playSound(SOUND_FANFARE2);
			}
			if(Rnd.chance(DROP_CHANCE2))
			{
				st.giveItems(RagnaOrcAmulet, 1, true, true);
