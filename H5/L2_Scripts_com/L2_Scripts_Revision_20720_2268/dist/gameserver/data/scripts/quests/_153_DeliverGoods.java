...
public class _153_DeliverGoods extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equals("30041-04.htm"))
		{
			...
			if(st.getQuestItemsCount(DELIVERY_LIST) == 0)
				st.giveItems(DELIVERY_LIST, 1, false, false);
			if(st.getQuestItemsCount(HEAVY_WOOD_BOX) == 0)
				st.giveItems(HEAVY_WOOD_BOX, 1, false, false);
			if(st.getQuestItemsCount(CLOTH_BUNDLE) == 0)
				st.giveItems(CLOTH_BUNDLE, 1, false, false);
			if(st.getQuestItemsCount(CLAY_POT) == 0)
				st.giveItems(CLAY_POT, 1, false, false);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 30041)
		{
			...
			{
				st.giveItems(RING_OF_KNOWLEDGE, 2, false, false);
		...
		else if(npcId == 30002)
		{
			if(cond == 1 && st.getQuestItemsCount(HEAVY_WOOD_BOX) >= 1)
			{
				...
				{
					st.giveItems(JACKSONS_RECEIPT, 1, false, false);
					if(st.haveQuestItem(JACKSONS_RECEIPT) && st.haveQuestItem(SILVIAS_RECEIPT) && st.haveQuestItem(RANTS_RECEIPT))
						st.setCond(2);
					else
		...
		else if(npcId == 30003)
		{
			if(cond == 1 && st.getQuestItemsCount(CLOTH_BUNDLE) >= 1)
			{
				...
				{
					st.giveItems(SILVIAS_RECEIPT, 1, false, false);
					if(st.getPlayer().getClassId().isMage())
						st.giveItems(2509, 3, false, false);
					else
						st.giveItems(1835, 6, false, false);
					if(st.haveQuestItem(JACKSONS_RECEIPT) && st.haveQuestItem(SILVIAS_RECEIPT) && st.haveQuestItem(RANTS_RECEIPT))
						st.setCond(2);
					else
						st.playSound(SOUND_ITEMGET);
			...
			if(cond == 1 && st.getQuestItemsCount(CLAY_POT) >= 1)
			{
				...
				{
					st.giveItems(RANTS_RECEIPT, 1, false, false);
					if(st.haveQuestItem(JACKSONS_RECEIPT) && st.haveQuestItem(SILVIAS_RECEIPT) && st.haveQuestItem(RANTS_RECEIPT))
						st.setCond(2);
					else
						st.playSound(SOUND_ITEMGET);
