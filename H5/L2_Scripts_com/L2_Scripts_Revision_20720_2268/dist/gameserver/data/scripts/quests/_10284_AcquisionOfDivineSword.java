...
public class _10284_AcquisionOfDivineSword extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("entermines"))
		{
			...
			if(st.getQuestItemsCount(ColdResistancePotion) < 1)
				st.giveItems(ColdResistancePotion, 1, false, false);
			enterInstance(st.getPlayer(), 138);
			return null;
		}
		else if(event.equalsIgnoreCase("leavemines"))
		{
			st.giveItems(ADENA_ID, 296425, true, true);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Rafforty)
		{
			if(cond == 0)
			{
				if(st.getPlayer().getLevel() >= 82 && st.getPlayer().isQuestCompleted(10283))
