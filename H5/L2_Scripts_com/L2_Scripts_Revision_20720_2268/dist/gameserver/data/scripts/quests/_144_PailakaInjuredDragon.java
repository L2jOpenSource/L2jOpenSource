...
public class _144_PailakaInjuredDragon extends QuestScript
{
	// NPC
	...
	private void makeBuff(NpcInstance npc, Player player, int skillId, int level)
	{
		...
		npc.callSkill(SkillHolder.getInstance().getSkill(skillId, level), target, true);
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(3);
			st.giveItems(SPEAR, 1, false, false);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == KOIO)
		{
			...
			if(st.getQuestItemsCount(SPEAR) > 0 && st.getQuestItemsCount(STAGE1) > 0)
			{
				...
				st.giveItems(ENCHSPEAR, 1, false, false);
			...
			if(st.getQuestItemsCount(ENCHSPEAR) > 0 && st.getQuestItemsCount(STAGE2) > 0)
			{
				...
				st.giveItems(LASTSPEAR, 1, false, false);
		...
		else if(npcId == KOSUPPORTER2)
		{
			if(cond == 4)
			{
				...
				st.giveItems(PSHIRT, 1, false, false);
		...
		switch(npcId)
		{
			...
			case VSCOMMAO2:
				if(st.getInt("stage") == 2)
					st.set("stage", "3");
				if(st.getQuestItemsCount(SPEAR) > 0 && st.getQuestItemsCount(STAGE1) == 0)
					st.giveItems(STAGE1, 1, false, false);
				break;
			case VSGMAG1:
			case VSGMAG2:
				if(st.getInt("stage") == 3)
					st.set("stage", "4");
				if(st.getQuestItemsCount(ENCHSPEAR) > 0 && st.getQuestItemsCount(STAGE2) == 0)
					st.giveItems(STAGE2, 1, false, false);
				break;
			case VSHGAPG1:
			case VSHGAPG2:
