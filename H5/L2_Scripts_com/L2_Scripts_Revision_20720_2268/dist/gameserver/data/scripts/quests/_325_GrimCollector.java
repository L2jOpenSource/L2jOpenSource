...
public class _325_GrimCollector extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("guard_curtiz_q0325_03.htm"))
		{
			...
			st.giveItems(ANATOMY_DIAGRAM, 1, false, false);
		else if(event.equalsIgnoreCase("samed_q0325_06.htm"))
		{
			if(pieces(st) > 0)
			{
				st.giveItems(ADENA_ID, 30 * st.getQuestItemsCount(ZOMBIE_HEAD) + 20 * st.getQuestItemsCount(ZOMBIE_HEART) + 20 * st.getQuestItemsCount(ZOMBIE_LIVER) + 50 * st.getQuestItemsCount(SKULL) + 15 * st.getQuestItemsCount(RIB_BONE) + 10 * st.getQuestItemsCount(SPINE) + 10 * st.getQuestItemsCount(ARM_BONE) + 10 * st.getQuestItemsCount(THIGH_BONE) + 2000 * st.getQuestItemsCount(COMPLETE_SKELETON), true, true);
		...
		{
			st.giveItems(ADENA_ID, 2000 * st.getQuestItemsCount(COMPLETE_SKELETON), true, true);
			st.takeItems(COMPLETE_SKELETON, -1);
		}
			...
			if(st.getQuestItemsCount(SPINE) != 0 && st.getQuestItemsCount(ARM_BONE) != 0 && st.getQuestItemsCount(SKULL) != 0 && st.getQuestItemsCount(RIB_BONE) != 0 && st.getQuestItemsCount(THIGH_BONE) != 0)
			{
				...
				st.takeItems(THIGH_BONE, 1);
				if(Rnd.chance(80))
					st.giveItems(COMPLETE_SKELETON, 1, false, false);
				else
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 20026)
		{
			if(n < 90)
			{
				...
				if(n < 40)
					st.giveItems(ZOMBIE_HEAD, 1, true, true);
				else if(n < 60)
					st.giveItems(ZOMBIE_HEART, 1, true, true);
				else
					st.giveItems(ZOMBIE_LIVER, 1, true, true);
			}
		}
		else if(npcId == 20029)
		{
		...
		else if(npcId == 20035)
		{
			if(n < 79)
			{
				st.playSound(SOUND_ITEMGET);
				if(n < 5)
					st.giveItems(SKULL, 1, true, true);
				else if(n < 15)
					st.giveItems(RIB_BONE, 1, true, true);
				else if(n < 29)
					st.giveItems(SPINE, 1, true, true);
				else
					st.giveItems(THIGH_BONE, 1, true, true);
			}
		}
		else if(npcId == 20042)
		{
			if(n < 86)
			{
				...
				if(n < 6)
					...
					st.giveItems(ARM_BONE, 1, true, true);
				else
					st.giveItems(THIGH_BONE, 1, true, true);
			}
		}
