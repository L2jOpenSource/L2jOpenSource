...
public class _102_SeaofSporesFever extends QuestScript
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(1);
			st.giveItems(ALBERRYUS_LETTER, 1, false, false);
		}
		return htmltext;
	}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 30284)
		{
			...
			else if(cond == 4 && st.getQuestItemsCount(COBS_MEDICINE1) >= 1)
			{
				...
				st.giveItems(ALBERRYUS_LIST, 1, false, false);
			...
			else if(cond == 6 && st.getQuestItemsCount(ALBERRYUS_LIST) >= 1)
			{
				...
				st.giveItems(ADENA_ID, 6331, true, true);
				st.addExpAndSp(30202, 1339);

				if(st.getPlayer().getClassId().isMage())
					st.giveItems(STAFF_OF_SENTINEL, 1, false, false);
				else
					st.giveItems(SWORD_OF_SENTINEL, 1, false, false);
		...
		else if(npcId == 30156)
		{
			if(cond == 1 && st.getQuestItemsCount(ALBERRYUS_LETTER) >= 1)
			{
				...
				st.giveItems(EVERGREEN_AMULET, 1, false, false);
				st.setCond(2);
			...
			else if(cond == 3 && st.getQuestItemsCount(EVERGREEN_AMULET) > 0 && st.getQuestItemsCount(DRYAD_TEARS) >= 10)
			{
				...
				st.giveItems(COBS_MEDICINE1, 1, false, false);
				st.giveItems(COBS_MEDICINE2, 1, false, false);
				st.giveItems(COBS_MEDICINE3, 1, false, false);
				st.giveItems(COBS_MEDICINE4, 1, false, false);
				st.giveItems(COBS_MEDICINE5, 1, false, false);
				st.setCond(4);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if((npcId == 20013 || npcId == 20019) && Rnd.chance(33))
			...
			{
				st.giveItems(DRYAD_TEARS, 1, true, true);
