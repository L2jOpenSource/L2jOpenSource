...
public class _213_TrialOfSeeker extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(1);
			st.giveItems(DufnersLetter, 1, false, false);
			if(!st.getPlayer().getVarBoolean("dd1"))
			{
				st.giveItems(7562, 128, false, false);
				st.getPlayer().setVar("dd1", "1", -1);
			}
		}
		else if(event.equalsIgnoreCase("terry_q0213_03.htm"))
		{
			st.giveItems(Terrys1stOrder, 1, false, false);
		...
		else if(event.equalsIgnoreCase("terry_q0213_07.htm"))
		{
			...
			st.giveItems(Terrys2ndOrder, 1, false, false);
			st.setCond(4);
		}
		else if(event.equalsIgnoreCase("terry_q0213_10.htm"))
		{
			...
			st.giveItems(TerrysLetter, 1, false, false);
			st.giveItems(TerrysBox, 1, false, false);
			st.setCond(6);
		}
		else if(event.equalsIgnoreCase("trader_viktor_q0213_05.htm"))
		{
			st.takeItems(TerrysLetter, -1);
			st.giveItems(ViktorsLetter, 1, false, false);
			st.setCond(7);
		}
		else if(event.equalsIgnoreCase("trader_viktor_q0213_11.htm"))
		{
			...
			st.giveItems(ViktorsRequest, 1, false, false);
			st.setCond(9);
		}
		else if(event.equalsIgnoreCase("trader_viktor_q0213_15.htm"))
		{
			...
			st.giveItems(ShilensRunestone, 1, false, false);
			st.giveItems(AnalysisRequest, 1, false, false);
			st.setCond(11);
		}
		...
		else if(event.equalsIgnoreCase("magister_marina_q0213_05.htm"))
		{
			...
			if(st.getPlayer().getLevel() < 36)
			{
				...
				st.giveItems(Terrys3rdOrder, 1, false, false);
			}
			else
			{
				htmltext = "terry_q0213_18.htm";
				st.giveItems(ListOfHost, 1, false, false);
				st.takeItems(AnalysisResult, -1);
				st.setCond(16);
			}
		return htmltext;
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Dufner)
		{
			if(cond == 0 && st.getQuestItemsCount(TerrysReport) == 0)
			{
			...
			else if(st.getQuestItemsCount(MarkofSeeker) != 0)
			{
		...
		else if(npcId == Terry)
		{
			...
			else if(cond == 7)
			{
				...
				st.giveItems(HawkeyesLetter, 1, false, false);
			...
			else if((cond == 14 || cond == 15) && st.getQuestItemsCount(Terrys3rdOrder) > 0)
			{
				...
				if(st.getQuestItemsCount(AbyssRunestone1) != 0 && st.getQuestItemsCount(AbyssRunestone2) != 0 && st.getQuestItemsCount(AbyssRunestone3) != 0 && st.getQuestItemsCount(AbyssRunestone4) != 0)
				{
					...
					st.giveItems(TerrysReport, 1, false, false);
		...
		else if(npcId == Marina)
		{
			...
			if(cond == 12)
			{
				...
				st.giveItems(ExperimentTools, 1, false, false);
				st.setCond(13);
			}
			else if(cond == 13)
