 ...
 */
public class _219_TestimonyOfFate extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(1);
			st.giveItems(KairasLetter, 1, false, false);
			if(!st.getPlayer().getVarBoolean("dd2"))
			{
				st.giveItems(7562, 98, false, false);
		...
		else if(event.equalsIgnoreCase("30114-04.htm"))
		{
			...
			st.giveItems(AldersReceipt, 1, false, false);
			st.setCond(12);
		}
		else if(event.equalsIgnoreCase("30476-12.htm"))
		{
			if(st.getPlayer().getLevel() >= 38)
			{
				...
				st.giveItems(KairasRecommendation, 1, false, false);
				st.setCond(15);
			}
			else
			{
				...
				st.takeItems(RevelationsManuscript, -1);
				st.giveItems(KairasInstructions, 1, false, false);
				st.setCond(14);
			}
		}
		else if(event.equalsIgnoreCase("30419-02.htm"))
		{
			...
			st.giveItems(ArkeniasNote, 1, false, false);
			st.setCond(17);
		}
		else if(event.equalsIgnoreCase("31845-02.htm"))
			st.giveItems(PixyGarnet, 1, false, false);
		else if(event.equalsIgnoreCase("31850-02.htm"))
			st.giveItems(BlightTreantSeed, 1, false, false);
		else if(event.equalsIgnoreCase("30419-05.htm"))
		{
			...
			st.giveItems(ArkeniasLetter, 1, false, false);
			st.setCond(18);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Kaira)
		{
			...
			else if(st.getQuestItemsCount(MarkofFate) != 0)
			{
			...
			else if(cond == 9 || cond == 10)
			{
				...
				if(AldersSpiritObject == null)
				{
					...
					if(st.getQuestItemsCount(AldersSkull2) == 0)
						st.giveItems(AldersSkull2, 1, false, false);
		...
		else if(npcId == Metheus)
		{
			if(cond == 1)
			{
				...
				st.giveItems(MetheussFuneralJar, 1, false, false);
				st.setCond(2);
			}
			else if(cond == 2)
			...
			else if(cond == 3)
			{
				...
				st.giveItems(HerbalismTextbook, 1, false, false);
			...
			else if(cond == 8)
			{
				...
				st.giveItems(AldersSkull1, 1, false, false);
		...
		else if(npcId == Ixia)
		{
			if(cond == 5)
			{
				...
				st.giveItems(IxiasList, 1, false, false);
			...
			else if(cond == 7 && st.getQuestItemsCount(MedusasIchor) >= 10 && st.getQuestItemsCount(MarshSpiderFluids) >= 10 && st.getQuestItemsCount(DeadSeekerDung) >= 10 && st.getQuestItemsCount(TyrantsBlood) >= 10 && st.getQuestItemsCount(NightshadeRoot) >= 10)
			{
				...
				st.giveItems(Belladonna, 1, false, false);
		...
		else if(npcId == Norman)
		{
			if(cond == 12)
			{
				...
				st.giveItems(RevelationsManuscript, 1, false, false);
		...
		else if(npcId == Thifiell)
		{
			if(cond == 15)
			{
				...
				st.giveItems(ThifiellsLetter, 1, false, false);
				st.giveItems(PalusCharm, 1, false, false);
		...
		else if(npcId == BloodyPixy && cond == 17)
		{
			...
			else if(st.getQuestItemsCount(RedFairyDust) == 0 && st.getQuestItemsCount(PixyGarnet) > 0 && st.getQuestItemsCount(GrandissSkull) >= 10 && st.getQuestItemsCount(KarulBugbearSkull) >= 10 && st.getQuestItemsCount(BrekaOverlordSkull) >= 10 && st.getQuestItemsCount(LetoOverlordSkull) >= 10)
			{
				...
				st.giveItems(RedFairyDust, 1, false, false);
			...
			else if(st.getQuestItemsCount(BlightTreantSap) == 0 && st.getQuestItemsCount(BlightTreantSeed) > 0 && st.getQuestItemsCount(BlackWillowLeaf) > 0)
			{
				...
				st.giveItems(BlightTreantSap, 1, false, false);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 2 && npcId == HangmanTree)
		{
			...
			st.giveItems(KasandrasRemains, 1, false, false);
			st.setCond(3);
		}
