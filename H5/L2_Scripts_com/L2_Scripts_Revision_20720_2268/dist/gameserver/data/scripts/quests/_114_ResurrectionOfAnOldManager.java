...
public class _114_ResurrectionOfAnOldManager extends QuestScript
{
	// NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("head_blacksmith_newyear_q0114_02.htm"))
		{
			...
			st.giveItems(STARSTONE2, 1, false, false);
		}
		...
		{
			st.setCond(17);
			st.giveItems(DETECTOR, 1, false, false);
		}
		...
		{
			st.setCond(14);
			st.giveItems(STARSTONE, 1, false, false);
			st.set("talk", "0");
		}
		return htmltext;
	}
	...
	public String onFirstTalk(NpcInstance npc, Player player)
	{
		...
		if(npcId == STONES && cond == 17)
		{
			...
			st.giveItems(DETECTOR2, 1, false, false);
			st.setCond(18);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == YUMI)
		{
			if(cond == 0)
			{
				if(st.getPlayer().getLevel() >= 70 && st.getPlayer().isQuestCompleted(121))
