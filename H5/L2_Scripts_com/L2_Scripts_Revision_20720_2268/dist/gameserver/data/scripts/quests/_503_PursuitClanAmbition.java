...
public class _503_PursuitClanAmbition extends QuestScript
{
	// Items
	...
	public void setLeaderVar(QuestState st, String var, String value)
	{
		...
		if(leader != null)
		{
			if ("cond".equalsIgnoreCase(var))
				leader.getQuestState(this).setCond(Integer.parseInt(value));
			else
				leader.getQuestState(this).set(var, value);
		}
	...
	public String exit503(boolean completed, QuestState st)
	{
		if(completed)
		{
			st.giveItems(Proof_Aspiration, 1, false, false);
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(G_Let_Martien, 1, false, false);
			st.setCond(1);
			st.set("Fritz", "1");
			st.set("Lutz", "1");
			st.set("Kurtz", "1");
		...
		{
			st.giveItems(G_Let_Balthazar, 1, false, false);
			st.setCond(4);
		}
		else if(event.equalsIgnoreCase("30760-16.htm"))
		{
			st.giveItems(G_Let_Rodemai, 1, false, false);
			st.setCond(7);
		}
		...
		else if(event.equalsIgnoreCase("30763-03.htm"))
		{
			if(st.getInt("Kurtz") == 1)
			{
				...
				st.giveItems(Mi_Drake_Eggs, 6, false, false);
				st.giveItems(Brooch, 1, false, false);
				st.set("Kurtz", "2");
			}
		}
		// Events Lutz
		else if(event.equalsIgnoreCase("30762-03.htm"))
		{
			...
			if(lutz == 1)
			{
				...
				st.giveItems(Mi_Drake_Eggs, 4, false, false);
				st.giveItems(Bl_Wyrm_Eggs, 3, false, false);
				st.set("Lutz", "2");
			}
		...
		else if(event.equalsIgnoreCase("30512-03.htm"))
		{
			...
			st.giveItems(Bl_Anvil_Coin, 1, false, false);
			st.set("Kurtz", "3");
		}
		// Events Balthazar
		...
		else if(event.equalsIgnoreCase("30764-06.htm"))
		{
			...
			st.set("Kurtz", "4");
			st.giveItems(Recipe_Spiteful_Soul_Energy, 1, false, false);
		}
		// Events Rodemai
		...
		else if(event.equalsIgnoreCase("30766-04.htm"))
		{
			...
			NpcInstance n = GameObjectsStorage.getByNpcId(Cleo);
			if(n != null)
				Functions.npcSay(n, "Blood and Honour");

			n = GameObjectsStorage.getByNpcId(Kalis);
			if(n != null)
				Functions.npcSay(n, "Ambition and Power");

			n = GameObjectsStorage.getByNpcId(Athrea);
			if(n != null)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(st.isNotAccepted() && npcId == Gustaf)
		{
				...
				else if(npcId == Coffer)
				{
					if(st.getCond() == 10)
						...
						else if(st.getInt("ImpGraveKeeper") == 3)
						{
							...
							st.giveItems(Scepter_Judgement, 1, false, false);
						}
						else
