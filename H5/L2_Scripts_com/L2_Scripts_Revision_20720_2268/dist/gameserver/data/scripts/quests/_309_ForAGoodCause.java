...
public class _309_ForAGoodCause extends QuestScript
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Atra)
		{
			if(cond == 0)
			{
				QuestState qs1 = st.getPlayer().getQuestState(308);
			...
			else if(cond == 1)
			{
				...
				if(fallen > 0)
					st.giveItems(MucrokianHide, fallen * 2, false, false);

				if(st.getQuestItemsCount(MucrokianHide) == 0)
					return "32647-06.htm"; // нечего менять
				else if(!st.getPlayer().isQuestCompleted(239))
