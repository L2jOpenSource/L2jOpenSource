...
public class _171_ActsOfEvil extends QuestScript
{
	//NPC
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.giveItems(RolentoCargobox, 1, false, false);
			st.giveItems(CertificateOfTheSilverScaleGuild, 1, false, false);
			st.setCond(9);
		}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Alvah)
		{
			...
			else if(cond == 11)
			{
				...
				st.giveItems(ADENA_ID, 95000, true, true);
		...
		else if(npcId == Tyra)
		{
			if(cond == 2)
			{
				if(st.getQuestItemsCount(BladeMold) >= 20)
				{
					...
					st.giveItems(TyrasBill, 1, false, false);
		...
		else if(npcId == Burai)
		{
			...
			if(cond == 10)
				if(st.getQuestItemsCount(OlMahumCaptainHead) >= 30)
				{
					...
					st.giveItems(ADENA_ID, 8000, true, true);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		else if(cond == 5 && npcId == TumranBugbear)
		{
			...
			{
				st.giveItems(WeaponsTradeContract, 1, false, false);
				st.playSound(SOUND_ITEMGET);
			}
			else if(st.getQuestItemsCount(AttackDirectives) == 0 && Rnd.chance(CHANCE25))
			{
				st.giveItems(AttackDirectives, 1, false, false);
				st.playSound(SOUND_ITEMGET);
			}
		return null;
	}
