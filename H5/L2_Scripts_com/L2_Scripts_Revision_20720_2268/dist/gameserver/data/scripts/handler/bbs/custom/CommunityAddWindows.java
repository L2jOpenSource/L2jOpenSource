...
public class CommunityAddWindows extends CustomCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		{
			if(Config.MAX_ACTIVE_ACCOUNTS_ON_ONE_HWID <= 0 || BBSConfig.ADD_WINDOW_SERVICE_COST_ITEM_ID_PER_1_DAY <= 0 && BBSConfig.ADD_WINDOW_SERVICE_COST_ITEM_ID_PER_FOREVER <= 0)
			{
			...
			TIntStringHashMap tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/pages/additional_windows.htm", player);
			html = tpls.get(0);

			final int[] limits = HardwareLimitsDAO.getInstance().select(player.getHWID());
			...
			else
			{
				...
				if("add".equals(cmd2))
				{
					...
					{
						HardwareLimitsDAO.getInstance().insert(player.getHWID(), activeWindows + 1, currentExpire);
				...
				else if("buy".equals(cmd2))
				{
					...
					if("unlim".equals(cmd3))
					{
						...
						if(price <= 0 || ItemFunctions.deleteItem(player, BBSConfig.ADD_WINDOW_SERVICE_COST_ITEM_ID_PER_1_DAY, price, true))
						{
							HardwareLimitsDAO.getInstance().insert(player.getHWID(), activeWindows, -1);
					...
					else
					{
						...
						if(price <= 0 || ItemFunctions.deleteItem(player, BBSConfig.ADD_WINDOW_SERVICE_COST_ITEM_ID_PER_1_DAY, price, true))
						{
							HardwareLimitsDAO.getInstance().insert(player.getHWID(), activeWindows, currentExpire + (days * 24 * 60 * 60));
