...
public final class CommunityClan extends ScriptsCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		else if("bbsclanlist".equals(cmd))
		{
			...
			TIntStringHashMap tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/bbs_clanlist.htm", player);
			String html = tpls.getNotNull(0);

			Clan playerClan = player.getClan();
			if(playerClan != null)
			{
				String my_clan = tpls.getNotNull(1);
		...
		else if("bbsclanhome".equals(cmd))
		{
			...
			TIntStringHashMap tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/bbs_clan.htm", player);
			String html = tpls.getNotNull(0);
			...
			if(player.getClanId() == clanId && player.isClanLeader())
				html = html.replace("<?menu?>", tpls.getNotNull(1));
			else
		...
		else if("bbsclanadmin".equals(cmd))
		{
			...
			player.sendPacket(new ShowBoardPacket(html, "1001", player));
		...
		else if("announcepledgewriteform".equals(cmd))
		{
			...
			TIntStringHashMap tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/bbs_clanannounce.htm", player);
			String html = tpls.getNotNull(0);
			...
			if(type == 0)
				html = html.replace("<?usage?>", tpls.getNotNull(1));
			else
				html = html.replace("<?usage?>", tpls.getNotNull(2));
	...
	private class Listener implements OnPlayerEnterListener
	{
		...
		public void onPlayerEnter(Player player)
		{
			...
			if(!clan.getNotice().isEmpty())
			{
				...
				player.sendPacket(new NpcHtmlMessagePacket(0).setHtml(html));
			}
		}
	}
