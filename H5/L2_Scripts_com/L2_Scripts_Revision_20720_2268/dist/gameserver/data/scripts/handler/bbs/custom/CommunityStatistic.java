...
public class CommunityStatistic extends CustomCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("cbbsstatistic".equals(cmd))
		{
			...
			TIntStringHashMap tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/pages/statistic.htm", player);
			html = tpls.get(0);
	...
	private void loadStatistic()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("SELECT c.obj_Id, c.char_name, c.pkkills, c.pvpkills, c.onlinetime, cs.exp FROM characters AS c LEFT JOIN character_subclasses AS cs ON c.obj_Id=cs.char_obj_id AND cs.isBase=1");
			...
			statement = con.prepareStatement("SELECT char_id, olympiad_points FROM olympiad_nobles");
			rset = statement.executeQuery();
			while(rset.next())
			{
	...
	private synchronized void refreshStatistic()
	{
		...
		StatisticData[] array = _statistic.values(new StatisticData[_statistic.size()]);
	...
	private void updateStatistic(Player player)
	{
		...
		stat.lvl = subClass != null ? subClass.getLevel() : player.getLevel();
		stat.online = (int) player.getOnlineTime();
		...
		stat.olympiad = Olympiad.getNoblePoints(player.getObjectId());
	}
}