...
public class CommunityAuctionHouse extends CustomCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if(!Config.ENABLE_AUCTION_SYSTEM)
		{
			...
			player.sendPacket(new ShowBoardPacket(msg, "101", player));
		}
					...
					if(action == 1)
					{
						...
						else
						{
							...
							else
							{
								...
								ConfirmDlgPacket packet = new ConfirmDlgPacket(SystemMsg.S1, 60000).addString("Are you sure, you want to buy " + quantity + ' ' + item.getName(player) + " for " + Util.formatAdena(realPrice) + " adena?");
		...
		else if(cmd.equals("bbsNewAuction"))
		{
			...
			if(buttonClicked != null)
			{
				...
				if(buttonClicked.equals("0"))
				{
					...
					{
						ConfirmDlgPacket packet = new ConfirmDlgPacket(SystemMsg.S1, 60000).addString("Are you sure, you want to sell " + item.getName(player) + "?");
				...
				else if(buttonClicked.equals("1"))
				{
					...
					{
						ConfirmDlgPacket packet = new ConfirmDlgPacket(SystemMsg.S1, 60000).addString("Are you sure, you want to cancel " + item.getName(player) + " auction?");
	...
	private String getItemName(Player player, ItemInstance item, int windowWidth, int windowHeight, String... addToItemName)
	{
		...
		if(item.getTemplate().isMasterwork())
