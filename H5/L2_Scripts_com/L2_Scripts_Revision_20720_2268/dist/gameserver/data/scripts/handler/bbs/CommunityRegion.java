...
public final class CommunityRegion extends ScriptsCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("bbsloc".equals(cmd))
		{
			...
			TIntStringHashMap tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/bbs_region_list.htm", player);
			String html = tpls.getNotNull(0);
			html = html.replace("%REGION_LIST%", rl.toString());
			html = html.replace("<?tree_menu?>", tpls.getNotNull(1));
		...
		else if("bbsregion".equals(cmd))
		{
			...
			String html = tpls.getNotNull(0);
			html = html.replace("%REGION_LIST%", rl.toString());
			html = html.replace("<?tree_menu?>", tpls.getNotNull(2).replace("%TREE%", "&nbsp;>&nbsp;" + HtmlUtils.htmlNpcString(town[0])));
		...
		else if("bbsregview".equals(cmd))
		{
			...
			if(type < 2)
			{
				...
				for(TradeItem ti : sl)
				{
					...
					if(item != null)
					{
						...
						String desc = "";
						if(item.getItemGrade() != ItemTemplate.Grade.NONE)
						{
							desc = _grade[item.getItemGrade().ordinal() - 1];
			...
			else
			{
				...
				for(ManufactureItem mi : cl)
				{
					...
					String desc = "";
					if(item.getItemGrade() != ItemTemplate.Grade.NONE)
						desc = _grade[item.getItemGrade().ordinal() - 1] + (item.getCrystalCount() > 0 ? (player.isLangRus() ? " Кристаллов: " : " Crystals: ") + item.getCrystalCount() + ";&nbsp;" : ";&nbsp;");
