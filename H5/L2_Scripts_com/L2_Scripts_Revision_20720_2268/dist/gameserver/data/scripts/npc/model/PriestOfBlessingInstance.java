...
public class PriestOfBlessingInstance extends NpcInstance
{
	...
	private void buyLimitedItem(Player player, String var, int itemId, int price, boolean isGlobalVar)
	{
		...
		if(isGlobalVar)
		{
			...
			{
				String val = CharacterVariablesDAO.getInstance().getVarFromPlayer(objId, var);
				if(val != null)
		...
		if(_remaining_time >= _reuse_time)
		{
			...
			{
				ItemFunctions.addItem(player, itemId, 1L, true, "Buy limited item by PriestOfBlessingInstance");
