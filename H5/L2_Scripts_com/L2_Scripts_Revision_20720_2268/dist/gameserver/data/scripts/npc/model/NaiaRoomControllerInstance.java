...
public class NaiaRoomControllerInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if(command.startsWith("challengeroom"))
		{
			...
			{
				player.sendPacket(new NpcHtmlMessagePacket(player, this).setHtml("Ingenious Contraption:<br><br>The room is already challenged."));
				return;
			}
	...
	private void spawnToRoom(int mobId, int count, Territory territory, int roomId)
	{
		for(int i = 0; i < count; i++)
		{
			try
			{
				...
				sp.setLoc(Territory.getRandomLoc(territory, false).setH(Rnd.get(65535)));
				sp.doSpawn(true);
				sp.stopRespawn();
