...
public class TrickOfTrans extends Functions implements ScriptFile, OnDeathListener, OnPlayerEnterListener
{
	...
	public void accept()
	{
		...
		if(!player.findRecipe(RED_PSTC_R))
			ItemFunctions.addItem(player, RED_PSTC, 1, "Trick of trans event give recipe");
		if(!player.findRecipe(BLACK_PSTC_R))
			ItemFunctions.addItem(player, BLACK_PSTC, 1, "Trick of trans event give recipe");
		if(!player.findRecipe(BLUE_PSTC_R))
			ItemFunctions.addItem(player, BLUE_PSTC, 1, "Trick of trans event give recipe");
		if(!player.findRecipe(GREEN_PSTC_R))
			ItemFunctions.addItem(player, GREEN_PSTC, 1, "Trick of trans event give recipe");
		if(!player.findRecipe(ORANGE_PSTC_R))
			ItemFunctions.addItem(player, ORANGE_PSTC, 1, "Trick of trans event give recipe");
		if(!player.findRecipe(WHITE_PSTC_R))
			ItemFunctions.addItem(player, WHITE_PSTC, 1, "Trick of trans event give recipe");
	...
	public void open()
	{
		...
		if(ItemFunctions.getItemCount(player, A_CHEST_KEY) > 0)
		{
			...
			ItemFunctions.addItem(player, PhilosophersStoneOre, Rnd.get(1, PhilosophersStoneOreMax), "Trick of trans open chest reward");
			ItemFunctions.addItem(player, MagicReagents, Rnd.get(1, MagicReagentsMax), "Trick of trans open chest reward");
			if(Rnd.chance(80))
				ItemFunctions.addItem(player, PhilosophersStoneConversionFormula, 1, "Trick of trans open chest reward");
