	...
	public void exchange(String[] var)
	{
		...
		if(var[0].equalsIgnoreCase("0"))
		{
			if(ItemFunctions.getItemCount(player, 5556) >= 4 && ItemFunctions.getItemCount(player, 5557) >= 4 && ItemFunctions.getItemCount(player, 5558) >= 10 && ItemFunctions.getItemCount(player, 5559) >= 1)
			{
				...
				ItemFunctions.addItem(player, 5560, 1, "Christmas event reward"); // Christmas Tree
				return;
			}
		...
		if(var[0].equalsIgnoreCase("1"))
		{
			if(ItemFunctions.getItemCount(player, 5560) >= 10)
			{
				...
				ItemFunctions.addItem(player, 5561, 1, "Christmas event reward"); // Special Christmas Tree
				return;
			}
			player.sendPacket(Msg.YOU_DO_NOT_HAVE_ENOUGH_REQUIRED_ITEMS);
		}
		if(var[0].equalsIgnoreCase("2"))
		{
			...
			{
				ItemFunctions.deleteItem(player, 5560, 10);
				ItemFunctions.addItem(player, 7836, 1, "Christmas event reward"); // Santa's Hat
				return;
			}
			player.sendPacket(Msg.YOU_DO_NOT_HAVE_ENOUGH_REQUIRED_ITEMS);
		}
		if(var[0].equalsIgnoreCase("3"))
		{
			...
			{
				ItemFunctions.deleteItem(player, 5560, 10);
				ItemFunctions.addItem(player, 8936, 1, "Christmas event reward"); // Santa's Antlers
				return;
			}
			player.sendPacket(Msg.YOU_DO_NOT_HAVE_ENOUGH_REQUIRED_ITEMS);
		}
		if(var[0].equalsIgnoreCase("4"))
		{
			if(ItemFunctions.getItemCount(player, 5560) >= 20)
			{
				...
				ItemFunctions.addItem(player, 10606, 1, "Christmas event reward"); // Agathion Seal Bracelet - Rudolph
				return;
			}
			player.sendPacket(Msg.YOU_DO_NOT_HAVE_ENOUGH_REQUIRED_ITEMS);
		}
