...
public class SavingSnowman extends Functions implements ScriptFile, OnDeathListener, OnPlayerEnterListener
{
	...
	public static void reward(NpcInstance rewarder, Player rewarded)
	{
		...
		ItemFunctions.addItem(rewarded, 14616, 1, "Saving Snowman reward"); // Gift from Santa Claus
	...
	public void buff()
	{
		...
		player.altOnMagicUseTimer(player, SkillHolder.getInstance().getSkill(23017, 1));
		...
		if(servitor != null)
		{
			servitor.broadcastPacket(new MagicSkillUse(servitor, servitor, 23017, 1, 0, 0));
			servitor.altOnMagicUseTimer(servitor, SkillHolder.getInstance().getSkill(23017, 1));
		}
	}
	...
	public void lotery()
	{
		...
		if(chance < 300000)
			ItemFunctions.addItem(player, 5561, 1, "Saving Snowman lotery reward");
		// Christmas Red Sock                18%
		else if(chance < 480000)
			ItemFunctions.addItem(player, 14612, 1, "Saving Snowman lotery reward");
		// Santa Claus' Weapon Exchange Ticket - 12 Hour Expiration Period      15%
		else if(chance < 630000)
			ItemFunctions.addItem(player, 20107, 1, "Saving Snowman lotery reward");
		// Gift from Santa Claus             5%
		else if(chance < 680000)
			ItemFunctions.addItem(player, 14616, 1, "Saving Snowman lotery reward");
		// Rudolph's Nose                    5%
		else if(chance < 730000 && ItemFunctions.getItemCount(player, 14611) == 0)
			ItemFunctions.addItem(player, 14611, 1, "Saving Snowman lotery reward");
		// Santa's Hat                       5%
		else if(chance < 780000 && ItemFunctions.getItemCount(player, 7836) == 0)
			ItemFunctions.addItem(player, 7836, 1, "Saving Snowman lotery reward");
		// Santa's Antlers                   5%
		else if(chance < 830000 && ItemFunctions.getItemCount(player, 8936) == 0)
			ItemFunctions.addItem(player, 8936, 1, "Saving Snowman lotery reward");
		// Agathion Seal Bracelet - Rudolph (постоянный предмет)                5%
		else if(chance < 880000 && ItemFunctions.getItemCount(player, 10606) == 0)
			ItemFunctions.addItem(player, 10606, 1, "Saving Snowman lotery reward");
		// Agathion Seal Bracelet: Rudolph - 30 дней со скилом на виталити      5%
			...
			ItemFunctions.addItem(player, 20094, 1, "Saving Snowman lotery reward");
		// Chest of Experience (Event)       3%
		else if(chance < 960000)
			ItemFunctions.addItem(player, 20575, 1, "Saving Snowman lotery reward");
		// Призрачные аксессуары             2.5%
		else if(chance < 985000)
			ItemFunctions.addItem(player, Rnd.get(9177, 9204), 1, "Saving Snowman lotery reward");
		// BOSE или BRES                     1.2%
		else if(chance < 997000)
			ItemFunctions.addItem(player, Rnd.get(9156, 9157), 1, "Saving Snowman lotery reward");
	}
	...
	{
		Territory terr = ZoneHolder.getInstance().getRandomZone().getTerritory();
		Location loc = Territory.getRandomLoc(terr, false).setH(Rnd.get(65535));
		return loc;
	}
	...
	public static void freeSnowman(Creature topDamager)
	{
		...
		ItemFunctions.addItem(player, 20034, 3, "Saving Snowman free snowman reward"); // Revita-Pop
		ItemFunctions.addItem(player, 20338, 1, "Saving Snowman free snowman reward"); // Rune of Experience Points 50%	10 Hour Expiration Period
		ItemFunctions.addItem(player, 20344, 1, "Saving Snowman free snowman reward"); // Rune of SP 50% 10 Hour Expiration Period
