...
//		}
		...
		env.skill = this;
		if (this != null && getId() == Skill.SKILL_SERVITOR_SHARE)
...
//			return SystemMsg.INVALID_TARGET;
			...
			if (pcTarget != null)
			{
				...
				if (isOffensive())
				{
					if (player.isInOlympiadMode() && !player.isOlympiadCompStarted()) // Ð‘Ð¾Ð¹ ÐµÑ‰Ðµ Ð½Ðµ Ð½Ð°Ñ‡Ð°Ð»Ñ�Ñ�
						return SystemMsg.INVALID_TARGET;
					if (player.isInOlympiadMode() && player.isOlympiadCompStarted() && player.getOlympiadSide() == pcTarget.getOlympiadSide() && !forceUse) // Ð¡Ð²Ð¾ÑŽ ÐºÐ¾Ð¼Ð°Ð½Ð´Ñƒ Ð°Ñ‚Ð°ÐºÐ¾Ð²Ð°Ñ‚ÑŒ Ð½ÐµÐ»ÑŒÐ·Ñ�
						return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
		...
		switch (_targetType)
		{
			...
			case TARGET_ALLY:
			{
				...
				for (Player target : World.getAroundPlayers(player, _skillRadius, 600))
				{
                  ...
                  // for fightclub event restriction to not buff/heal opponent	
                    if (player.isInFightClub() && target.isInFightClub() 
                            && player.getFightClubEvent().getFightClubPlayer((Creature) player).getTeam() != target.getFightClubEvent().getFightClubPlayer((Creature) target).getTeam())
                    {
                        continue;	
                    }	
	...
	private void addTargetsToList(List<Creature> targets, Creature aimingTarget, final Creature activeChar, boolean forceUse)
	{
		...
		if (_skillType == SkillType.AGGRESSION)
		{
			Collections.sort(list, new Comparator<Creature>()
			{
				...
				{
					return Boolean.compare(c1.getAI().getAttackTarget() == activeChar, c2.getAI().getAttackTarget() == activeChar);
				}
			});
		}
		else if (_skillType == SkillType.SPOIL)
		{
			Collections.sort(list, new Comparator<Creature>()
			{
				...
				public int compare(Creature c1, Creature c2)
				{
					return Boolean.compare(c1.isMonster() && ((MonsterInstance)c1).isSpoiled(), c2.isMonster() && ((MonsterInstance)c2).isSpoiled());
				}
			});
		}
	 ...
	 * @param inNewThread
	 */
	public final void getEffects(final Creature effector, final Creature effected, final boolean calcChance, final boolean applyOnCaster, final long timeConst, final double timeMult, final boolean skillReflected, boolean inNewThread)
	{
...
if(!BalancerConfig.CUSTOM_POWER_SKILLS_ENABLED)
				...
				if(BalancerConfig.CUSTOM_POWER_SKILLS.containsKey(skillId))
				{
					...
					Skill skillDatabase = SkillTable.getInstance().getInfo(skillId, getBaseLevel());
					Skill skillDatabaseDecreased = SkillTable.getInstance().getInfo(skillId, getBaseLevel()-1);
					...
					{
						Skill enchantSkillDatabase = SkillTable.getInstance().getInfo(skillId, currentSkillLevel);
						...
						if(enchantSkillDatabasePower > skillDatabaseBasePower)
						{
							...
							{
								Skill enchantSkillDatabaseDecreased = SkillTable.getInstance().getInfo(skillId, currentSkillLevel-1);
...
public int getCurrentEnchantLevel()
	{
		int baseMaxLvl = SkillTable.getInstance().getBaseLevel(getId());
	...
	public String getCurrentEnchantType()
	{
		int baseMaxLvl = SkillTable.getInstance().getBaseLevel(getId());
		int enchantType = (getLevel() - baseMaxLvl) / getEnchantLevelCount();

		return enchantType >= 3 ? "Power" : enchantType >= 2 ? "Cost" : (enchantType >= 1 ? "Time" : "Normal");
	}
}