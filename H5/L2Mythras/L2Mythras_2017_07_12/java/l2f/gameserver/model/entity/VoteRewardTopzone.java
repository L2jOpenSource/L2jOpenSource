...
public class VoteRewardTopzone
{
	...
	private static void reward()
	{
		...
		if (lastVotes == 0)
		{
			...
			{
				Announcements.getInstance().announceToAll("Topzone: We need " + (firstPageVotes - lastVotes) + " vote(s) to get to the first page of topzone for reward.");
				if (Config.ALLOW_TOPZONE_GAME_SERVER_REPORT)
				{
		...
		if (currentVotes >= (lastVotes + voteRewardVotesDifference))
		{
			if ((firstPageVotes - currentVotes) <= 0)
			{
				...
				Announcements.getInstance().announceToAll("Topzone: Everyone has been rewarded with reward.");
				Announcements.getInstance().announceToAll("Topzone: Current vote count is " + currentVotes + ".");
			...
			else
			{
				...
				Announcements.getInstance().announceToAll("Topzone: Everyone has been rewarded with reward.");
				Announcements.getInstance().announceToAll("Topzone: Current vote count is " + currentVotes + ".");
				Announcements.getInstance().announceToAll("Topzone: We need " + (firstPageVotes - currentVotes) + " vote(s) to get to the first page of topzone for reward.");
				for (Player player : GameObjectsStorage.getAllPlayers())
				{
					boolean canReward = false;
					String pIp = player.getIP();
