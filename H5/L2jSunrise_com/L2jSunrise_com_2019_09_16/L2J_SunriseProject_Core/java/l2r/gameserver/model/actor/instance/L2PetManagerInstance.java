...
public class L2PetManagerInstance extends L2MerchantInstance
{
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		else if (command.startsWith("evolve"))
		{
			...
			if (!ok)
			{
				...
				html.setFile(player, player.getHtmlPrefix(), "data/html/petmanager/evolve_no.htm");
				player.sendPacket(html);
			}
			return;
		}
		else if (command.startsWith("restore"))
		{
			...
			{
				NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
				html.setFile(player, player.getHtmlPrefix(), "data/html/petmanager/restore_no.htm");
				player.sendPacket(html);
			}
			return;
		}
	...
	public final void exchange(L2PcInstance player, int itemIdtake, int itemIdgive)
	{
		...
		if (player.destroyItemByItemId("Consume", itemIdtake, 1, this, true))
		{
			...
			html.setFile(player, player.getHtmlPrefix(), "data/html/petmanager/" + getId() + ".htm");
			player.sendPacket(html);
		}
		else
		{
			html.setFile(player, player.getHtmlPrefix(), "data/html/petmanager/exchange_no.htm");
			player.sendPacket(html);
		}
	}
}
