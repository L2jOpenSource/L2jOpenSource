...
public class L2DynamicZone extends L2ZoneType
{
	...
	private final L2Effect _skill;
	
	public L2DynamicZone(L2WorldRegion region, L2Character owner, L2Effect skill)
	{
		super(-1);
		_region = region;
		_owner = owner;
		...
		Runnable r = () -> remove();
		getSettings().setTask(ThreadPoolManager.getInstance().scheduleGeneral(r, skill.getRemainingTime()));
	}
	...
	protected void onEnter(L2Character character)
	{
		...
		if (_owner != null)
		{
			_skill.getSkill().getEffects(_owner, character);
		}
	}
	...
	protected void onExit(L2Character character)
	{
		...
		character.stopSkillEffects(_skill.getSkill().getId());
	}
	...
	protected void remove()
	{
		...
		{
			member.stopSkillEffects(_skill.getSkill().getId());
		}
		_owner.stopSkillEffects(_skill.getSkill().getId());
	}
