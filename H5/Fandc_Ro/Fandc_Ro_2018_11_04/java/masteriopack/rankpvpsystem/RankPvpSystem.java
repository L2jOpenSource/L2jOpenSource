...
public class RankPvpSystem 
{
	...
	public static void updateNickAndTitleColor(Player killer, PvpSummary killerPvpSummary)
	{
		...
		if(!RPSConfig.GM_IGNORE_ENABLED && killer.isGM())
			return;
	...
	private boolean checkLegalKillProtection(Pvp pvp)
	{
		...
		if(RPSConfig.DAILY_LEGAL_KILL_PROTECTION > 0 && pvp.getKillsLegalToday() > RPSConfig.DAILY_LEGAL_KILL_PROTECTION)
		{
			return false;
		}
	...
	private boolean checkLegalKillConditions(Player killer, Player victim, Pvp pvp)
	{
		...
		if(!RPSConfig.LEGAL_KILL_FOR_PK_KILLER_ENABLED && victim.getKarma() > 0)
		{
			return false;
		}
