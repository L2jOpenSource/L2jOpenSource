...
import masteriopack.rankpvpsystem.RPSConfig.DecreaseMethod;
...
public class PvpSummary
{
	...
	public int decreasePvpExpBy(int[] values)
	{
		...
		if(RPSConfig.PVP_EXP_DECREASE_METHOD == DecreaseMethod.FULL)
			value = values[0];
		else if(RPSConfig.PVP_EXP_DECREASE_METHOD == DecreaseMethod.BASIC)
			value = values[0] - values[1] - values[2] - values[3];
		else if(RPSConfig.PVP_EXP_DECREASE_METHOD == DecreaseMethod.CONSTANT)
			value = RPSConfig.PVP_EXP_DECREASE_CONSTANT;
		else if(RPSConfig.PVP_EXP_DECREASE_METHOD == DecreaseMethod.FRACTION)
