...
public class AutoHuntingManager
{
	...
	public synchronized void reportBot(Player reported, Player reporter, String typeOfReport, String moreInfo)
	{
		...
		/*
		 * if (!reported.isInCombat() && !reported.isInPeaceZone()) { reporter.sendMessage(new CustomMessage("l2f.gameserver.instancemanager.autohuntingmanager.message7", reporter)); return; }
		 */
		...
		try
		{
			...
			_unread.put(maxId, new String[] { reported.getName(), reporter.getName(), String.valueOf(date), typeOfReport, moreInfo });
		}
	...
	public boolean validateBot(Player reported, Player reporter)
	{
		...
		/*
		 * // Cannot report if reported and reporter are in war if (reported.getClan() != null && reporter.getClan() != null) { if (reported.getClan().isAtWarWith(reporter.getClanId())) { reporter.sendPacket(new SystemMessage2(SystemMsg.CANNOT_REPORT_TARGET_IN_CLAN_WAR)); return false; } }
		 */
