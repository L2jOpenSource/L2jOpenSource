...
//	}
	...
	{
		try (Connection con = LoginDatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("SELECT securityQuestion FROM accounts WHERE login='" + accountName + "'"); ResultSet rset = statement.executeQuery())
		{
			while (rset.next())
			{
	...
	{
		try (Connection con = LoginDatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("SELECT securityAnswer, email FROM accounts WHERE login='" + accountName + "'"); ResultSet rset = statement.executeQuery())
		{
			while (rset.next())
			{
	...
	{
		boolean contains = false;
		try (Connection con = LoginDatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("SELECT email FROM accounts WHERE login='" + accountName + "'"); ResultSet rset = statement.executeQuery())
		{
			while (rset.next())
			{
				String email = rset.getString("email");
	...
	{
		try (Connection con = LoginDatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("UPDATE accounts SET securityQuestion=?,securityAnswer=?,email=? WHERE login=?"))
		{
	...
	{
		try (Connection con = LoginDatabaseFactory.getInstance().getConnection(); PreparedStatement statement = con.prepareStatement("UPDATE accounts SET password = ? WHERE login = ?"))
		{
...
//	private static void finalSendMail(String username, String password, String recipientEmail, String ccEmail, String title, String message) throws AddressException, MessagingException
//	{
