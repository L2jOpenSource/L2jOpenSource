...
public class FightForThroneEvent extends AbstractFightClub
{
	...
	public Boolean canAttack(Creature actor, Creature target, Skill skill, boolean force)
	{
		if ((target.isSiegeGuard()) || (actor.isSiegeGuard()))
		{
			...
			if (getFightClubPlayer(target, new String[] { FIGHTING_PLAYERS }) != null)
			{
				defender = getFightClubPlayer(target, new String[] { FIGHTING_PLAYERS }).getTeam().getTeamType() == FightClubTeamType.DEFENDER;
			}
			else if (getFightClubPlayer(actor, new String[]	{ FIGHTING_PLAYERS }) != null)
			{
				defender = getFightClubPlayer(actor, new String[] { FIGHTING_PLAYERS }).getTeam().getTeamType() == FightClubTeamType.DEFENDER;
			}
			else
				return null;
			return !defender;
