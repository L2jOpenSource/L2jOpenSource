...
public class CharacterDAO
{
	...
	public static List<Integer> getCharIdsFromAccount(String account)
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("SELECT obj_Id FROM characters WHERE account_name=? AND accesslevel >= 0"))
		{
			statement.setString(1, account);
			try (ResultSet rset = statement.executeQuery())
			{
	...
	public static List<String> getCharNamesFromAccount(String account)
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("SELECT char_name FROM characters WHERE account_name=? AND accesslevel >= 0"))
		{
			statement.setString(1, account);
			try (ResultSet rset = statement.executeQuery())
			{
	...
	public boolean checkNameExists(String name)
	{
		...
		if (ConfigHolder.getBool("EnableMerge"))
		{
			try (Connection con = MergeDatabaseFactory.getInstance().getConnection();
			     PreparedStatement statement = con.prepareStatement("SELECT COUNT(old_login) FROM merge_data WHERE finished=0 AND new_char_name_1 = ? OR new_char_name_2 = ? OR new_char_name_3 = ? OR new_char_name_4 = ? OR new_char_name_5 = ? OR new_char_name_6 = ? OR new_char_name_7 = ? OR new_char_name_8 = ?"))
			{
				statement.setString(1, name);
	...
	public static String getLastHWIDByName(String playerName)
	{
		String result = "";
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("SELECT lastHWID FROM characters WHERE char_name=?"))
		{
