		...
		if (activeChar.getPlayerAccess().Menu)
		{
			switch (command)
			{
				...
				case admin_ready:
					try
					{
						String[] message = { activeChar.getName() + " is now available via Petition System."};
				...
				case admin_debugdata:
				{
					...
					try
					{
			            ...
			            for (Method method : Player.class.getDeclaredMethods())
			            {
			            	...
			            	if (method.getName().startsWith("is") && method.getParameterCount() == 0 && (method.getModifiers() & Modifier.PUBLIC) != 0
