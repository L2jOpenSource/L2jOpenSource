		...
		switch (command)
		{
			...
			case admin_clone:
			{
				try
				{
					...
					if (CharacterDAO.getInstance().getObjectIdByName(name) > 0 || FakePlayersTable.getActiveFakePlayers().contains(name)
						|| FakePlayersTable.isRealTimeFakePlayerExist(name))
					{
