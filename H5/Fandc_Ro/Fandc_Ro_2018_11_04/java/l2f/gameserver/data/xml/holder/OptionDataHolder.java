		...
		switch (filter)
		{
			case ACTIVE_SKILL:
			{
				for (OptionDataTemplate option : _templates.values())
				{
					...
					if (!options.containsKey(option.getSkills().get(0).getId())
						|| options.get(option.getSkills().get(0).getId()).getSkills().get(0).getLevel() < option.getSkills().get(0).getLevel())
		...
		for (OptionDataTemplate option : options.values())
		{
			...
			for (OptionDataTemplate option2 : options.values())
			{
				...
				switch (filter)
				{
					case CHANCE_SKILL:
					{
						if (skill.getName().equalsIgnoreCase(option2.getTriggerList().get(0).getSkill().getName())
							&& skill.getMagicLevel() < option2.getTriggerList().get(0).getSkill().getMagicLevel())
						{
							mustAddSkill = false;
						}
						break;
					...
					case ACTIVE_SKILL:
					{
						if (skill.getName().equalsIgnoreCase(option2.getSkills().get(0).getName())
							&& ((skill.getPower() > 0 && skill.getPower() < option2.getSkills().get(0).getPower()) || (skill.getPower() <= 0 && skill.getMagicLevel() < option2.getSkills().get(0).getMagicLevel())))
						{
							mustAddSkill = false;
						}
						break;
					...
					{
						if (skill.getName().equalsIgnoreCase(option2.getSkills().get(0).getName())
							&& skill.getMagicLevel() < option2.getSkills().get(0).getMagicLevel())
						{
							mustAddSkill = false;
						}
						break;
