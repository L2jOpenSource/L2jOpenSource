		...
		for (GameServer gs : GameServerManager.getInstance().getGameServers())
		{
			try
			{
				...
				if (proxies.containsKey(gs.getId()))
				{
					...
					_servers.add(new ServerData(gs.getId(), proxyIp, gs.getPort(), gs.isPvp(), gs.isShowingBrackets(), gs.getServerType(), gs.getOnline(), gs.getMaxPlayers(), gs.isOnline(), entry == null ? 0 : (int) entry.getKey(), gs.getAgeLimit(),
					                            entry == null ? ArrayUtils.EMPTY_INT_ARRAY : (int[]) entry.getValue()));
				}
				else
					_servers.add(new ServerData(gs.getId(), gs.getExternalHost(), gs.getPort(), gs.isPvp(), gs.isShowingBrackets(), gs.getServerType(), gs.getOnline(), gs.getMaxPlayers(), gs.isOnline(), entry == null ? 0 : (int) entry.getKey(), gs.getAgeLimit(),
					                            entry == null ? ArrayUtils.EMPTY_INT_ARRAY : (int[]) entry.getValue()));
			}
