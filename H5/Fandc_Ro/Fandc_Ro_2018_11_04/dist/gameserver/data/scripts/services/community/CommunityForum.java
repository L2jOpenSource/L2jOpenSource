...
public class CommunityForum implements ScriptFile, ICommunityBoardHandler
{
	...
	private void showMainPage(Player player)
	{
		...
		{
			html = html.replace("%marketLastPost%", "<font color=" + marketBoard.getLastPost().getWriter().getMemberGroup().getColor() + ">" + marketBoard.getLastPost().getWriter().getMemberName() + "</font>");
...
*/
		...
		{
			html = html.replace("%bugLastPost%", "<font color=" + suggestionsBoard.getLastPost().getWriter().getMemberGroup().getColor() + ">" + suggestionsBoard.getLastPost().getWriter().getMemberName() + "</font>");
	...
	private void showPostPage(Player player, ForumPost post)
	{
		...
		// Message
		html = html.replace("%message%", CommunityForum.convertQuotes(post.getMessage(), "<table cellspacing=8><tr><td fixwidth=568><table width=568 cellspacing=3 bgcolor=16595a><tr><td fixwidth=568><font color=63bea6>Quote By: ",
