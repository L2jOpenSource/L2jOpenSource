...
public class MergeCommunity implements ScriptFile, ICommunityBoardHandler
{
	...
	{
		return new String[] { "_merge", "_mergeFinalize", "_bbshome", "_bbsgetfav", "_bbsloc", "_bbsclan", "_bbsmemo", "_maillist_0_1_0_", "_friendlist_0_" };
	}
	...
	private static boolean checkNewAccountName(String accountNameToCheck)
	{
		...
		try (Connection con = MergeAuthDatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("SELECT COUNT(login) as existingLogins FROM accounts WHERE login = ?"))
		{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("SELECT COUNT(new_login) AS existingLogins FROM merge_data WHERE new_login = ?"))
		{
			statement.setString(1, accountNameToCheck);
			try (ResultSet rset = statement.executeQuery())
			{
	...
	private static boolean checkNewCharName(String charNameToCheck)
	{
		...
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement mergeCharsStatement = con.prepareStatement("SELECT old_login FROM merge_data WHERE new_char_name_1 = ? OR new_char_name_2 = ? OR new_char_name_3 = ? OR new_char_name_4 = ? OR new_char_name_5 = ? OR new_char_name_6 = ? OR new_char_name_7 = ? OR new_char_name_8 = ?"))
		{
	...
	private static boolean checkNewClanName(int oldClanId, String clanNameToCheck)
	{
		if (!MergeCommunity.CLAN_NAME_PATTERN.matcher(clanNameToCheck).matches())
			return false;
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement statement = con.prepareStatement("SELECT new_clan_name FROM merge_data_clan WHERE old_clan_id = ?"))
		{
	...
	{
		try (Connection con = DatabaseFactory.getInstance().getConnection();
		     PreparedStatement mergeCharsStatement = con.prepareStatement("SELECT new_login FROM merge_data WHERE old_login = ?"))
		{
