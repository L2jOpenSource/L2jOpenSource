			...
			switch (npc.getId())
			{
				case DEAD_SEEKER:
				{
					if (qs.isMemoState(7) && !hasQuestItems(killer, REPORT_EAST))
					{
						if (getRandom(100) < 20)
						{
							giveItems(killer, REPORT_EAST, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case MARSH_STAKATO_DRONE:
				{
					if (qs.isMemoState(7) && !hasQuestItems(killer, REPORT_NORTH))
					{
						if (getRandom(100) < 20)
						{
							giveItems(killer, REPORT_NORTH, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case BREKA_ORC_WARRIOR:
				{
					if (qs.isMemoState(2) && (getQuestItemsCount(killer, BREKA_ORC_HEAD) < 20))
					{
						...
						{
							giveItems(killer, BREKA_ORC_HEAD, 1 * (int) Config.RATE_QUEST_DROP);
							qs.setCond(3, true);
						}
						else
						{
