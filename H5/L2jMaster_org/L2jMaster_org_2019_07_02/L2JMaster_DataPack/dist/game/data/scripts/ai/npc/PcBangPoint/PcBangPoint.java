...
{
	// Item
	private static final int CAFE_100000 = 15359; // 100,000 PC Cafe points wining lottery
	private static final int CAFE_10000 = 15360; // 10,000 PC Cafe points wining lottery
	private static final int CAFE_2000 = 15361; // 2,000 PC Cafe points wining lottery
	private static final int CAFE_1000 = 15362; // 1,000 PC Cafe points wining lottery
	private static final int CAFE_100 = 15363; // 100 PC Cafe points wining lottery
	private static final int CAFE_INSTANT_LOTTERY = 15358; // For PC Cafe exclusive Instant Lottery
	...
	private final int WYVERN_DELAY_TIME = 5;
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("wyvern"))
		{
			...
			else if (player.getPcBangPoints() >= 2500)
			{
				if (WYVERN_ACTIVE)
				{
					htmltext = "wyvern02.htm";
				}
				else
				{
					...
					startQuestTimer("WYVERN_EFFECTS", WYVERN_DELAY_TIME * 60 * 1000, null, player, false);
					WYVERN_ACTIVE = true;
					return null;
				}
			}
		...
		else if (event.equalsIgnoreCase("Won100000"))
		{
			if ((player.getInventory().getItemByItemId(CAFE_100000) == null))
			{
				htmltext = "lottery_fail01.htm";
			}
			else if ((player.getPcBangPoints() + 100000) >= Config.PC_BANG_MAX_POINTS)
			{
				final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.YOU_EXCEEDED_MAX_OF_PC_CAFE_POINTS);
				player.sendPacket(sm);
				return htmltext = "lottery_fail02.htm";
			}
			else
			{
				takeItems(player, CAFE_100000, 1);
				final int cost = player.getPcBangPoints() + (100000);
				player.setPcBangPoints(cost);
				SystemMessage smsgpc = SystemMessage.getSystemMessage(SystemMessageId.ACQUIRED_S1_PCPOINT);
				smsgpc.addInt(100000);
				player.sendPacket(smsgpc);
				player.sendPacket(new ExPCCafePointInfo(player.getPcBangPoints(), 100000, 1));
				return null;
			}
		}
		else if (event.equalsIgnoreCase("Won10000"))
		{
			if ((player.getInventory().getItemByItemId(CAFE_10000) == null))
			{
				htmltext = "lottery_fail01.htm";
			}
			else if ((player.getPcBangPoints() + 10000) >= Config.PC_BANG_MAX_POINTS)
			{
				return htmltext = "lottery_fail02.htm";
			}
			else
			{
				takeItems(player, CAFE_10000, 1);
				final int cost = player.getPcBangPoints() + (10000);
				player.setPcBangPoints(cost);
				SystemMessage smsgpc = SystemMessage.getSystemMessage(SystemMessageId.ACQUIRED_S1_PCPOINT);
				smsgpc.addInt(10000);
				player.sendPacket(smsgpc);
				player.sendPacket(new ExPCCafePointInfo(player.getPcBangPoints(), 10000, 1));
				return null;
			}
		}
		else if (event.equalsIgnoreCase("Won2000"))
		{
			if ((player.getInventory().getItemByItemId(CAFE_2000) == null))
			{
				htmltext = "lottery_fail01.htm";
			}
			else if ((player.getPcBangPoints() + 2000) >= Config.PC_BANG_MAX_POINTS)
			{
				final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.YOU_EXCEEDED_MAX_OF_PC_CAFE_POINTS);
				player.sendPacket(sm);
				return htmltext = "lottery_fail02.htm";
			...
			{
				takeItems(player, CAFE_2000, 1);
				final int cost = player.getPcBangPoints() + (2000);
				player.setPcBangPoints(cost);
				SystemMessage smsgpc = SystemMessage.getSystemMessage(SystemMessageId.ACQUIRED_S1_PCPOINT);
				smsgpc.addInt(2000);
				player.sendPacket(smsgpc);
				player.sendPacket(new ExPCCafePointInfo(player.getPcBangPoints(), 2000, 1));
				return null;
			}
		}
		else if (event.equalsIgnoreCase("Won1000"))
		{
			if ((player.getInventory().getItemByItemId(CAFE_1000) == null))
			{
				htmltext = "lottery_fail01.htm";
			}
			else if ((player.getPcBangPoints() + 1000) >= Config.PC_BANG_MAX_POINTS)
			{
				final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.YOU_EXCEEDED_MAX_OF_PC_CAFE_POINTS);
				player.sendPacket(sm);
				return htmltext = "lottery_fail02.htm";
			...
			{
				takeItems(player, CAFE_1000, 1);
				final int cost = player.getPcBangPoints() + (1000);
				player.setPcBangPoints(cost);
				SystemMessage smsgpc = SystemMessage.getSystemMessage(SystemMessageId.ACQUIRED_S1_PCPOINT);
				smsgpc.addInt(1000);
				player.sendPacket(smsgpc);
				player.sendPacket(new ExPCCafePointInfo(player.getPcBangPoints(), 1000, 1));
				return null;
			}
		}
		else if (event.equalsIgnoreCase("Won100"))
		{
			if ((player.getInventory().getItemByItemId(CAFE_100) == null))
			{
				htmltext = "lottery_fail01.htm";
			}
			else if ((player.getPcBangPoints() + 100) >= Config.PC_BANG_MAX_POINTS)
			{
				final SystemMessage sm = SystemMessage.getSystemMessage(SystemMessageId.YOU_EXCEEDED_MAX_OF_PC_CAFE_POINTS);
				player.sendPacket(sm);
				return htmltext = "lottery_fail02.htm";
			...
			{
				takeItems(player, CAFE_100, 1);
				final int cost = player.getPcBangPoints() + (100);
				player.setPcBangPoints(cost);
				SystemMessage smsgpc = SystemMessage.getSystemMessage(SystemMessageId.ACQUIRED_S1_PCPOINT);
				smsgpc.addInt(100);
				player.sendPacket(smsgpc);
				player.sendPacket(new ExPCCafePointInfo(player.getPcBangPoints(), 100, 1));
				return null;
			}
		}
		else if (event.equalsIgnoreCase("giverandom"))
		{
			final QuestState qs = getQuestState(player, true);
			if (!qs.isNowAvailable())
			{
				htmltext = "lottery_notonedaty.htm";
			}
			else
			{
				giveItems(player, CAFE_INSTANT_LOTTERY, 1);
