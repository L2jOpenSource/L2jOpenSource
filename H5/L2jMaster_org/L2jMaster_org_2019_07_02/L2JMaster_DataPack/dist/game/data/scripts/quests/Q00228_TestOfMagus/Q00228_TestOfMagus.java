		...
		if ((qs != null) && qs.isStarted() && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getId())
			{
				case HARPY:
				{
					...
					{
						giveItems(killer, HARPYS_FEATHER, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case MARSH_STAKATO_DRONE:
				{
					...
					{
						giveItems(killer, DAZZLING_DROP, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case WYRM:
				{
					if (hasQuestItems(killer, SCORE_OF_ELEMENTS, SYLPH_CHARM) && (getQuestItemsCount(killer, WYRMS_WINGBONE) < 10))
					{
						if (getRandom(100) < 50)
						{
							giveItems(killer, WYRMS_WINGBONE, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case WINDSUS:
				{
					if (hasQuestItems(killer, SCORE_OF_ELEMENTS, SYLPH_CHARM) && (getQuestItemsCount(killer, WINDSUS_MANE) < 10))
					{
						if (getRandom(100) < 50)
						{
							giveItems(killer, WINDSUS_MANE, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case ENCHANTED_MONSTEREYE:
				{
					...
					{
						giveItems(killer, ENCHANTED_MONSTER_EYE_SHELL, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case ENCHANTED_STOLEN_GOLEM:
				{
					...
					{
						giveItems(killer, ENCHANTED_GOLEM_POWDER, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case ENCHANTED_IRON_GOLEM:
				{
					...
					{
						giveItems(killer, ENCHANTED_IRON_GOLEM_SCRAP, 1 * (int) Config.RATE_QUEST_DROP);
