	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		switch (npc.getId())
		{
			...
			case GRANITE_GOLEM:
			{
				...
				if (qs != null)
				{
					giveItems(qs.getPlayer(), GRANITE_WHETSTONE, 7 * (int) Config.RATE_QUEST_DROP);
			...
			case SILENOS:
			{
				...
				if (qs != null)
				{
					giveItems(qs.getPlayer(), BRAIDED_YARN, 10 * (int) Config.RATE_QUEST_DROP);
			...
			case GHOUL:
			{
				...
				if (qs != null)
				{
					giveItems(qs.getPlayer(), GRAY_BONE_POWDER, 5 * (int) Config.RATE_QUEST_DROP);
			...
			case DEAD_SEEKER:
			{
				...
				if (qs != null)
				{
					giveItems(qs.getPlayer(), RED_PIGMENT, 7 * (int) Config.RATE_QUEST_DROP);
			...
			case BREKA_ORC_WARRIOR:
			{
				...
				if (qs != null)
				{
					...
					{
						giveItems(qs.getPlayer(), DUNINGS_KEY, 1 * (int) Config.RATE_QUEST_DROP);
						playSound(qs.getPlayer(), Sound.ITEMSOUND_QUEST_ITEMGET);
					}
				}
				break;
