...
/**
 * Ol Mahums AI - flee and return with assistance
 * @author MaGa
 */

public final class SplinterStakatos extends AbstractNpcAI
{
	// NPC's
	...
	private SplinterStakatos()
	{
		super(SplinterStakatos.class.getSimpleName(), "ai/group_template");
		addAttackId(MOBS);
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon)
	{
		...
		else if ((npc.getCurrentHp() < (npc.getMaxHp() * 0.5)) && (npc.getCurrentHp() > (npc.getMaxHp() * 0.3)) && (attacker.getCurrentHp() > (attacker.getMaxHp() * 0.25)) && npc.hasAIValue("fleeX") && npc.hasAIValue("fleeY") && npc.hasAIValue("fleeZ") && (npc.getVariables().getInt("state") == 0) && (getRandom(100) < 50))
		{
			// Flee
