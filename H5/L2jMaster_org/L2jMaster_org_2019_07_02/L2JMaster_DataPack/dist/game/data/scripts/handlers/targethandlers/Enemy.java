	...
	public L2Object[] getTargetList(Skill skill, L2Character activeChar, boolean onlyFirst, L2Character target)
	{
		switch (skill.getAffectScope())
		{
			case SINGLE:
			{
				...
				if ((!activeChar.isPlayer() && !activeChar.isSummon() && (skill.getTargetType() == L2TargetType.ENEMY)) || //
					(activeChar.isServitor() && (activeChar.getSummon().getTemplate().getRace() == Race.SIEGE_WEAPON) && target.isDoor()) || //
					(skill.getId() == 21014) || (skill.getId() == 21015) || (skill.getId() == 21016) || // Scissors, Rock and Paper respectively
					(skill.getId() == 3260) || (skill.getId() == 3262)) // Forgiveness and Pardon respectively
				{
			...
			case SQUARE:
			{
				...
				for (L2Character character : knownCharacters)
				{
					...
					if (Util.checkIfInSquareRange(skill.getFanRangeValues()[0], skill.getFanRangeValues()[1], skill.getFanRangeValues()[2], skill.getFanRangeValues()[3], (L2Character) activeChar.getTarget(), character))
					{
						if (!Skill.checkForAreaOffensiveSkills(activeChar, character, skill, srcInArena))
						{
							continue;
			...
			case SQUARE_PB:
			{
				...
				for (L2Character character : knownCharacters)
				{
					...
					if (Util.checkIfInSquareRange(skill.getFanRangeValues()[0], skill.getFanRangeValues()[1], skill.getFanRangeValues()[2], skill.getFanRangeValues()[3], activeChar, character))
					{
						if (!Skill.checkForAreaOffensiveSkills(activeChar, character, skill, srcInArena))
						{
							continue;
