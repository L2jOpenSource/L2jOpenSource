		...
		if ((qs != null) && qs.isStarted() && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getId())
			{
				...
				case ANT_WARRIOR_CAPTAIN:
				{
					...
					{
						giveItems(killer, ANT_SOLDIER_ACID, 2 * (int) Config.RATE_QUEST_DROP);
				...
				case HARPY:
				{
					...
					{
						giveItems(killer, HARPYS_DOWN, 4 * (int) Config.RATE_QUEST_DROP);
				...
				case WYRM:
				{
					...
					{
						giveItems(killer, WYRMS_TALON, 4 * (int) Config.RATE_QUEST_DROP);
				...
				case MARSH_SPIDER:
				{
					...
					{
						giveItems(killer, SPIDER_ICHOR, 4 * (int) Config.RATE_QUEST_DROP);
						if (getQuestItemsCount(killer, SPIDER_ICHOR) == 20)
						{
							playSound(killer, Sound.ITEMSOUND_QUEST_MIDDLE);
							if (getQuestItemsCount(killer, HARPYS_DOWN) >= 20)
				...
				case GUARDIAN_BASILISK:
				{
					...
					{
						giveItems(killer, PURE_MITHRIL_ORE, 2 * (int) Config.RATE_QUEST_DROP);
