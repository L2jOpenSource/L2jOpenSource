		...
		if ((qs != null) && qs.isStarted() && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getId())
			{
				case MONSTER_EYE_DESTREOYER:
				{
					...
					{
						giveItems(killer, MONSTER_EYE_DESTROYER_SKIN, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case MEDUSA:
				{
					...
					{
						giveItems(killer, MEDUSAS_BLOOD, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case GHOUL:
				{
					...
					{
						giveItems(killer, GHOULS_SKIN, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case BREKA_ORC_SHAMAN:
				{
					...
					{
						giveItems(killer, SHAMANS_NECKLACE, 1 * (int) Config.RATE_QUEST_DROP);
						if ((getQuestItemsCount(killer, MONSTER_EYE_DESTROYER_SKIN) >= 5) && (getQuestItemsCount(killer, SHAMANS_NECKLACE) == 5) && (getQuestItemsCount(killer, SHACKLES_SCALP) >= 2))
						{
							qs.setCond(17, true);
						}
				...
				case FETTERED_SOUL:
				{
					...
					{
						giveItems(killer, FETTERED_SOULS_ICHOR, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case ENCHANTED_GARGOYLE:
				{
					...
					{
						giveItems(killer, ENCHANTED_GARGOYLES_NAIL, 1 * (int) Config.RATE_QUEST_DROP);
				...
				case LETO_LIZARDMAN_WARRIOR:
				{
					...
					{
						giveItems(killer, BROWN_SCROLL_SCRAP, 1 * (int) Config.RATE_QUEST_DROP);
