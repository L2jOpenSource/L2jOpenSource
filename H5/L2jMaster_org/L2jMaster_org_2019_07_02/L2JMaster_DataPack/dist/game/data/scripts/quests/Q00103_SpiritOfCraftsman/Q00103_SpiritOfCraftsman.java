...
{
	// NPCs
	private static final int BLACKSMITH_KARROD = 30307;
	private static final int CECON = 30132;
	private static final int HARNE = 30144;
	// Items
	private static final int KARRODS_LETTER = 968;
	...
	public Q00103_SpiritOfCraftsman()
	{
		...
		addStartNpc(BLACKSMITH_KARROD);
		addTalkId(BLACKSMITH_KARROD, CECON, HARNE);
		addKillId(MARSH_ZOMBIE, DOOM_SOLDIER, SKELETON_HUNTER, SKELETON_HUNTER_ARCHER);
		registerQuestItems(KARRODS_LETTER, CECKTINONS_VOUCHER1, CECKTINONS_VOUCHER2, SOUL_CATCHER, PRESERVE_OIL, ZOMBIE_HEAD, STEELBENDERS_HEAD, BONE_FRAGMENT);
	}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			...
			case "30307-05.htm":
			{
				...
				{
					qs.startQuest();
					giveItems(player, KARRODS_LETTER, 1);
					htmltext = event;
				}
				break;
			}
	...
	public String onTalk(L2Npc npc, L2PcInstance talker)
	{
		...
		switch (npc.getId())
		{
			case BLACKSMITH_KARROD:
			{
				...
				{
					if (hasAtLeastOneQuestItem(talker, KARRODS_LETTER, CECKTINONS_VOUCHER1, CECKTINONS_VOUCHER2))
					{
					...
					else if (hasQuestItems(talker, STEELBENDERS_HEAD))
					{
						if (!talker.isMageClass())
						{
							...
							takeItems(talker, STEELBENDERS_HEAD, 1);
			...
			case CECON:
			{
				if (qs.isStarted())
				{
					if (hasQuestItems(talker, KARRODS_LETTER))
					{
						qs.setCond(2, true);
						takeItems(talker, KARRODS_LETTER, 1);
