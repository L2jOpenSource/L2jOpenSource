 ...
 */
package gracia.instances.SeedOfInfinity.HeartOfInfinityDefence;
...
/**
 * Seed of Infinity (Heart Of Infinity Defense) instance zone.
 * @author MaGa1, Sacrifice
 */
public class HeartOfInfinityDefence extends AbstractNpcAI
{
	...
	private static final int INSTANCE_ID = 122;
	...
	public HeartOfInfinityDefence()
	{
		super(HeartOfInfinityDefence.class.getSimpleName(), "gracia/instances");
	...
	protected int enterInstance(L2PcInstance player, String template, int[] coords)
	{
		...
		else
		{
			for (L2PcInstance partyMember : player.getParty().getCommandChannel().getMembers())
			{
				...
				if (partyMember.getQuestState(HeartOfInfinityDefence.class.getSimpleName()) == null)
				{
	...
	private void conquestBegins(final HIDWorld world)
	{
		ThreadPoolManager.getInstance().scheduleGeneral(() ->
		{
			...
			for (int[] spawn1 : ROOMS_MOBS)
			{
				...
				{
					L2Npc npc3 = addSpawn(spawn1[0], spawn1[1], spawn1[2], spawn1[3], spawn1[4], false, 0, false, world.getInstanceId());
					npc3.getSpawn().setRespawnDelay(spawn1[5]);
					npc3.getSpawn().setAmount(1);
					if (spawn1[5] > 0)
					{
						npc3.getSpawn().startRespawn();
					}
					else
					{
			...
			for (int[] spawn2 : ROOMS_TUMORS)
			{
				...
				{
					L2Npc npc4 = addSpawn(spawn2[0], spawn2[1], spawn2[2], spawn2[3], spawn2[4], false, 0, false, world.getInstanceId());
			...
			ThreadPoolManager.getInstance().scheduleGeneral(() ->
			{
				if (!conquestEnded)
				{
					if (!world.deadTumors.isEmpty())
					{
						for (L2Npc npc1 : world.deadTumors)
						{
							if (npc1 != null)
							{
								npc1.deleteMe();
					...
					for (int[] spawn : ROOMS_ALIVE_TUMORS)
					{
						...
						{
							L2Npc npc2 = addSpawn(spawn[0], spawn[1], spawn[2], spawn[3], spawn[4], false, 0, false, world.getInstanceId());
							npc2.setCurrentHp(npc2.getMaxHp() * .5);
	...
	private void spawnCoffin(L2Npc npc, HIDWorld world)
	{
		addSpawn(REGENERATION_COFFIN, npc.getLocation().getX(), npc.getLocation().getY(), npc.getLocation().getZ(), 0, false, 0, false, world.getInstanceId());
	}
	...
	{
		int npcId = npc.getId();
		QuestState st = player.getQuestState(HeartOfInfinityDefence.class.getSimpleName());
		if (st == null)
		{
	...
	public String onAggroRangeEnter(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if (tmpworld instanceof HIDWorld)
		{
			...
			if (npc.getId() == 18668)
			{
				...
				{
					addSpawn(mobs[Rnd.get(mobs.length)], npc.getLocation().getX(), npc.getLocation().getY(), npc.getLocation().getZ(), 0, false, 0, false, world.getInstanceId());
				}
				npc.doDie(npc);
			}
		}
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if (tmpworld instanceof HIDWorld)
		{
			...
			if (npc.getId() == ALIVETUMOR)
			{
				...
				npc.deleteMe();
				world.deadTumor = addSpawn(DEADTUMOR, loc.getX(), loc.getY(), loc.getZ(), 0, false, 0, false, world.getInstanceId());
				...
				ThreadPoolManager.getInstance().scheduleGeneral(() ->
				{
					...
					L2Npc alivetumor = addSpawn(ALIVETUMOR, loc.getX(), loc.getY(), loc.getZ(), 0, false, 0, false, world.getInstanceId());
	...
	protected void notifyWagonArrived(L2Npc npc, HIDWorld world)
	{
		...
		else
		{
			...
			// int[] spawn = SOULWAGON_SPAWN[Rnd.get(0, SOULWAGON_SPAWN.length - 1)];
			// addSpawn(spawn[0], spawn[1], spawn[2], spawn[3], spawn[4], false, 0, false, world.getInstanceId()); //Need Add later
		}
	}
