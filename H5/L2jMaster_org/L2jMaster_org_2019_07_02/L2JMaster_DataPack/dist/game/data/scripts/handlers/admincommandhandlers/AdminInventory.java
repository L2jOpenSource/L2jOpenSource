...
public class AdminInventory implements IAdminCommandHandler
{
	...
	private void showItemsPage(L2PcInstance activeChar, int page)
	{
		...
		{
			int pagenr = x + 1;
			sbPages.append("<td><button value=\"" + String.valueOf(pagenr) + "\" action=\"bypass -h admin_show_inv " + String.valueOf(x) + "\" width=20 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
		}
		...
		for (int i = itemsStart; i < itemsEnd; i++)
		{
			...
			sbItems.append("<td><button action=\"bypass -h admin_del_item " + String.valueOf(items[i].getObjectId()) + "\" width=16 height=16 back=\"L2UI_ct1.Button_DF_Delete\" fore=\"L2UI_ct1.Button_DF_Delete\">" + "</td></tr>");
		}
