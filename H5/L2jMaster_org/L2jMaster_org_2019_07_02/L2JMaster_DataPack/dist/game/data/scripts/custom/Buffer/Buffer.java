...
public class Buffer extends Quest
{
	...
	private static final String QUEST_LOADING_INFO = "Buffer";
	private static final int NPC_ID = 9999;
	...
	private String rebuildMainHtml(QuestState st)
	{
		...
		if (MESSAGE.length() > 0)
		{
			MAIN_HTML_MESSAGE += "<BR1><table width=100% border=0 cellspacing=0 cellpadding=1 bgcolor=444444><tr>" + "<td><font color=00FFFF>Preset:</font></td><td align=right><font color=LEVEL>" + formatAdena(BUFF_SET_PRICE) + "</font> adena</td></tr></table>"
	...
	private String generateScheme(QuestState st)
	{
		...
		{
			PreparedStatement rss = con.prepareStatement("SELECT * FROM buffer_scheme_list WHERE player_id=?");
	...
	private int getBuffCount(String scheme)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement rss = con.prepareStatement("SELECT buff_class FROM buffer_scheme_contents WHERE scheme_id=?");
	...
	private String getBuffType(int id)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement act = con.prepareStatement("SELECT buffType FROM buffer_buff_list WHERE buffId=? LIMIT 1");
			act.setInt(1, id);
	...
	private boolean isEnabled(int id, int level)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement act = con.prepareStatement("SELECT canUse FROM buffer_buff_list WHERE buffId=? AND buffLevel=? LIMIT 1");
			act.setInt(1, id);
			act.setInt(2, level);
			ResultSet rs = act.executeQuery();
			if (rs.next())
	...
	private boolean isUsed(String scheme, int id, int level)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement rss = con.prepareStatement("SELECT id FROM buffer_scheme_contents WHERE scheme_id=? AND skill_id=? AND skill_level=? LIMIT 1");
			rss.setString(1, scheme);
			rss.setInt(2, id);
			rss.setInt(3, level);
			ResultSet action = rss.executeQuery();
	...
	private int getClassBuff(String id)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement getTipo = con.prepareStatement("SELECT buff_class FROM buffer_buff_list WHERE buffId=?");
	...
	private Buffer(int i, String string, String string2)
	{
		super(-1, Buffer.class.getSimpleName(), "custom");
		addStartNpc(NPC_ID);
	...
	private String buildHtml(String buffType)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement getList = con.prepareStatement("SELECT buffId,buffLevel FROM buffer_buff_list WHERE buffType=\"" + buffType + "\" AND canUse=1  ORDER BY Buff_Class ASC, id");
	...
	private String viewAllSchemeBuffs(String scheme, String page, String action)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			if (action.equals("add"))
			{
				...
				String QUERY = "SELECT * FROM buffer_buff_list WHERE buffType IN (" + generateQuery(BUFF_COUNT, DANCE_SONG) + ") AND canUse=1 ORDER BY Buff_Class ASC, id";
			...
			else if (action.equals("remove"))
			{
				...
				String QUERY = "SELECT * FROM buffer_scheme_contents WHERE scheme_id=? ORDER BY Buff_Class ASC, id";
				PreparedStatement getBuffCount = con.prepareStatement(QUERY);
	...
	private String viewAllBuffs(String type, String typeName, String page)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			...
			if (type.equals("set"))
			{
				getBuffCount = con.prepareStatement("SELECT * FROM buffer_buff_list WHERE buffType IN (" + generateQuery(0, 0) + ") AND canUse=1");
			}
			else
			{
				getBuffCount = con.prepareStatement("SELECT * FROM buffer_buff_list WHERE buffType=?");
	...
	private void manageSelectedBuff(String buffPosId, String canUseBuff)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement upd = con.prepareStatement("UPDATE buffer_buff_list SET canUse=? WHERE buffId=? AND buffLevel=? LIMIT 1");
	...
	private String manageSelectedSet(String id, String newVal, String opt3)
	{
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection())
		{
			PreparedStatement upd = con.prepareStatement("UPDATE buffer_buff_list SET forClass=? WHERE buffId=? AND bufflevel=?");
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (eventParam0)
		{
			...
			case "create":
			{
				...
				try (Connection con = ConnectionFactory.getInstance().getConnection())
				{
					PreparedStatement ins = con.prepareStatement("INSERT INTO buffer_scheme_list (player_id,scheme_name) VALUES (?,?)");
			...
			case "delete":
				try (Connection con = ConnectionFactory.getInstance().getConnection())
				{
					...
					rem = con.prepareStatement("DELETE FROM buffer_scheme_list WHERE id=? LIMIT 1");
					rem.setString(1, eventParam1);
					rem.executeUpdate();
					rem.close();
					rem = con.prepareStatement("DELETE FROM buffer_scheme_contents WHERE scheme_id=?");
					rem.setString(1, eventParam1);
					rem.executeUpdate();
					rem.close();
				}
			...
			case "add_buff":
			{
				...
				try (Connection con = ConnectionFactory.getInstance().getConnection())
				{
					PreparedStatement ins = con.prepareStatement("INSERT INTO buffer_scheme_contents (scheme_id,skill_id,skill_level,buff_class) VALUES (?,?,?,?)");
			...
			case "cast":
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					try (Connection con = ConnectionFactory.getInstance().getConnection())
					{
						PreparedStatement rss = con.prepareStatement("SELECT * FROM buffer_scheme_contents WHERE scheme_id=? ORDER BY id");
			...
			case "castBuffSet":
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					if (!getSummonbuff)
					{
						try (Connection con = ConnectionFactory.getInstance().getConnection())
						{
							PreparedStatement getSimilarNameCount = con.prepareStatement("SELECT buffId,buffLevel FROM buffer_buff_list WHERE forClass IN (?,?) ORDER BY id ASC");
	...
	{
		new Buffer(-1, Buffer.class.getSimpleName(), "custom");
		_log.info("Buffer Manager: Enabled.");
	}
}