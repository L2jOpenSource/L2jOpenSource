...
public final class Q00234_FatesWhisper extends Quest
{
	...
	// Monsters
	private static final int PLATINUM_TRIBE_SOLDIER = 20823;
	...
	public Q00234_FatesWhisper()
	{
		...
		addKillId(PLATINUM_TRIBE_SOLDIER, PLATINUM_TRIBE_ARCHER, PLATINUM_TRIBE_WARRIOR, PLATINUM_TRIBE_SHAMAN, PLATINUM_TRIBE_OVERLORD, GUARDIAN_ANGEL, SEAL_ANGEL);
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		switch (npc.getId())
		{
			...
			case MASTER_KASPAR:
			{
				...
				if (qs.isMemoState(8) && !qs.hasQuestItems(Q_RED_PIPETTE_KNIFE) && qs.hasQuestItems(Q_PIPETTE_KNIFE))
				{
					return "30833-03.html";
				}
				else if (qs.isMemoState(8) && qs.hasQuestItems(Q_RED_PIPETTE_KNIFE))
				{
				...
				else if (qs.isMemoState(8) && !qs.hasQuestItems(Q_RED_PIPETTE_KNIFE) && (qs.getQuestItemsCount(Q_BLOODY_FABRIC_Q0234) != 30) && (qs.getQuestItemsCount(Q_WHITE_FABRIC_Q0234)) != 0)
				{
					return "30833-03c.html";
				}
				else if (qs.isMemoState(8) && !qs.hasQuestItems(Q_RED_PIPETTE_KNIFE) && (qs.getQuestItemsCount(Q_BLOODY_FABRIC_Q0234) >= 30))
				{
					qs.giveItems(Q_MAESTRO_REORINS_MOLD, 1);
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		...
		if ((qs != null) && qs.isStarted() && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getId())
			{
				case PLATINUM_TRIBE_SOLDIER:
				case PLATINUM_TRIBE_ARCHER:
				case PLATINUM_TRIBE_WARRIOR:
				case PLATINUM_TRIBE_SHAMAN:
				case PLATINUM_TRIBE_OVERLORD:
				{
					if ((getQuestItemsCount(killer, Q_BLOODY_FABRIC_Q0234) < 30) && (getRandom(100) < 15))
					{
						giveItems(killer, Q_BLOODY_FABRIC_Q0234, 1);
						qs.takeItems(Q_WHITE_FABRIC_Q0234, 1);
						qs.playSound(Sound.ITEMSOUND_QUEST_ITEMGET);
						if (getQuestItemsCount(killer, Q_BLOODY_FABRIC_Q0234) >= 30)
						{
		...
		if ((qs != null) && qs.isStarted() && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getId())
			{
				...
				case SEAL_ANGEL:
				{
					if ((getQuestItemsCount(killer, Q_BLOODY_FABRIC_Q0234) < 30) && (getRandom(100) < 33))
					{
						giveItems(killer, Q_BLOODY_FABRIC_Q0234, 1);
						qs.takeItems(Q_WHITE_FABRIC_Q0234, 1);
						qs.playSound(Sound.ITEMSOUND_QUEST_ITEMGET);
