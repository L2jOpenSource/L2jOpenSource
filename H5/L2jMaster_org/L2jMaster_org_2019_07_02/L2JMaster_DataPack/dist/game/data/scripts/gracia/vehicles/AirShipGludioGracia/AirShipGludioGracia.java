...
public final class AirShipGludioGracia extends Quest implements Runnable
{
	...
	private static final VehiclePathPoint[] WARPGATE_TO_GRACIA =
	{
		...
		new VehiclePathPoint(-171822, 250061, 535),
		new VehiclePathPoint(-172595, 247737, 398),
		new VehiclePathPoint(-174538, 246185, 39),
		new VehiclePathPoint(-179440, 244651, 1337),
	...
	private static final int CHANCE_TO_SHOW_MOVIE = 30;
	private static final int MOVIE_ID = 1;
	
	private static final L2AirShipInstance SHIP = AirShipManager.getInstance().getNewAirShip(-149378, 252552, 198, 33837);
	
	private static int _cycle = 0;
	
	private static boolean _foundAtControllerGludio = false;
	private static L2Npc _atControllerGludio = null;
	
	private static boolean _foundAtControllerGracia = false;
	private static L2Npc _atControllerGracia = null;
	...
	{
		if (!_foundAtControllerGludio)
		{
			_foundAtControllerGludio = true;
			_atControllerGludio = findController();
		}
		if (_atControllerGludio != null)
		{
			_atControllerGludio.broadcastPacket(new NpcSay(_atControllerGludio.getObjectId(), Say2.NPC_SHOUT, _atControllerGludio.getId(), npcString));
		}
	}
	
	private final void broadcastInGracia(NpcStringId npcStringId)
	{
		if (!_foundAtControllerGracia)
		{
			_foundAtControllerGracia = true;
			_atControllerGracia = findController();
		}
		if (_atControllerGracia != null)
		{
			_atControllerGracia.broadcastPacket(new NpcSay(_atControllerGracia.getObjectId(), Say2.NPC_SHOUT, _atControllerGracia.getId(), npcStringId));
		}
	}
	...
	private final void checkThreadStatus()
	{
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (SHIP.isInDock() && SHIP.isInsideRadius(player, 600, true, false))
		{
			SHIP.addPassenger(player);
			player.stopMove(null);
			
			if ((getRandom(100) < CHANCE_TO_SHOW_MOVIE) && (npc.getId() == CONTROLLERS[0]))
			{
