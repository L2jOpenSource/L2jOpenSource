...
public class TvTLeaderboard
{
	...
	public Map<Integer, TvTRank> _ranks = new HashMap<>();
	...
	public void formRank()
	{
		...
		if (arTop == null)
		{
			Broadcast.toAllOnlinePlayers(new CreatureSay(1, Say2.CRITICAL_ANNOUNCE, "", ("TvTMaster: No winners at this time!")));
			_ranks.clear();
			return;
		}
		...
		Broadcast.toAllOnlinePlayers(new CreatureSay(1, Say2.CRITICAL_ANNOUNCE, "", ("TvTMaster: " + arTop.name + " is the winner for this time with " + arTop.kills + "/" + arTop.death + ". Next calculation in " + Config.RANK_TVT_INTERVAL + " min(s).")));
		if ((winner != null) && (Config.RANK_TVT_REWARD_ID > 0) && (Config.RANK_TVT_REWARD_COUNT > 0))
		{
			...
			{
				winner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S).addItemName(Config.RANK_TVT_REWARD_ID));
			}
			else
			{
