...
public class CraftLeaderboard
{
	...
	public Map<Integer, CraftRank> _ranks = new HashMap<>();
	...
	public void formRank()
	{
		...
		if (arTop == null)
		{
			Broadcast.toAllOnlinePlayers(new CreatureSay(1, Say2.CRITICAL_ANNOUNCE, "", ("CraftMaster: No winners at this time!")));
			_ranks.clear();
			return;
		}
		...
		Broadcast.toAllOnlinePlayers(new CreatureSay(1, Say2.CRITICAL_ANNOUNCE, "", ("Attention Players: " + arTop.name + " is the winner for this time with " + arTop.sucess + "/" + arTop.fail + ". Next calculation in " + Config.RANK_CRAFT_INTERVAL + " min(s).")));
		if ((winner != null) && (Config.RANK_CRAFT_REWARD_ID > 0) && (Config.RANK_CRAFT_REWARD_COUNT > 0))
		{
			...
			{
				winner.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EARNED_S2_S1_S).addItemName(Config.RANK_CRAFT_REWARD_ID));
			}
			else
			{
