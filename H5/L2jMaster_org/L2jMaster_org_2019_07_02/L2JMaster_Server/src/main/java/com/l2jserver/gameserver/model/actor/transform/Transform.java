		...
		if (template != null)
		{
			...
			if (!template.getSkills().isEmpty())
			{
				for (SkillHolder holder : template.getSkills())
				{
					...
					if (ConfigWD.WD_STACK_SUBCLASS_SKILL_ENABLE && player.getAllSkills().contains(skill))
					{
						continue;
					}
					
					if (!ConfigWD.WD_STACK_SUBCLASS_SKILL_ENABLE && !SkillTreesData.getInstance().isSkillAllowed(player, skill))
					{
			...
			if (!template.getAdditionalSkills().isEmpty())
			{
				for (AdditionalSkillHolder holder : template.getAdditionalSkills())
				{
					...
					if (ConfigWD.WD_STACK_SUBCLASS_SKILL_ENABLE && (player.getLevel() >= holder.getMinLevel()) && player.getAllSkills().contains(skill))
					{
						continue;
					}
					
					if (!ConfigWD.WD_STACK_SUBCLASS_SKILL_ENABLE && (player.getLevel() >= holder.getMinLevel()) && !SkillTreesData.getInstance().isSkillAllowed(player, skill))
					{
						player.removeSkill(skill, false, skill.isPassive());
					}
				}
